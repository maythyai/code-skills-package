...
### Changed
- Removed OpenSpec layer (`csp-spec/`); spec-driven methodology merged into `csp-meta/skills/spec-driven-development/` and `csp-spec-contract/`
...
### Removed
- OpenSpec OPSX commands: `/csp-propose`, `/csp-apply`, `/csp-sync`, `/csp-archive`, etc.
- External `openspec` CLI dependency references
### Added (historical draft)
- `csp-skill-optimizer` — skill 自优化元技能，基于用户反馈自动收集信号、分析缺口、优化 skill 并支持 PR 回上游
- Full skill audit across all 6 layers (router, meta, workflow, spec, patterns, runtime)
...
### Added
- Five-layer architecture (L0-L4, spec layer absorbed) with token-optimized sharded loading
...
- Spec-driven development (later absorbed into csp-meta + csp-workflow in Unreleased)
