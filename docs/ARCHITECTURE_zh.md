...
> 版本: v0.7.1 | 日期: 2026-07-04
> 状态: 状态感知路由 + 置信度评分 + 技能知识图谱增强
...
4. **自动化路由** — 用户描述任务,系统自动选择最合适的 skill 组合
5. **状态感知** — 基于项目状态、开发阶段、技术栈自动调整路由策略
6. **统一命名空间** — 所有 skill 和命令均仅使用 `csp-` 前缀
...
- 所有连接符统一使用短横线 `-`,不使用冒号 `:`
- slash commands 统一使用 `/csp-xxx` 格式
- 文档和代码注释中不引用原始项目前缀
...
技能知识图谱 (Skill Knowledge & Practice Graph) 是一个表示技能之间关系的图结构，用于:
- 依赖分析：确定技能间的前置依赖关系
- 影响分析：当修改某技能时，分析受影响的其他技能
- 路径查找：寻找从一个技能到另一个技能的最佳路径
- 推荐系统：基于当前技能推荐相关技能
...
- **依赖检查**: 激活技能前检查是否满足前置依赖
- **智能推荐**: 基于图谱结构推荐相关技能
- **冲突检测**: 检测互斥技能组合
- **路径优化**: 为复杂任务规划最优技能执行路径
...
路由器（L0 常驻）：
  ...
  3. 状态感知：当前 phase=building → 偏向 build 类 skill
  4. 组合决策：加载 csp-code-review + csp-react-reviewer + csp-tdd
  ↓
执行：输出 REVIEW.md
```

**安装即用：** `./install.sh` 一键部署到 22 个平台，无需任何配置。
...
├─────────────────────────────────────────────────┤
│  信号层 2：状态感知（权重 25%）                  │
│  ─ git=dirty → 偏向 debug 类                     │
│  ─ phase=review → 偏向 review 类                 │
│  ─ test=failing → 偏向 debug 类                  │
├─────────────────────────────────────────────────┤
│  信号层 3：触发词匹配（权重 30%）                │
...
路由器决策过程：
  信号 1：无显式指令 → 跳过
  信号 2：状态检测：git=clean, phase=planning → 偏向 plan 类
  信号 3：触发词匹配："规划" → csp-plan-phase (L2) [权重 50]
  信号 4：无特定文件类型 → 跳过
  信号 5：检测到 Django 项目 → csp-django-security (L4) [权重 40]
  信号 6：历史偏好 → 跳过
...
**状态**: git=clean | lang=python | phase=planning
...
**SKPG 提示**: csp-plan-phase 依赖 csp-brainstorming
```
