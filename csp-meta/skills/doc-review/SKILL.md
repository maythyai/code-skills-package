...
**Headless mode** changes the interaction model, not the classification boundaries. csp-doc-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
```
Skill("csp-doc-review", "mode:headless docs/plans/my-plan.md")
```
...
**If no document is specified (interactive mode):** Ask which document to review, or find the most recent in `docs/brainstorms/` or `docs/plans/` using a file-search/glob tool.

**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with: Skill(\"csp-doc-review\", \"mode:headless <path>\")" without dispatching agents.
...
```
Reviewing with:
- csp-coherence-reviewer (always-on)
- csp-feasibility-reviewer (always-on)
- csp-scope-guardian-reviewer -- plan has 12 requirements across 3 priority levels
- csp-security-lens-reviewer -- plan adds API endpoints with auth flow
```

### Build Agent List

Always include:
- `csp-coherence-reviewer`
- `csp-feasibility-reviewer`

Add activated conditional personas:
- `csp-product-lens-reviewer`
- `csp-design-lens-reviewer`
- `csp-security-lens-reviewer`
- `csp-scope-guardian-reviewer`
- `csp-adversarial-reviewer`

### Dispatch

Dispatch agents using **bounded parallelism** with the platform's subagent primitive (e.g., `Agent` in Claude Code). Respect the current harness's active-subagent limit: queue selected reviewers, dispatch only as many as the harness accepts, and fill freed slots as reviewers complete. Treat active-agent/thread/concurrency-limit spawn errors as backpressure, not reviewer failure: leave the reviewer queued and retry after a slot frees. Record a reviewer as failed only after a successful dispatch times out/fails, or when dispatch fails for a non-capacity reason.
