...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new csp-plan / csp-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- **Matching predicate:** same as R30 — `normalize(section) + normalize(title)` fingerprint augmented with evidencsp-substring overlap check (>50%). If a current-round finding matches a prior-round rejected finding on fingerprint AND evidence overlap, drop the current-round finding.
...
- Requirements document → `csp-plan`
- Plan document → `csp-work`
...
- Do not modify caller skills (csp-brainstorm, csp-plan, or external plugin skills that invoke csp-doc-review)
