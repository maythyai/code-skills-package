...
- **状态感知路由**: `state-detector.mjs` 自动检测 Git/技术栈/项目阶段/测试状态
- **置信度路由器**: `confidence-router.mjs` 使用加权评分（关键词 40% + 意图 30% + 上下文 30%）
- **SKILL.md v2 规范**: 增强 phase/domain/role/model_rules/anti_rationalizations 字段
- **技能元数据注册表**: `skill-metadata.yaml` 集中元数据，供路由器快速查询
- **技能知识图谱 (SKPG)**: 轻量级 JSON 图谱建模技能关系/分类（580+ 节点，800+ 边）
- **意图模式识别**: triggers.yaml 中的高级意图分类，新增 9 个正则 pattern
- **V2 技能升级**: 21 个高频技能升级到 v2 格式，带结构化元数据
- **工作流模式引擎**: `csp-workflow-schema` — 声明式 JSON 工作流引擎，支持阶段定义、条件表达式、失败策略、artifact 传递
- **预算强制器**: `csp-budget-enforcer` — 四级 Token 预算降级（OK→WARNING→SOFT→HARD），自动模型降级
- **并行 Worktree**: `csp-parallel-worktree` — 并行任务自动 worktree 分配，冲突检测，自动合并
- **复杂度分类器**: `csp-complexity-classifier` — 启发式任务复杂度分类（simple/medium/complex）
- **模型选择器**: `csp-model-selector` — 复杂度→模型自动映射（haiku/sonnet/opus）
