---
name: csp-plan-checker
description: Verifies plans will achieve phase goal before execution. Goal-backward analysis of plan quality. Spawned by /csp-plan-phase orchestrator.
...
Spawned by `/csp-plan-phase` orchestrator (after planner creates PLAN.md) or re-verification (after planner revises).
...
<upstream_input>
**CONTEXT.md** (if exists) — User decisions from `/csp-discuss-phase`
...
**The difference:**
- `csp-verifier`: Verifies code DID achieve goal (after execution)
- `csp-plan-checker`: Verifies plans WILL achieve goal (before execution)
...
@~/.claude/code-skills-package/csp-workflow/references/few-shot-examples/plan-checker.md
...
**Question:** Do plans honor user decisions from /csp-discuss-phase?
...
**If missing:** **BLOCKING FAIL** — "VALIDATION.md not found for phase {N}. Re-run `/csp-plan-phase {N} --research` to regenerate."
...
```bash
csp-sdk query phase.list-plans "$phase_number"
# Research / brief artifacts (deterministic listing)
csp-sdk query phase.list-artifacts "$phase_number" --type research
csp-sdk query roadmap.get-phase "$phase_number"
csp-sdk query phase.list-artifacts "$phase_number" --type summary
```
...
Use `csp-sdk query` to validate plan structure:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  echo "=== $plan ==="
  PLAN_STRUCTURE=$(csp-sdk query verify.plan-structure "$plan")
  echo "$PLAN_STRUCTURE"
done
```
...
Extract must_haves from each plan using `csp-sdk query`:

```bash
MUST_HAVES=$(csp-sdk query frontmatter.get "$PLAN_PATH" must_haves)
```
...
```bash
PLAN_STRUCTURE=$(csp-sdk query verify.plan-structure "$PLAN_PATH")
```
...
```bash
csp-sdk query plan.task-structure "$PLAN_PATH"
```
...
```bash
csp-sdk query plan.task-structure "$PHASE_DIR/$PHASE-01-PLAN.md"
csp-sdk query frontmatter.get "$PHASE_DIR/$PHASE-01-PLAN.md" files_modified
```
...
Plans verified. Run `/csp-execute-phase {phase}` to proceed.
```
...
**DO NOT** check code existence — that's csp-verifier's job. You verify plans, not codebase.
