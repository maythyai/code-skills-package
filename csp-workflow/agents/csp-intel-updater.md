...
- **Cross-platform.** Use Glob, Read, and Grep tools for filesystem work — never raw OS commands (`ls`, `find`, `cat`); they fail on Windows. CLI invocations go through `csp-tools intel <subcommand>`, which routes through the Shell Command Projection Module that formats per-OS automatically.
...
### From `/csp-map-codebase --query` Command

- **Spawned by:** `/csp-map-codebase --query` command
...
The /csp-map-codebase --query command has already confirmed that intel.enabled is true before spawning this agent. Proceed directly to Step 1.
</upstream_input>

## Project Scope

<!-- Layout detection: only meaningful when analysing the CSP framework's own repo (#3290). -->

**Runtime layout detection (CSP framework repo only):** If `package.json` `"name"` equals `"code-skills-package-cc"`, this project IS the CSP framework. In that case, detect the runtime root to choose canonical paths:

```bash
# Only run layout detection when analysing the CSP framework repo itself.
if [[ "$(jq -r '.name // ""' package.json 2>/dev/null)" == "code-skills-package-cc" ]]; then
  ls -d .kilo 2>/dev/null && echo "kilo" || (ls -d .claude/code-skills-package 2>/dev/null && echo "claude") || echo "unknown"
fi
```
...
| Command files | `commands/csp/*.md` | `.kilo/command/*.md` |
| CLI tooling | `code-skills-package/bin/` | `.kilo/code-skills-package/bin/` |
| Workflow files | `code-skills-package/workflows/` | `.kilo/code-skills-package/workflows/` |
| Reference docs | `code-skills-package/references/` | `.kilo/code-skills-package/references/` |
...
**exports constraint:** Array of ACTUAL exported symbol names extracted from `module.exports` or `export` statements. MUST be real identifiers (e.g., `"configLoad"`, `"stateUpdate"`), NOT descriptions (e.g., `"config operations"`). If an export string contains a space, it is wrong -- extract the actual symbol name instead. Use `csp-tools intel extract-exports <file>` to get accurate exports.
