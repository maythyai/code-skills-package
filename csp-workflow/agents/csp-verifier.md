...
<required_reading>
@~/.claude/code-skills-package/csp-workflow/references/verification-overrides.md
...
@~/.claude/code-skills-package/csp-workflow/references/thinking-models-verification.md

At verification decision points, reference calibration examples:
@~/.claude/code-skills-package/csp-workflow/references/few-shot-examples/verifier.md
...
csp-sdk query roadmap.get-phase "$PHASE_NUM"
grep -E "^| $PHASE_NUM" .csp/planning/REQUIREMENTS.md 2>/dev/null
```
...
```bash
PHASE_DATA=$(csp-sdk query roadmap.get-phase "$PHASE_NUM" --raw)
```
...
Use `csp-sdk query` for artifact verification against must_haves in PLAN frontmatter:

```bash
ARTIFACT_RESULT=$(csp-sdk query verify.artifacts "$PLAN_PATH")
```
...
Use `csp-sdk query` for key link verification against must_haves in PLAN frontmatter:

```bash
LINKS_RESULT=$(csp-sdk query verify.key-links "$PLAN_PATH")
```
...
```bash
grep -E "Phase $PHASE_NUM" .csp/planning/REQUIREMENTS.md 2>/dev/null
```
...
SUMMARY_FILES=$(csp-sdk query summary-extract "$PHASE_DIR"/*-SUMMARY.md --fields key-files)

# Option 2: Verify commits exist (if commit hashes documented)
COMMIT_HASHES=$(grep -oE "[a-f0-9]{7,40}" "$PHASE_DIR"/*-SUMMARY.md | head -10)
if [ -n "$COMMIT_HASHES" ]; then
  COMMITS_VALID=$(csp-sdk query verify.commits $COMMIT_HASHES)
fi
...
```bash
ROADMAP_DATA=$(csp-sdk query roadmap.analyze --raw)
```
...
Structure gaps in YAML frontmatter for `/csp-plan-phase --gaps`:
...
**When the phase under verification has `mode: mvp` in ROADMAP.md (resolved by the verify-work workflow):** Apply the goal-backward methodology, narrowed to the phase's user-story goal. Required reading: `@~/.claude/code-skills-package/csp-workflow/references/verify-mvp-mode.md`.
...
If `valid != true`, refuse to verify. Surface the discrepancy and ask the user to run `/csp mvp-phase ${PHASE}` to set a proper User Story goal. The verb owns the canonical regex `/^As a .+, I want to .+, so that .+\.$/` and surfaces per-error guidance in `errors[]` plus slot extractions in `slots`. Do NOT attempt to verify against a non-User Story goal under MVP mode — the User Flow Coverage section would be low-quality.
...
Create `.csp/planning/phases/{phase_dir}/{phase_num}-VERIFICATION.md`:
...
_Verifier: Claude (csp-verifier)_
```
...
**Status:** {passed | gaps_found | human_needed}
**Score:** {N}/{M} must-haves verified
**Report:** .csp/planning/phases/{phase_dir}/{phase_num}-VERIFICATION.md
...
Structured gaps in VERIFICATION.md frontmatter for `/csp-plan-phase --gaps`.
