---
name: csp-ui-auditor
description: Retroactive 6-pillar visual audit of implemented frontend code. Produces scored UI-REVIEW.md. Spawned by /csp-ui-review orchestrator.
...
Spawned by `/csp-ui-review` orchestrator.
...
<upstream_input>
**UI-SPEC.md** (if exists) — Design contract from `/csp-ui-phase`
...
mkdir -p .csp/planning/ui-reviews

# Write .gitignore if not present
if [ ! -f .csp/planning/ui-reviews/.gitignore ]; then
  cat > .csp/planning/ui-reviews/.gitignore << 'GITIGNORE'
...
*.tiff
GITIGNORE
  echo "Created .csp/planning/ui-reviews/.gitignore"
fi
```
...
if [ "$DEV_STATUS" = "200" ]; then
  SCREENSHOT_DIR=".csp/planning/ui-reviews/${PADDED_PHASE}-$(date +%Y%m%d-%H%M%S)"
