...
description: |
  ⚠️ AI 编程下绝对工时/人天估算无意义（产出节奏与日历无关，可能几小时完成一个版本）。本 skill 仅作**复杂度相对参考**与**依赖/并行度分析**，不产绝对时间线/排期/人天；返回时须声明"AI 产出节奏不可估时，以下仅复杂度相对参考"。
  复杂度相对估算引擎。从技术方案和任务拆解出发，分析复杂度、不确定性、并行度（非绝对工时）。
  方法：类比估算、三点估算(PERT)、COCOMO II（仅作复杂度相对参考，不产绝对人天）。
  输出 WBS 复杂度汇总、依赖/并行计划、相对里程碑序列（按版本号，不按日期）。
  当任务拆解完成后需要复杂度相对参考、或用户需要"估时""排期""资源计划"时使用。
...
anti_rationalizations:
  "估时总是错的，没必要估": "绝对工时估算在 AI 编程下确实无意义（产出节奏与日历无关，可能几小时完成一个版本），但复杂度相对参考与不确定性/并行度分析仍有价值——本 skill 输出复杂度相对参考，非绝对工时/排期。"
...
> ⚠️ **AI 编程下绝对工时无意义**：AI 产出节奏与日历无关（可能几小时完成一个版本），按人天/小时/日期估时或定版本毫无依据。本 skill 的 PERT/COCOMO 等方法**仅作复杂度相对参考**与不确定性/并行度分析，**不产绝对工时/排期/交付日期**。返回结果时须显式声明"以下仅复杂度相对参考，非可承诺的工期"。版本规划与发布按 SemVer 版本号（X+1 breaking/架构重组 / Y+1 功能扩展可几百上千 / Z+1 fix 可多个攒一批），不按时间。

工作量与资源估算引擎 — 从任务拆解出发，分析复杂度、依赖与并行度（非绝对工时）。
