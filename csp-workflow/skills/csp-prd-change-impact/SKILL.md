...
4. 评估每个层级的影响：
   ...
   c. Task 层: 新增/修改/删除 Task（计 Task 数，不估工时）
   d. Code 层: 新增/修改/删除文件
   e. 测试层: 新增/修改测试用例
   f. 文档层: 更新相关文档
   g. 风险层: 重新评估风险
5. 量化影响（范围: Task/Feature/文件数、风险、受影响依赖链与 Wave）——**不估工时**
6. 输出影响分析报告
7. 给出变更建议
```
...
> AI 编程下工时方差极大（可能几分钟到几小时完成同一 Task），估算无意义且易腐烂。
> 量化影响用**可数范围**（Task/Feature/文件/测试用例数）+ 风险 + 受影响依赖链，不用工时。
...
| 指标 | 变更前 | 变更后 | 影响 |
|------|--------|--------|------|
| 关键路径（依赖链节点数） | 12 | 14 | +2 节点 |
| 受影响 Wave | Wave 2-3 | Wave 2-3 | 需重排 2 个 Wave |
| 风险等级 | 中 | 中 | 新增 2 个风险 |
```

## 变更方案对比

对同一变更，可能有多种实现方式，提供对比分析（按**范围/风险/价值**，不按工时/工期）：
...
| csp-effort-estimation | （可选，按需）当前范围 → 工时/资源估算 |
