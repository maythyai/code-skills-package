...
type: prompt
name: csp-forensics
description: Post-mortem investigation for failed CSP workflows — diagnoses what went wrong.
...
<objective>
Investigate what went wrong during a CSP workflow execution. Analyzes git history, `.csp/planning/` artifacts, and file system state to detect anomalies and generate a structured diagnostic report.

Purpose: Diagnose failed or stuck workflows so the user can understand root cause and take corrective action.
Output: Forensic report saved to `.csp/planning/forensics/`, presented inline, with optional issue creation.
</objective>

<execution_context>
@~/.claude/code-skills-package/csp-workflow/workflows/forensics.md
</execution_context>
...
- `.csp/planning/STATE.md` (current position, session history)
- `.csp/planning/ROADMAP.md` (phase scope and progress)
- `.csp/planning/phases/*/` (PLAN.md, SUMMARY.md, VERIFICATION.md, CONTEXT.md)
- `.csp/planning/reports/SESSION_REPORT.md` (last session outcomes)
...
- Structured forensic report written to `.csp/planning/forensics/report-{timestamp}.md`
