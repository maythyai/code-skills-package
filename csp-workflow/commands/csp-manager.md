...
<execution_context>
@~/.claude/code-skills-package/csp-workflow/workflows/manager.md
...
Project context, phase list, dependencies, and recommendations are resolved inside the workflow using `csp-sdk query init.manager`. No upfront context loading needed.
</context>

<process>
If `--analyze-deps` is in $ARGUMENTS:
Read and execute `~/.claude/code-skills-package/csp-workflow/workflows/analyze-dependencies.md` end-to-end.
