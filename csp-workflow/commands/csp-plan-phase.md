...
**Research-only mode (`--research-phase <N>`):** Spawn `csp-phase-researcher` for phase `N`, write `RESEARCH.md`, then exit before the planner runs. Useful for cross-phase research, doc review before committing to a planning approach, and correction-without-replanning loops where iterating on research alone is dramatically cheaper than re-spawning the planner. Replaces the deleted research-phase command (#3042).
...
**Orchestrator role:** Parse arguments, validate phase, research domain (unless skipped), spawn csp-planner, verify with csp-plan-checker, iterate until pass or max iterations, present results.
</objective>

<execution_context>
@~/.claude/code-skills-package/csp-workflow/workflows/plan-phase.md
...
- `--reviews` — Replan incorporating cross-AI review feedback from REVIEWS.md (produced by `/csp-review`)
