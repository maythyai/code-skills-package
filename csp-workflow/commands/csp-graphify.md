...
**CJS-only (graphify):** `graphify` subcommands are not registered on `csp-sdk query`. Use `node $HOME/.claude/code-skills-package/bin/csp-tools.cjs graphify …` as documented in this command and in `docs/CLI-TOOLS.md`. Other tooling may still use `csp-sdk query` where a handler exists.
...
Knowledge graph is disabled. To activate:
  ...
  node $HOME/.claude/code-skills-package/bin/csp-tools.cjs config-set graphify.enabled true

Then run /csp-graphify build to create the initial graph.
```
...
```bash
node $HOME/.claude/code-skills-package/bin/csp-tools.cjs graphify query <term>
```

Parse the JSON output and display results:
- If the output contains `"disabled": true`, display the disabled message from Step 1 and **STOP**
- If the output contains `"error"` field, display the error message and **STOP**
- If no nodes found, display: `No graph matches for '<term>'. Try /csp-graphify build to create or rebuild the graph.`
...
```bash
node $HOME/.claude/code-skills-package/bin/csp-tools.cjs graphify diff
```
...
```bash
node "$HOME/.claude/code-skills-package/bin/csp-tools.cjs" graphify build
```
...
**MVP-mode rendering.** When a phase has `**Mode:** mvp` in ROADMAP.md (resolved via `csp-sdk query roadmap.get-phase --pick mode`), render its graph node with two distinct visual signals:
...
5. DO NOT use `csp-tools config get-value` for the config gate -- it exits on missing keys.
