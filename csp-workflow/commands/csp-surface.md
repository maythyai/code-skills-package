...
<objective>
Manage the runtime skill surface without reinstall. Reads/writes `~/.claude/.csp-surface.json`
(sibling to `~/.claude/.csp-profile`) and re-stages the active skills directory in place.
Skill dirs live at `~/.claude/skills/csp-*/`.
...
| `reset` | Delete `.csp-surface.json`, return to install-time profile |
| *(none)* | Treat as `list` |
...
`code-skills-package/bin/lib/surface.cjs`. Display:
...
```
Base profile:   standard  (from .csp-surface.json)
Install profile: standard  (from .csp-profile)
```
...
1. Check if `.csp-surface.json` exists.
2. Delete it.
...
`~/.claude/skills/csp-*/` because the `claude global` layout has `destSubpath =
...
Surface state is stored at `${RUNTIME_CONFIG_DIR}/.csp-surface.json`
(i.e. `~/.claude/.csp-surface.json`).
...
- Missing `surface.cjs` → prompt: "Run `npm i -g code-skills-package` to reinstall CSP."

<execution_context>
Surface state file: `~/.claude/.csp-surface.json`
Install profile marker: `~/.claude/.csp-profile`
Skill dirs: `~/.claude/skills/csp-*/`
Engine module: `~/.claude/code-skills-package/csp-workflow/bin/lib/surface.cjs`
Cluster definitions: `~/.claude/code-skills-package/csp-workflow/bin/lib/clusters.cjs`
</execution_context>
