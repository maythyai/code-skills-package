---
name: csp-config
description: Configure CSP settings — workflow toggles, advanced knobs, integrations, and model profile
...
<objective>
Configure CSP settings interactively with a single consolidated command.
...
| --profile &lt;name&gt; | Switch model profile without interactive prompt | csp-sdk config-set-model-profile |

</routing>

<execution_context>
@~/.claude/code-skills-package/csp-workflow/workflows/settings.md
@~/.claude/code-skills-package/csp-workflow/workflows/settings-advanced.md
@~/.claude/code-skills-package/csp-workflow/workflows/settings-integrations.md
</execution_context>
...
- If it starts with `--profile`: extract the profile name (remainder after `--profile`), then:
  1. **Pre-flight check (#2439):** verify `csp-sdk` is on PATH via `command -v csp-sdk`.
     If absent, emit the install hint `Install CSP via 'npm i -g code-skills-package'` and stop —
     do NOT invoke `csp-sdk` directly (avoids the opaque `command not found: csp-sdk` failure).
  2. Run: `csp-sdk query config-set-model-profile <profile-name> --raw` and display the output verbatim.
