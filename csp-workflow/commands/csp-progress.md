---
name: csp-progress
description: Check progress, advance workflow, or dispatch freeform intent — the unified CSP situational command
...
- **--do "task description"**: Analyze freeform natural language and dispatch to the most appropriate CSP command. Never does the work itself — matches intent, confirms, hands off.
- **--forensic**: Append a 6-check integrity audit after the standard progress report.
</objective>

<flags>
- **--next**: Detect current project state and automatically invoke the next logical CSP workflow step. Scans all prior phases for incomplete work before routing. `--next --force` bypasses safety gates.
- **--do "..."**: Smart dispatcher — match freeform intent to the best CSP command using routing rules, confirm the match, then hand off.
...
<execution_context>
@~/.claude/code-skills-package/csp-workflow/workflows/progress.md
@~/.claude/code-skills-package/csp-workflow/workflows/next.md
@~/.claude/code-skills-package/csp-workflow/workflows/do.md
