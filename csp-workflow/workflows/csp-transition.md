...
There is no `/csp-transition` command. This workflow is invoked automatically by
`execute-phase` during auto-advance, or inline by the orchestrator after phase
verification. Users should never be told to run `/csp-transition`.

**Valid user commands for phase progression:**
- `/csp-discuss-phase {N}` — discuss a phase before planning
- `/csp-plan-phase {N}` — plan a phase
- `/csp-execute-phase {N}` — execute a phase
- `/csp-progress` — see roadmap progress
...
These will carry forward as debt. Review: `/csp-audit-uat`
```
...
**Delegate ROADMAP.md and STATE.md updates to `csp-sdk query phase.complete`:**

```bash
TRANSITION=$(csp-sdk query phase.complete "${current_phase}")
```
...
```text
@~/.claude/code-skills-package/csp-workflow/workflows/graduation.md
```
...
**Note:** Basic position updates (Current Phase, Status, Current Plan, Last Activity) were already handled by `csp-sdk query phase.complete` in the update_roadmap_and_state step.

Verify the updates are correct by reading STATE.md. If the progress bar needs updating, use:

```bash
PROGRESS=$(csp-sdk query progress.bar --raw)
```
...
**Use the transition result from `csp-sdk query phase.complete`:**
...
**Skip this check if NOT in workstream mode** (i.e., `CSP_WORKSTREAM` is not set / flat mode).
In flat mode, go directly to **Route B**.

```bash
# Only check if we're in workstream mode
if [ -n "$CSP_WORKSTREAM" ]; then
  WS_LIST=$(csp-sdk query workstream.list --raw)
fi
```
...
Filter out the current workstream (`$CSP_WORKSTREAM`) and any workstreams with
...
Exit skill and invoke SlashCommand("/csp-plan-phase [X+1] --auto ${CSP_WS}")
...
Exit skill and invoke SlashCommand("/csp-discuss-phase [X+1] --auto ${CSP_WS}")
...
`/csp-discuss-phase [X+1] ${CSP_WS}` — gather context and clarify approach

---

**Also available:**
- `/csp-plan-phase [X+1] ${CSP_WS}` — skip discussion, plan directly
- `/csp-plan-phase --research-phase [X+1] ${CSP_WS}` — investigate unknowns
...
`/csp-plan-phase [X+1] ${CSP_WS}`

---

**Also available:**
- `/csp-discuss-phase [X+1] ${CSP_WS}` — revisit context
- `/csp-plan-phase --research-phase [X+1] ${CSP_WS}` — investigate unknowns
...
```bash
csp-sdk query config-set workflow._auto_chain_active false
```
...
`/csp-workstreams complete {current_ws_name} ${CSP_WS}`

See overall milestone progress:

`/csp-workstreams progress ${CSP_WS}`
...
Do NOT suggest `/csp-complete-milestone` or `/csp-new-milestone`.
...
Exit skill and invoke SlashCommand("/csp-complete-milestone {version} ${CSP_WS}")
...
`/csp-complete-milestone {version} ${CSP_WS}`
