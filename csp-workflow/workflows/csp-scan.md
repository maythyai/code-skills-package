...
Valid CCSP subagent types (use exact names — do not fall back to 'general-purpose'):
...
```bash
INIT=$(csp-sdk query init.map-codebase 2>/dev/null || echo "{}")
...
Spawn a single `csp-codebase-mapper` agent with the selected focus area:
...
Use `/csp-map-codebase` for a comprehensive 4-area parallel scan.
```
