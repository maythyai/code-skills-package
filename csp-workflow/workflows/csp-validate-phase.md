...
AGENT_SKILLS_AUDITOR=$(csp-sdk query agent-skills csp-nyquist-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(csp-sdk query resolve-model csp-nyquist-auditor --raw)
NYQUIST_CFG=$(csp-sdk query config-get workflow.nyquist_validation --raw)
```

If `NYQUIST_CFG` is `false`: exit with "Nyquist validation is disabled. Enable via /csp-settings."

Display banner: `CSP > VALIDATE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /csp-execute-phase {N} ${CSP_WS} first."
...
Agent(
  ...
  subagent_type="csp-nyquist-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/code-skills-package/csp-workflow/templates/VALIDATION.md`
...
csp-sdk query commit "docs(phase-${PHASE}): add/update validation strategy"
```
...
**Compliant:**
```
CSP > PHASE {N} IS NYQUIST-COMPLIANT
All requirements have automated verification.
▶ Next: /csp-audit-milestone ${CSP_WS}
```

**Partial:**
```
CSP > PHASE {N} VALIDATED (PARTIAL)
{M} automated, {K} manual-only.
▶ Retry: /csp-validate-phase {N} ${CSP_WS}
```
