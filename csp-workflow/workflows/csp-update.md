<purpose>
Check for CSP updates via npm, display changelog for versions between installed and latest, obtain user confirmation, and execute clean installation with cache clearing.
</purpose>
...
Detect whether CSP is installed locally or globally by checking both locations and validating install integrity.

First, derive `PREFERRED_CONFIG_DIR` and `PREFERRED_RUNTIME` from the invoking prompt's `execution_context` path:
- If the path contains `/code-skills-package/workflows/update.md`, strip that suffix and store the remainder as `PREFERRED_CONFIG_DIR`
...
Use `PREFERRED_RUNTIME` as the first runtime checked so `/csp-update` targets the runtime that invoked it.
...
# runtime directories.
if [ -n "$PREFERRED_CONFIG_DIR" ] && { [ -f "$PREFERRED_CONFIG_DIR/code-skills-package/VERSION" ] || [ -f "$PREFERRED_CONFIG_DIR/code-skills-package/workflows/update.md" ]; }; then
  ...
  if [ -f "$PREFERRED_CONFIG_DIR/code-skills-package/VERSION" ] && grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' "$PREFERRED_CONFIG_DIR/code-skills-package/VERSION"; then
    INSTALLED_VERSION="$(cat "$PREFERRED_CONFIG_DIR/code-skills-package/VERSION")"
  else
  ...
  # CSP_DIR or downstream check_latest_version misreads the install as
  # UNKNOWN. PREFERRED_CONFIG_DIR is the resolved config dir we just
  # validated above (line 95-96); it is the right CSP_DIR value for
  # this fast path.
...
for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
  runtime="${entry%%:*}"
  dir="${entry#*:}"
  if [ -f "./$dir/code-skills-package/VERSION" ] || [ -f "./$dir/code-skills-package/workflows/update.md" ]; then
    LOCAL_RUNTIME="$runtime"
    LOCAL_VERSION_FILE="./$dir/code-skills-package/VERSION"
    LOCAL_MARKER_FILE="./$dir/code-skills-package/workflows/update.md"
...
for entry in "${ORDERED_ENV_RUNTIME_DIRS[@]}"; do
  ...
  if [ -f "$dir/code-skills-package/VERSION" ] || [ -f "$dir/code-skills-package/workflows/update.md" ]; then
    ...
    GLOBAL_VERSION_FILE="$dir/code-skills-package/VERSION"
    GLOBAL_MARKER_FILE="$dir/code-skills-package/workflows/update.md"
...
if [ -z "$GLOBAL_RUNTIME" ]; then
  for entry in "${ORDERED_RUNTIME_DIRS[@]}"; do
    runtime="${entry%%:*}"
    dir="${entry#*:}"
    if [ -f "$HOME/$dir/code-skills-package/VERSION" ] || [ -f "$HOME/$dir/code-skills-package/workflows/update.md" ]; then
      GLOBAL_RUNTIME="$runtime"
      GLOBAL_VERSION_FILE="$HOME/$dir/code-skills-package/VERSION"
      GLOBAL_MARKER_FILE="$HOME/$dir/code-skills-package/workflows/update.md"
...
- Line 4 = resolved CSP config dir (e.g. `/Users/me/.claude`, `/Users/me/.gemini`); empty if scope is `UNKNOWN`. Capture this as `CSP_DIR` and pass it to subsequent steps so they don't have to re-derive the runtime path.
...
The `CSP_DIR` value emitted by `get_installed_version` (line 4) resolves to the runtime-specific config dir (`~/.claude/`, `~/.gemini/`, `~/.codex/`, etc.), so the script invocation works for every runtime — not just Claude. If `CSP_DIR` is empty (scope `UNKNOWN`), skip this step and go directly to install.

`LATEST_RESULT` is a JSON document with the documented shape `{ ok: bool, version: string, reason: string, detail?: string }`. Parse via `jq` ONLY when the script actually ran. When `CSP_DIR` is empty (scope `UNKNOWN`), skip the check entirely and seed the parsed fields with their no-op values so downstream logic does not mistake an unset `LATEST_RESULT` for a failed network check (#2993 CR feedback):
...
if [ -z "$CSP_DIR" ]; then
  ...
  LATEST_RESULT="$(node "$CSP_DIR/code-skills-package/bin/check-latest-version.cjs" --json 2>/dev/null)"
  LATEST_STATUS=$?
...
⚠️  **Note:** The installer performs a clean install of CSP folders:
- `commands/csp/` will be wiped and replaced
- `code-skills-package/` will be wiped and replaced
- `agents/csp-*` files will be replaced
...
- Custom commands not in `commands/csp/` ✓
- Custom agents not prefixed with `csp-` ✓
- Custom hooks ✓
...
CSP-managed directories. These are files that exist on disk but are NOT listed
in `csp-file-manifest.json` — i.e., files the user added themselves that the
...
Back up each custom file to `$RUNTIME_DIR/csp-user-files-backup/` before the
installer wipes the directories:

```bash
BACKUP_DIR="$RUNTIME_DIR/csp-user-files-backup"
mkdir -p "$BACKUP_DIR"
...
```
⚠️  Found N custom file(s) inside CSP-managed directories.
    These have been backed up to csp-user-files-backup/ before the update.
...
for dir in "${CACHE_DIRS[@]}"; do
  if [ -n "$dir" ]; then
    rm -f "$dir/cache/csp-update-check.json"
  fi
done

for dir in .claude .config/opencode .opencode .gemini/antigravity .gemini .config/kilo .kilo .codex; do
  rm -f "./$dir/cache/csp-update-check.json"
  rm -f "$HOME/$dir/cache/csp-update-check.json"
done

# Clear the shared tool-agnostic cache written by csp-check-update.js hook (#2784).
# The hook uses ~/.cache/csp/csp-update-check.json regardless of runtime; clear it
# so the statusline stops showing the stale "⬆ /csp-update" indicator after update.
rm -f "$HOME/.cache/csp/csp-update-check.json"
```

The SessionStart hook (`csp-check-update.js`) writes to the detected runtime's cache directory, so preferred/env-derived paths and default paths must all be cleared to prevent stale update indicators.
</step>
...
Run `/csp-update --reapply` to merge your modifications into the new version.
```
