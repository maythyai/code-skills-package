...
1. Check `.csp/planning/STATE.md` for current milestone version
2. Check `.csp/planning/milestones/` for the latest archived version
3. If neither found, check if `.csp/planning/ROADMAP.md` exists (project may be mid-milestone)
4. If nothing found: error "No milestone found. Run /csp-new-project or /csp-new-milestone first."
...
**Archived milestone** (`.csp/planning/milestones/v{VERSION}-ROADMAP.md` exists):
```
ROADMAP_PATH=".csp/planning/milestones/v${VERSION}-ROADMAP.md"
REQUIREMENTS_PATH=".csp/planning/milestones/v${VERSION}-REQUIREMENTS.md"
AUDIT_PATH=".csp/planning/milestones/v${VERSION}-MILESTONE-AUDIT.md"
```

**Current/in-progress milestone** (no archive yet):
```
ROADMAP_PATH=".csp/planning/ROADMAP.md"
REQUIREMENTS_PATH=".csp/planning/REQUIREMENTS.md"
AUDIT_PATH=".csp/planning/v${VERSION}-MILESTONE-AUDIT.md"
```

Note: The audit file moves to `.csp/planning/milestones/` on archive (per `complete-milestone` workflow). Check both locations as a fallback.

**Always available:**
```
PROJECT_PATH=".csp/planning/PROJECT.md"
RETRO_PATH=".csp/planning/RETROCSPECTIVE.md"
STATE_PATH=".csp/planning/STATE.md"
```
...
Find the earliest `.csp/planning/phases/` commit:
```bash
git log --oneline --diff-filter=A -- ".csp/planning/phases/" | tail -1
```
...
Write to `.csp/planning/reports/MILESTONE_SUMMARY-v${VERSION}.md`:
...
{From RETROCSPECTIVE.md: lessons learned, what to improve}
...
**Overwrite guard:** If `.csp/planning/reports/MILESTONE_SUMMARY-v${VERSION}.md` already exists, ask the user:
...
```bash
mkdir -p .csp/planning/reports
```

Write the summary, then commit:
```bash
csp-sdk query commit "docs(v${VERSION}): generate milestone summary for onboarding" --files \
  ".csp/planning/reports/MILESTONE_SUMMARY-v${VERSION}.md"
```
...
> "Summary written to `.csp/planning/reports/MILESTONE_SUMMARY-v{VERSION}.md`.
>
...
If the user is done:
- Suggest next steps: `/csp-new-milestone`, `/csp-progress`, or sharing the summary with the team
