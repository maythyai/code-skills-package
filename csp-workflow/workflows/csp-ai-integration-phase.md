<purpose>
Generate an AI design contract (AI-CSPEC.md) for phases that involve building AI systems. Orchestrates csp-framework-selector → csp-ai-researcher → csp-domain-researcher → csp-eval-planner with a validation gate. Inserts between discuss-phase and plan-phase in the CSP lifecycle.

AI-CSPEC.md locks four things before the planner creates tasks:
...
<required_reading>
@~/.claude/code-skills-package/csp-workflow/references/ai-frameworks.md
...
```bash
SELECTOR_MODEL=$(csp-sdk query resolve-model csp-framework-selector 2>/dev/null | jq -r '.model' 2>/dev/null || true)
RESEARCHER_MODEL=$(csp-sdk query resolve-model csp-ai-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
DOMAIN_MODEL=$(csp-sdk query resolve-model csp-domain-researcher 2>/dev/null | jq -r '.model' 2>/dev/null || true)
PLANNER_MODEL=$(csp-sdk query resolve-model csp-eval-planner 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Check config:
```bash
AI_PHASE_ENABLED=$(csp-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**If `AI_PHASE_ENABLED` is `false`:**
```
AI phase is disabled in config. Enable via /csp-settings.
```
Exit workflow.
...
Recommended: run /csp-discuss-phase {N} first to capture framework preferences.
Continuing without user decisions — framework selector will ask all questions.
```
Continue (non-blocking).

## 4. Check Existing AI-CSPEC
...
- header: "Existing AI-CSPEC"
- question: "AI-CSPEC.md already exists for Phase {N}. What would you like to do?"
- options:
  - "Update — re-run with existing as baseline"
  - "View — display current AI-CSPEC and exit"
  - "Skip — keep current AI-CSPEC and exit"

If "View": display file contents, exit.
If "Skip": exit.
If "Update": continue to step 5.

## 5. Spawn csp-framework-selector

Display:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► AI DESIGN CONTRACT — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
**If selector fails or returns empty:** Exit with error — "Framework selection failed. Re-run /csp-ai-integration-phase {N} or answer the framework question in /csp-discuss-phase {N} first."

## 6. Initialize AI-CSPEC.md

Copy template:
```bash
cp "$HOME/.claude/code-skills-package/templates/AI-CSPEC.md" "${PHASE_DIR}/${PADDED_PHASE}-AI-CSPEC.md"
```
...
## 7. Spawn csp-ai-researcher

> **Ordering note (prevents tool-level last-writer-wins race):** Steps 7 and 8 write disjoint sections of AI-CSPEC.md but MUST run sequentially — wait for Step 7 to complete before spawning Step 8. Both agents use the `Edit` tool exclusively (never `Write`) when modifying AI-CSPEC.md. A `Write` on a shared file replaces the entire file, silently overwriting the other agent's work; `Edit` targets only the relevant lines. See #3096 for a confirmed 40%-incidence race on parallel dispatch.
...
Write Sections 5, 6, and 7 of AI-CSPEC.md
AI-CSPEC.md now contains domain context (Section 1b) — use it as your rubric starting point.
</objective>
...
## 10. Validate AI-CSPEC Completeness

Read the completed AI-CSPEC.md. Check that:
...
```bash
git add "${AI_CSPEC_FILE}"
git commit -m "docs({phase_slug}): generate AI-CSPEC.md — {primary_framework} + domain context + eval strategy"
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► AI-CSPEC COMPLETE — PHASE {N}: {name}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Next step:
  /csp-plan-phase {N}   — planner will consume AI-CSPEC.md
```
...
- [ ] AI-CSPEC.md created from template
...
- [ ] AI-CSPEC.md validated (Sections 1b, 2, 3, 4b, 5, 6 all non-empty)
