...
<required_reading>
@~/.claude/code-skills-package/csp-workflow/references/verification-patterns.md
@~/.claude/code-skills-package/csp-workflow/references/artifact-verification.md
@~/.claude/code-skills-package/csp-workflow/templates/verification-report.md
</required_reading>
...
Use `csp-sdk query` verify handlers (or legacy csp-tools) to extract must_haves from each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(csp-sdk query frontmatter.get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(csp-sdk query roadmap.get-phase "${phase_number}" --raw)
```
...
Use `csp-sdk query verify.artifacts` (or legacy csp-tools) for artifact verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(csp-sdk query verify.artifacts "$plan")
...
Use `csp-sdk query verify.key-links` (or legacy csp-tools) for key link verification against must_haves in each PLAN:

```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(csp-sdk query verify.key-links "$plan")
...
```bash
grep -E "Phase ${PHASE_NUM}" .csp/planning/REQUIREMENTS.md 2>/dev/null || true
```
...
if [ "$GATE_CFG" != "false" ]; then
  ...
  DECISION_RESULT=$(csp-sdk query check.decision-coverage-verify "${PHASE_DIR}" "${CONTEXT_PATH}")
fi
```
...
TEST_CMD=$(csp-sdk query config-get workflow.test_command --default "" 2>/dev/null || true)
if [ -z "$TEST_CMD" ]; then
...
- UNKNOWN: No provenance information — treat as SUCSPECT
...
See ~/.claude/code-skills-package/csp-workflow/templates/verification-report.md for complete template.
</step>
