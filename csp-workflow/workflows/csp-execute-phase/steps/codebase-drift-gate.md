...
```bash
DRIFT=$(csp-tools verify codebase-drift 2>/dev/null || echo '{"skipped":true,"reason":"sdk-failed"}')
```
...
Run /csp-map-codebase --paths {affected_paths} to refresh planning context.
```
...
from step `init_context` is for `csp-executor`, not the mapper):

```bash
AGENT_SKILLS_MAPPER=$(csp-sdk query agent-skills csp-codebase-mapper)
```

Then spawn `csp-codebase-mapper` agents with the `--paths` hint:
