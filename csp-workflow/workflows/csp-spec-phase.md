...
Produces a CSPEC.md with falsifiable requirements that discuss-phase treats as locked decisions.
...
**Gate:** ambiguity ≤ 0.20 AND all dimensions ≥ their minimums → ready to write CSPEC.md.
...
```bash
INIT=$(node "$HOME/.claude/code-skills-package/bin/csp-tools.cjs" init phase-op "${PHASE}")
...
Use /csp-progress to see available phases.
```
Exit.

**Check for existing CSPEC.md:**
```bash
ls ${phase_dir}/*-CSPEC.md 2>/dev/null | grep -v AI-CSPEC | head -1 || true
```

If CSPEC.md already exists:

**If `--auto`:** Auto-select "Update it". Log: `[auto] CSPEC.md exists — updating.`

**Otherwise:** Use AskUserQuestion:
- header: "Spec"
- question: "Phase [X] already has a CSPEC.md. What do you want to do?"
- options:
  - "Update it" — Revise and re-score
  - "View it" — Show current spec
  - "Skip" — Exit (use existing spec as-is)

If "View": Display CSPEC.md, then offer Update/Skip.
If "Skip": Exit with message: "Existing CSPEC.md unchanged. Run /csp-discuss-phase [X] to continue."
If "Update": Load existing CSPEC.md, continue to Step 3.
...
**If `--auto` and initial ambiguity already ≤ 0.20 with all minimums met:** Skip interview — derive CSPEC.md directly from roadmap + requirements. Log: `[auto] Phase requirements are already sufficiently clear — generating CSPEC.md from existing context.` Jump to Step 6.
...
- question: "Ambiguity is [score] — requirements are clear enough to write CSPEC.md. Proceed?"
- options:
  - "Yes — write CSPEC.md" → Jump to Step 6
  - "One more round" → Continue interview
  - "Done talking — write it" → Jump to Step 6

**If max rounds reached (6) and gate not passed:**

**If `--auto`:** Write CSPEC.md anyway — flag unresolved dimensions. Log: `[auto] Max rounds reached. Writing CSPEC.md with [N] dimensions below minimum. Planner will need to treat these as assumptions.`

**Otherwise:** AskUserQuestion:
- header: "Max Rounds"
- question: "After 6 rounds, ambiguity is [score]. [List dimensions still below minimum.] What would you like to do?"
- options:
  - "Write CSPEC.md anyway — flag gaps" → Write CSPEC.md, mark unresolved dimensions in Ambiguity Report
...
## Step 6: Generate CSPEC.md

Use the CSPEC.md template from @~/.claude/code-skills-package/csp-workflow/templates/spec.md.
...
**Count requirements.** The display in discuss-phase reads: "Found CSPEC.md — {N} requirements locked."
...
Write to: `{phase_dir}/{padded_phase}-CSPEC.md`

## Step 7: Commit

```bash
git add "${phase_dir}/${padded_phase}-CSPEC.md"
git commit -m "spec(phase-${phase_number}): add CSPEC.md for ${phase_name} — ${requirement_count} requirements (#2213)"
```

If `commit_docs` is false: Skip commit. Note that CSPEC.md was written but not committed.
...
```
CSPEC.md written — {N} requirements locked.

  Phase {X}: {name}
  Ambiguity: {final_score} (gate: ≤ 0.20)

Next: /csp-discuss-phase {X}
  discuss-phase will detect CSPEC.md and focus on implementation decisions only.
```
...
- CSPEC.md is NEVER written if the user selects "Abandon"
...
- CSPEC.md contains only falsifiable requirements
- Boundaries are explicit (in scope / out of scope with reasoning)
- Acceptance criteria are pass/fail checkboxes
- CSPEC.md committed atomically (when commit_docs is true)
- User directed to /csp-discuss-phase as next step
</success_criteria>
