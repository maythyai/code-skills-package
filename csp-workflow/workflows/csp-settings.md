...
- `workflow.pattern_mapper` — run csp-pattern-mapper between research and planning (default: true if absent)
- `workflow.ui_phase` — generate UI-CSPEC.md design contracts for frontend phases (default: true if absent)
- `workflow.ui_safety_gate` — prompt to run /csp-ui-phase before planning frontend phases (default: true if absent)
...
AskUserQuestion([
  ...
  {
    question: "Enable Code Review? (/csp-code-review and /csp-code-review --fix commands)",
    header: "Code Review",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Enable /csp-code-review commands for reviewing source files changed during a phase." },
  ...
  {
    question: "Code Review Depth? (default depth for /csp-code-review — override per-run with --depth=)",
    header: "Review Depth",
    multiSelect: false,
    options: [
  ...
  {
    question: "Enable UI Review? (visual quality audit via /csp-ui-review in autonomous mode)",
    header: "UI Review",
    multiSelect: false,
    options: [
  ...
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "csp-pattern-mapper runs between research and plan steps. Surfaces conventions so new code follows house style." },
  ...
  {
    question: "Enable UI Phase? (generates UI-CSPEC.md design contracts for frontend phases)",
    header: "UI Phase",
    multiSelect: false,
    options: [
      ...
      { label: "No", description: "Skip UI-CSPEC generation. Good for backend-only projects or API phases." }
    ]
  },
  {
    question: "Enable UI Safety Gate? (prompts to run /csp-ui-phase before planning frontend phases)",
    header: "UI Gate",
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "plan-phase asks to run /csp-ui-phase first when frontend indicators detected." },
      { label: "No", description: "No prompt — plan-phase proceeds without UI-CSPEC check." }
    ]
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes (Recommended)", description: "Run /csp-ai-integration-phase before planning AI system phases. Surfaces the right framework, researches its docs, and designs the evaluation strategy." },
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Per Phase", description: "Create branch for each phase (csp/phase-{N}-{name})" },
      { label: "Per Milestone", description: "Create branch for entire milestone (csp/{version}-{name})" }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Skip discuss in /csp-autonomous — chain directly to plan. Best for backend/pipeline work where phase descriptions are the spec." }
    ]
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Enable /csp-map-codebase --query commands. Builds and queries a JSON index of the codebase." }
    ]
  },
  {
    ...
    options: [
      ...
      { label: "Yes", description: "Enable /csp-graphify commands. Builds and queries a project knowledge graph." }
    ]
  },
  {
    ...
    multiSelect: false,
    options: [
      { label: "No (Recommended)", description: "Manual /csp-graphify build only. Conservative default — opt in if you want fresh context on every /csp-quick or /csp-plan-phase." },
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "Yes", description: "New projects start with these settings (saved to ~/.csp/defaults.json)" },
...
If "Yes": write the same config object (minus project-specific fields like `brave_search`) to `~/.csp/defaults.json`:

```bash
mkdir -p ~/.csp
```

Write `~/.csp/defaults.json` with:
```json
{
  "mode": <current>,
  "granularity": <current>,
...
These settings apply to future /csp-plan-phase and /csp-execute-phase runs.

Quick commands:
- /csp-config --integrations — configure API keys (Brave/Firecrawl/Exa), review.models CLI routing, and agent_skills injection
- /csp-config --profile <profile> — switch model profile
- /csp-plan-phase --research — force research
- /csp-plan-phase --skip-research — skip research
- /csp-plan-phase --skip-verify — skip plan check
- /csp-config --advanced — power-user tuning (plan bounce, timeouts, branch templates, cross-AI, context window)
```
</step>
...
- [ ] User offered to save as global defaults (~/.csp/defaults.json)
