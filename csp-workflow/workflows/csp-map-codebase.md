...
post-execute codebase-drift gate in `/csp-execute-phase` or by a user running
`/csp-map-codebase --paths apps/accounting,packages/ui`), the workflow
operates in **incremental-remap mode**:

- Pass `--paths <p1>,<p2>,...` through to each spawned `csp-codebase-mapper`
...
```bash
INIT=$(csp-sdk query init.map-codebase)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_MAPPER=$(csp-sdk query agent-skills csp-codebase-mapper)
```
...
Spawn 4 parallel csp-codebase-mapper agents.

Use Agent tool with `subagent_type="csp-codebase-mapper"`, `model="{mapper_model}"`, and `run_in_background=true` for parallel execution.

**CRITICAL:** Use the dedicated `csp-codebase-mapper` agent, NOT `Explore` or `browser_subagent`. The mapper agent writes documents directly.
...
- User offered clear next steps in CSP style
</success_criteria>
