...
```bash
INIT=$(csp-sdk query init.milestone-op)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
```

Parse JSON for: `milestone_version`, `milestone_name`, `phase_count`, `completed_phases`, `roadmap_exists`, `state_exists`, `commit_docs`.

**If `roadmap_exists` is false:** Error — "No ROADMAP.md found. Run `/csp-new-milestone` first."
**If `state_exists` is false:** Error — "No STATE.md found. Run `/csp-new-milestone` first."
...
```bash
ROADMAP=$(csp-sdk query roadmap.analyze)
```
...
```bash
DETAIL=$(csp-sdk query roadmap.get-phase ${PHASE_NUM})
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► AUTONOMOUS ▸ Phase {N}/{T}: {Name} [████░░░░] {P}%
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
```
...
```bash
PHASE_STATE=$(csp-sdk query init.phase-op ${PHASE_NUM})
```
...
```bash
SKIP_DISCUSS=$(csp-sdk query config-get workflow.skip_discuss 2>/dev/null || echo "false")
```
...
```bash
csp-sdk query commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```
Skill(skill="csp-discuss-phase", args="${PHASE_NUM}")
```
...
Check if this phase has frontend indicators and whether a UI-CSPEC already exists:

```bash
PHASE_SECTION=$(csp-sdk query roadmap.get-phase ${PHASE_NUM} 2>/dev/null)
...
**If `HAS_UI` is 0 (frontend indicators found) AND `UI_CSPEC_FILE` is empty (no UI-CSPEC exists) AND `UI_PHASE_CFG` is not `false`:**
...
```
Skill(skill="csp-ui-phase", args="${PHASE_NUM}")
```

Verify UI-CSPEC was created:

```bash
UI_CSPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-CSPEC.md 2>/dev/null | head -1)
```

**If `UI_CSPEC_FILE` is still empty after ui-phase:** Display warning `Phase ${PHASE_NUM}: UI-CSPEC generation did not produce output — continuing without design contract.` and proceed to 3b.

**If `HAS_UI` is 1 (no frontend indicators) OR `UI_CSPEC_FILE` is not empty (UI-CSPEC already exists) OR `UI_PHASE_CFG` is `false`:** Skip silently to 3b.
...
Agent(
  ...
  prompt="Run plan-phase for phase ${PHASE_NUM}: Skill(skill=\"csp-plan-phase\", args=\"${PHASE_NUM}\")"
)
```
...
```
Skill(skill="csp-plan-phase", args="${PHASE_NUM}")
```
...
Agent(
  ...
  run_in_background=true,
  prompt="Run execute-phase for phase ${PHASE_NUM}: Skill(skill=\"csp-execute-phase\", args=\"${PHASE_NUM} --no-transition\")"
)
```
...
```
Skill(skill="csp-execute-phase", args="${PHASE_NUM} --no-transition")
```
...
```
Skill(skill="csp-plan-phase", args="${PHASE_NUM} --gaps")
```
...
Check if this phase had a UI-CSPEC (created in step 3a.5 or pre-existing):
...
```bash
UI_REVIEW_CFG=$(csp-sdk query config-get workflow.ui_review 2>/dev/null || echo "true")
```

**If `UI_CSPEC_FILE` is not empty AND `UI_REVIEW_CFG` is not `false`:**

Display:

```
Phase ${PHASE_NUM}: Frontend phase with UI-CSPEC — running UI review audit...
```

```
Skill(skill="csp-ui-review", args="${PHASE_NUM}")
```
...
**If `UI_CSPEC_FILE` is empty OR `UI_REVIEW_CFG` is `false`:** Skip silently to iterate step.
...
> Full instructions are in `code-skills-package/references/autonomous-smart-discuss.md`. Read that file now and follow it exactly.

Smart discuss is an autonomous-optimized variant of `csp-discuss-phase`. It proposes grey area answers in batch tables — the user accepts or overrides per area — and writes an identical CONTEXT.md to what discuss-phase produces.

**Inputs:** `PHASE_NUM` from execute_phase.

Read and execute: `$HOME/.claude/code-skills-package/references/autonomous-smart-discuss.md`
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► AUTONOMOUS ▸ --to ${TO_PHASE} REACHED
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Completed through phase ${TO_PHASE} as requested.
 Remaining phases were not executed.

 Resume with: /csp-autonomous --from ${next_incomplete_phase}
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► AUTONOMOUS ▸ PHASE ${ONLY_PHASE} COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

 Phase ${ONLY_PHASE}: ${PHASE_NAME} — Done
 Mode: Single phase (--only)

 Lifecycle skipped — run /csp-autonomous without --only
...
```
Skill(skill="csp-audit-milestone")
```
...
On **"Stop"**: Go to handle_blocker with "User stopped — audit gaps remain. Run /csp-audit-milestone to review, then /csp-complete-milestone when ready."
...
On **"Stop"**: Go to handle_blocker with "User stopped — tech debt to address. Run /csp-audit-milestone to review details."

**5b. Complete Milestone**

```
Skill(skill="csp-complete-milestone", args="${milestone_version}")
```
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 ...
 Resume with: /csp-autonomous ${ONLY_PHASE ? "--only " + ONLY_PHASE : "--from " + next_phase}${TO_PHASE ? " --to " + TO_PHASE : ""}
```
...
- [ ] Audit invoked via Skill(skill="csp-audit-milestone")
...
- [ ] Frontend phases get UI-CSPEC generated before planning (step 3a.5) if not already present
- [ ] Frontend phases get UI review audit after successful execution (step 3d.5) if UI-CSPEC exists
...
- [ ] `--interactive` runs discuss inline via csp-discuss-phase (asks questions, waits for user)
