...
- Check for MILESTONE-CONTEXT.md (from /csp-discuss-milestone)
...
```bash
csp-sdk query state.milestone-switch --milestone "v[X.Y]" --name "[Name]"
```
...
```bash
csp-sdk query phases.clear --confirm
```
...
```bash
INIT=$(csp-sdk query init.new-milestone)
...
- Tell the user to complete/archive the previous milestone first, then rerun `/csp-new-milestone --reset-phase-numbers ${CSP_WS}`.
...
**IMPORTANT:** Do NOT persist this choice to config.json. The `workflow.research` setting is a persistent user preference that controls plan-phase behavior across the project. Changing it here would silently alter future `/csp-plan-phase` behavior. To change the default, use `/csp-settings`.
...
`/csp-discuss-phase [N] ${CSP_WS}` — gather context and clarify approach

Also: `/csp-plan-phase [N] ${CSP_WS}` — skip discussion, plan directly
```
...
- [ ] csp-roadmapper spawned with phase numbering context
...
- [ ] User knows next step: `/csp-discuss-phase [N] ${CSP_WS}`
