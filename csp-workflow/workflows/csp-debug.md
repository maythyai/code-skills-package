...
Invoked by `/csp-debug` (`commands/csp/debug.md`).

Systematic debugging using the scientific method with subagent isolation.
Orchestrates symptom gathering, session creation, and delegation to `csp-debug-session-manager`.

<available_agent_types>
Valid CSP subagent types (use exact names — do not fall back to 'general-purpose'):
- csp-debug-session-manager — manages debug checkpoint/continuation loop in isolated context
- csp-debugger — investigates bugs using scientific method
</available_agent_types>
...
```bash
debugger_model=$(csp-sdk query resolve-model csp-debugger 2>/dev/null | jq -r '.model' 2>/dev/null || true)
```

Read TDD mode from config:
```bash
TDD_MODE=$(csp-sdk query config-get workflow.tdd_mode 2>/dev/null | jq -r 'if type == "boolean" then tostring else . end' 2>/dev/null || echo "false")
```
...
─────────────────────────────────────────────
Run `/csp-debug continue <slug>` to resume a session.
No sessions? `/csp-debug <description>` to start.
```

If no files exist or the glob returns nothing: print "No active debug sessions. Run `/csp-debug <issue description>` to start one."
...
""",
  subagent_type="csp-debug-session-manager",
...
## 4. Session Management (delegated to csp-debug-session-manager)

After initial context setup, spawn the session manager to handle the full checkpoint/continuation loop. The session manager handles specialist_hint dispatch internally: when csp-debugger returns ROOT CAUSE FOUND it extracts the specialist_hint field and invokes the matching skill (e.g. typescript-expert, swift-concurrency) before offering fix options.
...
- [ ] csp-debug-session-manager spawned with security-hardened session_params
