...
> `$HOME/.claude/code-skills-package/USER-PROFILE.md` exists). Skip the Read
...
```bash
ADVISOR_MODEL=$(csp-sdk query resolve-model csp-advisor-researcher --raw)
```
...
```bash
PROFILE_CONTENT=$(cat "$HOME/.claude/code-skills-package/USER-PROFILE.md" 2>/dev/null || true)
```
...
2. For EACH user-selected gray area, spawn a `Agent()` in parallel:
   ...
   ```
   Agent(
     prompt="First, read @~/.claude/agents/csp-advisor-researcher.md for your role and instructions.
