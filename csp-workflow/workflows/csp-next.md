<purpose>
Detect current project state and automatically advance to the next logical CSP workflow step.
...
```bash
# Get state snapshot
csp-sdk query state.json 2>/dev/null || echo "{}"
```
...
Run `/csp-health` to diagnose, or manually fix STATE.md.
Use `--force` to bypass this check.
```
Exit.
...
After passing all three hard-stop gates, scan all phases that precede the current phase in ROADMAP.md order for incomplete work. For each prior phase number `N`, use `csp-sdk query find-phase <N>` JSON (plans, summaries, incomplete_plans, etc.) to inspect that phase.
...
**Source phase:** {src}
**Deferred at:** {date} during /csp-progress --next advancement to Phase {dest}
**Plans:**
- [ ] {N}-{M}: {slug} (ran, no SUMMARY.md)
```
2. Commit the deferral record:
```bash
csp-sdk query commit "docs: defer incomplete Phase {src} items to backlog"
```
...
→ Next action: `/csp-discuss-phase <first-phase>`
...
▶ **Next step:** `/csp-[command] [args]`
  [One-line explanation of why this is the next step]
```

Then immediately invoke the determined command via SlashCommand.
Do not ask for confirmation — the whole point of `/csp-progress --next` is zero-friction advancement.
</step>
