...
```bash
GRADUATION_ENABLED=$(csp-sdk query config-get features.graduation 2>/dev/null || echo "true")
GRADUATION_WINDOW=$(csp-sdk query config-get features.graduation_window 2>/dev/null || echo "5")
GRADUATION_THRESHOLD=$(csp-sdk query config-get features.graduation_threshold 2>/dev/null || echo "3")
```
...
```bash
find .csp/planning/phases -name "*-LEARNINGS.md" | sort | tail -n "$GRADUATION_WINDOW"
```
...
Read `.csp/planning/STATE.md` `graduation_backlog` section (if present). Format:
...
Write to `.csp/planning/STATE.md` under `graduation_backlog`:
```yaml
- cluster_id: "{sha256}"
  status: "deferred"
