<purpose>
Review source files changed during a phase for bugs, security issues, and code quality problems. Computes file scope (--files override > SUMMARY.md > git diff fallback), checks config gate, spawns csp-code-reviewer agent, commits REVIEW.md, and presents results to user. When --fix is passed, delegates to code-review-fix.md after review to auto-apply findings via csp-code-fixer.
</purpose>
...
- csp-code-reviewer: Reviews source files for bugs and quality issues
- csp-code-fixer: Applies fixes to code review findings (used via dispatch_fix → code-review-fix.md when --fix is passed)
</available_agent_types>
...
```
Error: Phase ${PHASE_ARG} not found. Run /csp-progress to see available phases.
```
...
FLAGS_JSON=$(node -e "
  const { parseCodeReviewFlags } = require('./code-skills-package/bin/lib/code-review-flags.cjs');
...
2. Config value: `csp-sdk query config-get workflow.code_review_depth 2>/dev/null`
3. Default: "standard"
...
if [ -n "$DEPTH_OVERRIDE" ]; then
  ...
  CONFIG_DEPTH=$(csp-sdk query config-get workflow.code_review_depth 2>/dev/null || echo "")
...
if [ ${#REVIEW_FILES[@]} -eq 0 ]; then
  ...
  if [ -n "$PHASE_COMMITS" ]; then
    ...
    echo "Use --files flag to specify files explicitly: /csp-code-review ${PHASE_ARG} --files=file1,file2,..."
  fi
fi
```
...
```bash
FALLOW_ENABLED=$(csp-sdk query config-get code_quality.fallow.enabled 2>/dev/null || echo "false")
FALLOW_SCOPE=$(csp-sdk query config-get code_quality.fallow.scope 2>/dev/null || echo "phase")
FALLOW_PROFILE=$(csp-sdk query config-get code_quality.fallow.profile 2>/dev/null || echo "standard")
FALLOW_MCP=$(csp-sdk query config-get code_quality.fallow.mcp 2>/dev/null || echo "false")
```
...
const { resolveFallowBinary } = require('./code-skills-package/bin/lib/fallow-runner.cjs');
...
Spawn the csp-code-reviewer agent:

```
Agent(subagent_type="csp-code-reviewer", prompt="
<files_to_read>
${FILES_TO_READ}
</files_to_read>
...
No REVIEW.md created. You can retry with /csp-code-review ${PHASE_ARG} or check agent logs.
```
...
if [ -f "${REVIEW_PATH}" ]; then
  ...
  echo "No REVIEW.md to commit. Please retry with /csp-code-review ${PHASE_ARG}"
fi
```
</step>
...
if [ "$FIX_FLAG" = "true" ]; then
  ...
  # csp-code-fixer agent dispatch, --auto iteration loop, REVIEW-FIX.md commit,
  # and result presentation. Do not duplicate that logic here.
  Workflow(workflow="code-skills-package/workflows/code-review-fix.md", args="${FIX_ARGS}")
...
Next steps:
  /csp-code-review ${PHASE_NUMBER} --fix  — Auto-fix issues
