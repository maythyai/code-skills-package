<purpose>
Create an isolated workspace directory with git repo copies (worktrees or clones) and an independent `.csp/planning/` directory. Supports multi-repo orchestration and single-repo feature branch isolation.
</purpose>
...
```bash
INIT=$(csp-sdk query init.new-workspace)
...
- `--name` → `WORKCSPACE_NAME` (required)
- `--repos` → `REPO_LIST` (comma-separated paths or names)
- `--path` → `TARGET_PATH` (defaults to `$default_workspace_base/$WORKCSPACE_NAME`)
- `--strategy` → `STRATEGY` (defaults to `worktree`)
- `--branch` → `BRANCH_NAME` (defaults to `workspace/$WORKCSPACE_NAME`)
...
Run this command from a directory containing git repos, or specify repos explicitly:
  /csp-workspace --new --name my-workspace --repos /path/to/repo1,/path/to/repo2
```
Exit.
...
Usage:
  /csp-workspace --new --name my-workspace --repos repo1,repo2 --auto
```
Exit.
...
## 7. Write WORKCSPACE.md

Write the workspace manifest at `$TARGET_PATH/WORKCSPACE.md`:

```markdown
# Workspace: $WORKCSPACE_NAME
...
## 8. Initialize .csp/planning/

```bash
mkdir -p "$TARGET_PATH/.csp/planning"
```
...
Next steps:
  cd "$TARGET_PATH"
  /csp-new-project    # Initialize CSP in the workspace
```
...
**Offer to initialize CSP (if not `--auto`):**

Use AskUserQuestion:
- header: "Initialize CSP"
- question: "Would you like to initialize a CSP project in the new workspace?"
- options:
  - "Yes — run /csp-new-project" → tell user to `cd "$TARGET_PATH"` first, then run `/csp-new-project`
...
- [ ] WORKCSPACE.md manifest written with correct repo table
- [ ] `.csp/planning/` directory initialized at workspace root
