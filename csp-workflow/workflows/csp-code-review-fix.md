<purpose>
Auto-fix issues from REVIEW.md. Validates phase, checks config gate, verifies REVIEW.md exists and has fixable issues, spawns csp-code-fixer agent, handles --auto iteration loop (capped at 3), commits REVIEW-FIX.md once at the end, and presents results.
</purpose>
...
- csp-code-fixer: Applies fixes to code review findings
- csp-code-reviewer: Reviews source files for bugs and issues
</available_agent_types>
...
if [ ! -f "${REVIEW_PATH}" ]; then
  echo "Error: No REVIEW.md found for Phase ${PHASE_ARG}. Run /csp-code-review ${PHASE_ARG} first."
  exit 1
fi
```
...
Spawn the csp-code-fixer agent with config:
...
```text
Agent(subagent_type="csp-code-fixer", prompt="
<files_to_read>
${REVIEW_PATH}
</files_to_read>
...
You can retry with /csp-code-review ${PHASE_ARG} --fix.
```
...
if [ "$AUTO_MODE" = "true" ]; then
  ...
  - ${f}"
    ...
    # Spawn csp-code-reviewer agent to re-review
...
if [ ! -f "${FIX_REPORT_PATH}" ]; then
  ...
  echo ""
  echo "Retry: /csp-code-review ${PHASE_ARG} --fix"
  echo ""
  echo "═══════════════════════════════════════════════════════════════"
  exit 1
fi
...
if [ "$FIX_STATUS" = "all_fixed" ]; then
  ...
  echo ""
  echo "Next step:"
  echo "  /csp-verify-work  — Verify phase completion"
  echo ""
fi
```
...
if [ "$FIX_STATUS" = "partial" ] || [ "$FIX_STATUS" = "none_fixed" ]; then
  ...
  echo "  /csp-code-review ${PHASE_NUMBER}           — Re-review code"
  echo "  /csp-verify-work                           — Verify phase completion"
  echo ""
fi
```
