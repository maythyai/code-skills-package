...
Invoked by `/csp-thread` (`commands/csp/thread.md`).
...
```
No threads found. Create one with: /csp-thread <description>
```
...
2. Read the file and display a summary:
   ...
   ─────────────────────────────────────
   Resume with: /csp-thread {SLUG}
   Close with:  /csp-thread close {SLUG}
   ```
...
- Lighter weight than /csp-pause-work — no phase state, no plan context
...
- Status fields read via csp-sdk query frontmatter.get — never eval'd or shell-expanded
- The generate-slug call for new threads runs through csp-sdk query (or csp-tools) which sanitizes input — keep that pattern
</security_notes>
