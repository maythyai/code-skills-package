<purpose>
Create all phases necessary to close gaps identified by `/csp-audit-milestone`. Reads MILESTONE-AUDIT.md, groups gaps into logical phases, creates phase entries in ROADMAP.md, and offers to plan each phase. One command creates all fix phases — no manual `/csp-add-phase` per gap.
</purpose>
...
```
No audit gaps found. Run `/csp-audit-milestone` first.
```
...
HIGHEST=$(csp-sdk query phases.list --pick directories[-1])
```
...
`/csp-audit-milestone` — re-audit to verify gaps closed
`/csp-complete-milestone {version}` — archive when audit passes
```
...
- [ ] User knows to run `/csp-plan-phase` next
</success_criteria>
