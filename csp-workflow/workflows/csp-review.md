...
OLLAMA_HOST=$(csp-sdk query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_HOST=$(csp-sdk query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LLAMA_CPP_HOST=$(csp-sdk query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
- Value must be a non-empty array of slug strings (configured via `csp config-set review.default_reviewers '["gemini","codex"]'`)
...
Write to a temp file: `/tmp/csp-review-prompt-{phase}.md`
...
cp "$INSTRUCTIONS_BLOCK_FILE" "/tmp/csp-review-${PHASE}-instructions.md"
cp "$ROADMAP_SECTION_FILE" "/tmp/csp-review-${PHASE}-roadmap.md"

# Plan files: copy each PLAN.md to a predictable numbered path
PLAN_INDEX=0
for PLAN_FILE in "${PHASE_DIR}"/*-PLAN.md; do
  PADDED_IDX=$(printf '%02d' "$PLAN_INDEX")
  cp "$PLAN_FILE" "/tmp/csp-review-${PHASE}-plan-${PADDED_IDX}.md"
...
```bash
# JSON scalars from csp-sdk query; use jq -r to strip JSON string quotes (install jq if missing)
GEMINI_MODEL=$(csp-sdk query config-get review.models.gemini 2>/dev/null | jq -r '.' 2>/dev/null || true)
CLAUDE_MODEL=$(csp-sdk query config-get review.models.claude 2>/dev/null | jq -r '.' 2>/dev/null || true)
CODEX_MODEL=$(csp-sdk query config-get review.models.codex 2>/dev/null | jq -r '.' 2>/dev/null || true)
OPENCODE_MODEL=$(csp-sdk query config-get review.models.opencode 2>/dev/null | jq -r '.' 2>/dev/null || true)
```
...
if [ -n "$GEMINI_MODEL" ] && [ "$GEMINI_MODEL" != "null" ]; then
  cat /tmp/csp-review-prompt-{phase}.md | gemini -m "$GEMINI_MODEL" -p - 2>/dev/null > /tmp/csp-review-gemini-{phase}.md
else
  cat /tmp/csp-review-prompt-{phase}.md | gemini -p - 2>/dev/null > /tmp/csp-review-gemini-{phase}.md
fi
```
...
if [ -n "$CLAUDE_MODEL" ] && [ "$CLAUDE_MODEL" != "null" ]; then
  cat /tmp/csp-review-prompt-{phase}.md | claude --model "$CLAUDE_MODEL" -p - 2>/dev/null > /tmp/csp-review-claude-{phase}.md
else
  cat /tmp/csp-review-prompt-{phase}.md | claude -p - 2>/dev/null > /tmp/csp-review-claude-{phase}.md
fi
```
...
if [ -n "$CODEX_MODEL" ] && [ "$CODEX_MODEL" != "null" ]; then
  cat /tmp/csp-review-prompt-{phase}.md | codex exec --model "$CODEX_MODEL" --skip-git-repo-check - 2>/dev/null > /tmp/csp-review-codex-{phase}.md
else
  cat /tmp/csp-review-prompt-{phase}.md | codex exec --skip-git-repo-check - 2>/dev/null > /tmp/csp-review-codex-{phase}.md
fi
```
...
```bash
coderabbit review --prompt-only 2>/dev/null > /tmp/csp-review-coderabbit-{phase}.md
```

**OpenCode (via GitHub Copilot):**
```bash
if [ -n "$OPENCODE_MODEL" ] && [ "$OPENCODE_MODEL" != "null" ]; then
  cat /tmp/csp-review-prompt-{phase}.md | opencode run --model "$OPENCODE_MODEL" - 2>/dev/null > /tmp/csp-review-opencode-{phase}.md
else
  cat /tmp/csp-review-prompt-{phase}.md | opencode run - 2>/dev/null > /tmp/csp-review-opencode-{phase}.md
fi
if [ ! -s /tmp/csp-review-opencode-{phase}.md ]; then
  echo "OpenCode review failed or returned empty output." > /tmp/csp-review-opencode-{phase}.md
fi
```

**Qwen Code:**
```bash
cat /tmp/csp-review-prompt-{phase}.md | qwen - 2>/dev/null > /tmp/csp-review-qwen-{phase}.md
if [ ! -s /tmp/csp-review-qwen-{phase}.md ]; then
  echo "Qwen review failed or returned empty output." > /tmp/csp-review-qwen-{phase}.md
fi
```

**Cursor:**
```bash
cat /tmp/csp-review-prompt-{phase}.md | cursor agent -p --mode ask --trust 2>/dev/null > /tmp/csp-review-cursor-{phase}.md
if [ ! -s /tmp/csp-review-cursor-{phase}.md ]; then
  echo "Cursor review failed or returned empty output." > /tmp/csp-review-cursor-{phase}.md
fi
```
...
prepare_trimmed_prompt_for_reviewer() {
  ...
  PLAN_FILE_ARGS=""
  for p in /tmp/csp-review-{phase}-plan-*.md; do
    [ -f "$p" ] && PLAN_FILE_ARGS="$PLAN_FILE_ARGS --plan-file $p"
  done
  PROJECT_ARG=""
  [ -f "/tmp/csp-review-{phase}-project.md" ] && PROJECT_ARG="--project-file /tmp/csp-review-{phase}-project.md"
  CONTEXT_ARG=""
  [ -f "/tmp/csp-review-{phase}-context.md" ] && CONTEXT_ARG="--context-file /tmp/csp-review-{phase}-context.md"
  RESEARCH_ARG=""
  [ -f "/tmp/csp-review-{phase}-research.md" ] && RESEARCH_ARG="--research-file /tmp/csp-review-{phase}-research.md"
  REQUIREMENTS_ARG=""
  [ -f "/tmp/csp-review-{phase}-requirements.md" ] && REQUIREMENTS_ARG="--requirements-file /tmp/csp-review-{phase}-requirements.md"

  csp-sdk query prompt-budget \
    --budget "$REVIEWER_BUDGET" \
    --instructions-file "/tmp/csp-review-{phase}-instructions.md" \
    --roadmap-file "/tmp/csp-review-{phase}-roadmap.md" \
...
OLLAMA_REVIEWER_BUDGET=$(csp-sdk query config-get review.max_prompt_tokens_per_reviewer.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$OLLAMA_REVIEWER_BUDGET" ] || [ "$OLLAMA_REVIEWER_BUDGET" = "null" ]; then
  OLLAMA_REVIEWER_BUDGET=$(csp-sdk query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for Ollama if a budget is configured
OLLAMA_PROMPT_FILE="/tmp/csp-review-prompt-{phase}.md"
OLLAMA_SKIP=0
if [ -n "$OLLAMA_REVIEWER_BUDGET" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "null" ] && [ "$OLLAMA_REVIEWER_BUDGET" != "0" ]; then
  OLLAMA_TRIMMED_PROMPT="/tmp/csp-review-prompt-{phase}-ollama.md"
  OLLAMA_TRIM_META="/tmp/csp-review-prompt-{phase}-ollama.metadata.json"
...
OLLAMA_HOST=$(csp-sdk query config-get review.ollama_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$OLLAMA_HOST" ] || [ "$OLLAMA_HOST" = "null" ]; then OLLAMA_HOST="http://localhost:11434"; fi
OLLAMA_MODEL=$(csp-sdk query config-get review.models.ollama 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
jq -n --rawfile content "$OLLAMA_PROMPT_FILE" \
  ...
  > /tmp/csp-review-ollama-{phase}.md
if [ ! -s /tmp/csp-review-ollama-{phase}.md ]; then
  echo "Ollama review failed or returned empty output." > /tmp/csp-review-ollama-{phase}.md
fi
fi
```
...
LM_STUDIO_REVIEWER_BUDGET=$(csp-sdk query config-get review.max_prompt_tokens_per_reviewer.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$LM_STUDIO_REVIEWER_BUDGET" ] || [ "$LM_STUDIO_REVIEWER_BUDGET" = "null" ]; then
  LM_STUDIO_REVIEWER_BUDGET=$(csp-sdk query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for LM Studio if a budget is configured
LM_STUDIO_PROMPT_FILE="/tmp/csp-review-prompt-{phase}.md"
LM_STUDIO_SKIP=0
if [ -n "$LM_STUDIO_REVIEWER_BUDGET" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "null" ] && [ "$LM_STUDIO_REVIEWER_BUDGET" != "0" ]; then
  LM_STUDIO_TRIMMED_PROMPT="/tmp/csp-review-prompt-{phase}-lm_studio.md"
  LM_STUDIO_TRIM_META="/tmp/csp-review-prompt-{phase}-lm_studio.metadata.json"
...
LM_STUDIO_HOST=$(csp-sdk query config-get review.lm_studio_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LM_STUDIO_HOST" ] || [ "$LM_STUDIO_HOST" = "null" ]; then LM_STUDIO_HOST="http://localhost:1234"; fi
LM_STUDIO_MODEL=$(csp-sdk query config-get review.models.lm_studio 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
LM_STUDIO_RECSPONSE=$(jq -n --rawfile content "$LM_STUDIO_PROMPT_FILE" \
  --arg model "$LM_STUDIO_MODEL" \
  '{model: $model, messages: [{role: "user", content: $content}]}' | \
  curl -s --max-time 120 -X POST "${LM_STUDIO_HOST}/v1/chat/completions" \
    -H "Content-Type: application/json" -d @- 2>/dev/null)
LM_STUDIO_ACTUAL_MODEL=$(echo "$LM_STUDIO_RECSPONSE" | jq -r '.model // ""' 2>/dev/null || echo "")
...
LM_STUDIO_CONTENT=$(echo "$LM_STUDIO_RECSPONSE" | jq -r '.choices[0].message.content // ""' 2>/dev/null || echo "")
if [ -n "$LM_STUDIO_CONTENT" ]; then
  echo "$LM_STUDIO_CONTENT" > /tmp/csp-review-lm_studio-{phase}.md
else
...
LLAMA_CPP_REVIEWER_BUDGET=$(csp-sdk query config-get review.max_prompt_tokens_per_reviewer.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
if [ -z "$LLAMA_CPP_REVIEWER_BUDGET" ] || [ "$LLAMA_CPP_REVIEWER_BUDGET" = "null" ]; then
  LLAMA_CPP_REVIEWER_BUDGET=$(csp-sdk query config-get review.max_prompt_tokens 2>/dev/null | jq -r '.' 2>/dev/null || echo "null")
fi

# Apply budget trim for llama.cpp if a budget is configured
LLAMA_CPP_PROMPT_FILE="/tmp/csp-review-prompt-{phase}.md"
LLAMA_CPP_SKIP=0
if [ -n "$LLAMA_CPP_REVIEWER_BUDGET" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "null" ] && [ "$LLAMA_CPP_REVIEWER_BUDGET" != "0" ]; then
  LLAMA_CPP_TRIMMED_PROMPT="/tmp/csp-review-prompt-{phase}-llama_cpp.md"
  LLAMA_CPP_TRIM_META="/tmp/csp-review-prompt-{phase}-llama_cpp.metadata.json"
...
LLAMA_CPP_HOST=$(csp-sdk query config-get review.llama_cpp_host 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
if [ -z "$LLAMA_CPP_HOST" ] || [ "$LLAMA_CPP_HOST" = "null" ]; then LLAMA_CPP_HOST="http://localhost:8080"; fi
LLAMA_CPP_MODEL=$(csp-sdk query config-get review.models.llama_cpp 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
...
if [ -n "$LLAMA_CPP_CONTENT" ]; then
  echo "$LLAMA_CPP_CONTENT" > /tmp/csp-review-llama_cpp-{phase}.md
else
...
Commit:
```bash
csp-sdk query commit "docs: cross-AI review for phase {N}" --files {phase_dir}/{padded_phase}-REVIEWS.md
```
</step>
...
- [ ] User knows how to use feedback (/csp-plan-phase --reviews)
</success_criteria>
