...
```bash
INIT=$(csp-sdk query init.progress)
...
Run /csp-new-project to start a new project.
```

Exit.

If missing STATE.md: suggest `/csp-new-project`.

**If ROADMAP.md missing but PROJECT.md exists:**

This means a milestone was completed and archived. Go to **Route F** (between milestones).

If missing both ROADMAP.md and PROJECT.md: suggest `/csp-new-project`.
</step>

<step name="load">
**Use structured extraction from `csp-sdk query` (or legacy csp-tools.cjs):**

Instead of reading full files, use targeted tools to get only the data needed for the report:
- `ROADMAP=$(csp-sdk query roadmap.analyze)`
- `STATE=$(csp-sdk query state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  csp-sdk query summary-extract <path> --fields one_liner
  ```
...
**Generate progress bar from `csp-sdk query progress` / `progress.json`, then present rich status report:**

```bash
# Get formatted progress bar
PROGRESS_BAR=$(csp-sdk query progress.bar --raw)
```
...
## Pending Todos
- [count] pending — /csp-capture --list to review

## Active Debug Sessions
- [count] active — /csp-debug to continue
...
```bash
MVP_MODE=$(csp-sdk query phase.mvp-mode "${PHASE_NUMBER}" --pick active)
```
...
```bash
DEBT=$(csp-sdk query audit-uat --raw 2>/dev/null)
```
...
Review: `/csp-audit-uat ${CSP_WS}` — full cross-phase audit
Resume testing: `/csp-verify-work {phase} ${CSP_WS}` — retest specific phase
```
...
```bash
PHASE_SECTION=$(csp-sdk query roadmap.get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
`/csp-plan-phase {phase-number} ${CSP_WS}`
...
`/csp-discuss-phase {phase}` — gather context and clarify approach

---

**Also available:**
- `/csp-ui-phase {phase}` — generate UI design contract (recommended for frontend phases)
- `/csp-plan-phase {phase}` — skip discussion, plan directly
- `/csp-discuss-phase {phase}` — include assumptions check before planning
...
`/csp-discuss-phase {phase} ${CSP_WS}` — gather context and clarify approach

---

**Also available:**
- `/csp-plan-phase {phase} ${CSP_WS}` — skip discussion, plan directly
- `/csp-discuss-phase {phase} ${CSP_WS}` — include assumptions check before planning
...
`/csp-plan-phase {phase} --gaps ${CSP_WS}`

---

**Also available:**
- `/csp-execute-phase {phase} ${CSP_WS}` — execute phase plans
- `/csp-verify-work {phase} ${CSP_WS}` — run more UAT testing
...
`/csp-verify-work {phase} ${CSP_WS}` — resume testing from where you left off

---

**Also available:**
- `/csp-audit-uat ${CSP_WS}` — full cross-phase UAT audit
- `/csp-execute-phase {phase} ${CSP_WS}` — execute phase plans
...
```bash
NEXT_PHASE_SECTION=$(csp-sdk query roadmap.get-phase "$((Z+1))" 2>/dev/null)
...
`/csp-discuss-phase {Z+1}` — gather context and clarify approach

---

**Also available:**
- `/csp-ui-phase {Z+1}` — generate UI design contract (recommended for frontend phases)
- `/csp-plan-phase {Z+1}` — skip discussion, plan directly
- `/csp-verify-work {Z}` — user acceptance test before continuing
...
`/csp-discuss-phase {Z+1} ${CSP_WS}` — gather context and clarify approach

---

**Also available:**
- `/csp-plan-phase {Z+1} ${CSP_WS}` — skip discussion, plan directly
- `/csp-verify-work {Z} ${CSP_WS}` — user acceptance test before continuing
...
`/csp-complete-milestone ${CSP_WS}`

---

**Also available:**
- `/csp-verify-work ${CSP_WS}` — user acceptance test before completing milestone
...
- Phase complete but next phase not planned → offer `/csp-plan-phase [next] ${CSP_WS}`
- All work complete → offer milestone completion
- Blockers present → highlight before offering to continue
- Handoff file exists → mention it, offer `/csp-resume-work ${CSP_WS}`
</step>
...
- [ ] Smart routing: /csp-execute-phase if plans exist, /csp-plan-phase if not
- [ ] User confirms before any action
- [ ] Seamless handoff to appropriate csp command
      </success_criteria>
