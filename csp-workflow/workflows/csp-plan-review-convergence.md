...
csp-plan-phase N → csp-review N --codex → csp-plan-phase N --reviews → csp-review N --codex → ...
...
@$HOME/.claude/code-skills-package/references/revision-loop.md
@$HOME/.claude/code-skills-package/references/gates.md
@$HOME/.claude/code-skills-package/references/agent-contracts.md
</required_reading>
...
CSP_WS=""
echo "$ARGUMENTS" | grep -qE '\-\-ws\s+\S+' && CSP_WS=$(echo "$ARGUMENTS" | grep -oE '\-\-ws\s+\S+')
```

## 1.5. Config Gate (feature disabled by default)

```bash
CONVERGENCE_ENABLED=$(csp-sdk query config-get workflow.plan_review_convergence 2>/dev/null || echo "false")
```

**If `CONVERGENCE_ENABLED` is not `"true"`:** Display and exit:

```text
csp-plan-review-convergence is disabled (workflow.plan_review_convergence=false).

This feature automates the plan→review→replan loop using external AI reviewers.
Enable it with:

  csp config-set workflow.plan_review_convergence true

Then re-run: /csp-plan-review-convergence {PHASE}
```

## 2. Initialize

```bash
INIT=$(node "$HOME/.claude/code-skills-package/bin/csp-tools.cjs" init plan-phase "$PHASE")
...
```bash
PHASE_INFO=$(node "$HOME/.claude/code-skills-package/bin/csp-tools.cjs" roadmap get-phase "${PHASE}")
```
...
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► PLAN CONVERGENCE — Phase {phase_number}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
Agent(
  ...
  prompt="Run /csp-plan-phase for Phase {PHASE}.

Execute: Skill(skill='csp-plan-phase', args='{PHASE} {CSP_WS}')
...
Agent(
  ...
  prompt="Run /csp-review for Phase {PHASE}.

Execute: Skill(skill='csp-review', args='--phase {PHASE} {REVIEWER_FLAGS} {CSP_WS}')
...
```bash
node "$HOME/.claude/code-skills-package/bin/csp-tools.cjs" state planned-phase --phase "${PHASE}" --name "${phase_name}" --plans "${PLAN_COUNT}"
```

Display:
```text
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► CONVERGENCE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
To replan manually:  /csp-plan-phase {PHASE} --reviews
To restart loop:     /csp-plan-review-convergence {PHASE} {REVIEWER_FLAGS}
```
Exit workflow.
...
Agent(
  ...
  prompt="Run /csp-plan-phase with --reviews for Phase {PHASE}.

Execute: Skill(skill='csp-plan-phase', args='{PHASE} --reviews --skip-research {CSP_WS}')
...
IMPORTANT: When csp-plan-phase outputs '## PLANNING COMPLETE', that means replanning is done. Return at that point.",
  mode="auto"
)
```
...
- [ ] Initial planning via Agent → Skill("csp-plan-phase") if no plans exist
- [ ] Review via Agent → Skill("csp-review") — isolated, not inline; {CSP_WS} forwarded
- [ ] Replan via Agent → Skill("csp-plan-phase --reviews") — isolated, not inline
