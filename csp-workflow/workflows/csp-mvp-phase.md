<purpose>
Guide the user through MVP-mode planning for a phase. Prompts for an "As a / I want to / So that" user story, runs CSPIDR splitting check on the story, writes the result to ROADMAP.md, and delegates to `/csp plan-phase` (which auto-detects MVP via the roadmap mode field shipped in PRD Phase 1).
</purpose>

<required_reading>
@~/.claude/code-skills-package/csp-workflow/references/user-story-template.md
@~/.claude/code-skills-package/csp-workflow/references/spidr-splitting.md
@~/.claude/code-skills-package/csp-workflow/references/planner-mvp-mode.md
</required_reading>
...
Usage: /csp mvp-phase <phase-number>
Example: /csp mvp-phase 1
Example: /csp mvp-phase 2.1
```
Exit.

Normalize per `@~/.claude/code-skills-package/csp-workflow/references/phase-argument-parsing.md` (zero-pad integer phases to two digits).
...
ANALYZE=$(csp-sdk query roadmap.analyze)
...
If `PHASE_FOUND` is `false`: error and exit. Suggest `/csp add-phase` or `/csp insert-phase` to create the phase first.
...
To proceed anyway: /csp mvp-phase ${PHASE} --force
```

**Already-MVP guard.** If `PHASE_MODE` is already `mvp`, surface this and ask whether to re-prompt the user story or abort:

> "Phase ${PHASE} is already in MVP mode with goal: «${PHASE_GOAL}». Re-run user-story prompts and CSPIDR check?"

Use `AskUserQuestion` with options [Re-prompt / Abort]. On Abort, exit cleanly. On Re-prompt, proceed.

## 3. User story prompts

Run three sequential `AskUserQuestion` calls. Each is free-text. After all three, assemble into the canonical sentence per `@~/.claude/code-skills-package/csp-workflow/references/user-story-template.md`:
...
```bash
USER_STORY_RESULT=$(csp-sdk query user-story.validate --story "$USER_STORY")
...
## 4. CSPIDR splitting check

Run the CSPIDR rules from `@~/.claude/code-skills-package/csp-workflow/references/spidr-splitting.md`. Briefly:

**Trigger evaluation.** Check the assembled `USER_STORY` against the four size signals from the reference (compound capabilities, multi-actor, length > 120 chars, vague capability). If none fire, **skip CSPIDR** entirely — go to step 5.
...
>
> Want to walk through CSPIDR splitting?"

Use `AskUserQuestion` with options [Yes, walk through CSPIDR / No, proceed with the story as-is].

If "No": skip CSPIDR, go to step 5.

If "Yes": continue to (b).

b) Ask which CSPIDR axis fits best:
...
- **Accept**: `USER_STORY` becomes the first split's story (`${HAPPY_STORY}` in the example). Surface the remaining splits as a list of `/csp add-phase` invocations the user can run after this command completes — do NOT auto-create the new phases (preserve user control over numbering).
...
```bash
NEW_MODE=$(csp-sdk query roadmap.get-phase "${PHASE}" --pick mode)
NEW_GOAL=$(csp-sdk query roadmap.get-phase "${PHASE}" --pick goal)
```
...
## 7. Delegate to /csp plan-phase

Invoke `/csp plan-phase ${PHASE}` (no flags). Phase 1's MVP_MODE resolution chain (CLI flag → roadmap mode → config → false) will detect the new `**Mode:** mvp` line and run plan-phase in vertical-slice mode automatically.
...
If CSPIDR produced a split in step 4, append a final user-facing message:

> "**CSPIDR split deferred phases.**
>
...
>
> - `/csp add-phase` — for the next slice: «${CSPLIT_2_STORY}»
> - `/csp add-phase` — for the next slice: «${CSPLIT_3_STORY}»
>
> Each will be added to the end of the current milestone. You can then run
> `/csp mvp-phase <new-phase-number>` on each to plan them as MVP slices."
