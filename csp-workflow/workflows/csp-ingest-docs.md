...
```bash
INIT=$(node "$HOME/.claude/code-skills-package/bin/csp-tools.cjs" init ingest-docs)
...
Each entry provides `path` (required, relative to repo root) + `type` (required, one of ADR|PRD|CSPEC|DOC) + `precedence` (optional integer).
...
# CSPECs / RFCs
find {SCAN_PATH} -type f \( -path '*/spec/*' -o -path '*/specs/*' -o -path '*/rfc/*' -o -path '*/rfcs/*' -o -name 'CSPEC-*.md' -o -name 'RFC-*.md' \) 2>/dev/null
...
```
CSP > Discovered {N} docs, which exceeds the v1 cap of 50.
      Use --manifest to narrow the set to ≤ 50 files, or run
      /csp-ingest-docs again with a narrower <path>.
```
...
Discovered {N} documents:
  {N} ADR | {N} PRD | {N} CSPEC | {N} DOC | {N} unclassified
...
Spawn `csp-doc-synthesizer` once:
...
Agent({
  ...
  prompt: "
    ...
    <required_reading>
    - agents/csp-doc-synthesizer.md
    - code-skills-package/references/doc-conflict-engine.md
    </required_reading>
  "
})
```
...
```
CSP > BLOCKED: {N} blockers must be resolved before ingest can proceed.
```
...
Proceed to routing silently, or optionally display `CSP > No conflicts. Auto-resolved: {N}.`
