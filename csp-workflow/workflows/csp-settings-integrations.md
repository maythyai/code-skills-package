...
and any `/csp-settings-advanced` tuning surface. It exists because API keys and
...
- **`config-set` output is masked** for keys in the secret set
  ...
  `code-skills-package/bin/lib/secrets.cjs`.
...
Store `$CSP_CONFIG_PATH`. Every subsequent read/write uses it.
</step>
...
```bash
BRAVE=$(csp-sdk query config-get brave_search --default null)
FIRECRAWL=$(csp-sdk query config-get firecrawl --default null)
EXA=$(csp-sdk query config-get exa_search --default null)
SEARCH_GITIGNORED=$(csp-sdk query config-get search_gitignored --default false)
```
...
```bash
csp-sdk query config-set brave_search "<value>"     # masked in output
csp-sdk query config-set firecrawl "<value>"        # masked in output
csp-sdk query config-set exa_search "<value>"       # masked in output
csp-sdk query config-set search_gitignored true|false
```

For "Clear", write `null`:

```bash
csp-sdk query config-set brave_search null
```
</step>
...
```bash
csp-sdk query config-set review.models.<cli> "<command string>"
```
...
AskUserQuestion([
  {
    ...
    multiSelect: false,
    options: [
      { label: "csp-executor", description: "Skills injected when spawning executor agents" },
      { label: "csp-planner", description: "Skills injected when spawning planner agents" },
      { label: "csp-verifier", description: "Skills injected when spawning verifier agents" },
...
```bash
csp-sdk query config-set agent_skills.<slug> "<skill-a,skill-b,skill-c>"
```
...
Quick commands:
- /csp-settings — workflow toggles and model profile
- /csp-set-profile <profile> — switch model profile
```
</step>
...
<success_criteria>
- [ ] Current config read from `$CSP_CONFIG_PATH`
