<purpose>
Interactive configuration of CSP power-user knobs — plan bounce, node repair, subagent timeouts,
...
This is a companion to `/csp-settings` — the common-case prompt there covers model profile,
...
- `git.phase_branch_template` (default: `csp/phase-{phase}-{slug}`)
- `git.milestone_branch_template` (default: `csp/{milestone}-{slug}`)
...
AskUserQuestion([
  ...
  {
    question: "Phase branch template? (current: <value or csp/phase-{phase}-{slug}>)",
  ...
  {
    question: "Milestone branch template? (current: <value or csp/{milestone}-{slug}>)",
...
This section lets the user inspect and override the built-in model IDs CSP resolves for each
...
`csp-sdk query config-set runtime <value>` before proceeding to Step C.
...
```bash
csp-sdk query config-set model_profile_overrides.<runtime>.<tier> "<model-id>"
```

For each tier where the user chose "Clear override", remove the key by setting it to null:
```bash
csp-sdk query config-set model_profile_overrides.<runtime>.<tier> null
```
...
Merge the new settings into the existing config at `$CSP_CONFIG_PATH`. This merge is the
core correctness invariant: **preserve every unrelated key** — do not clobber siblings.

Apply each selected value via `csp-sdk query config-set <key> <value>` so the central
...
csp-sdk query config-set workflow.plan_bounce_passes 5
csp-sdk query config-set workflow.subagent_timeout 900
csp-sdk query config-set git.base_branch main
csp-sdk query config-set context_window 1000000
# Runtime model tier examples:
csp-sdk query config-set runtime gemini
csp-sdk query config-set model_profile_overrides.gemini.opus gemini-3-ultra
csp-sdk query config-set model_profile_overrides.gemini.haiku null
```
...
route each write through `csp-sdk query config-set` so sibling preservation is handled by
the central setter.
</step>
...
These settings apply to future /csp-plan-phase, /csp-execute-phase, /csp-discuss-phase,
and /csp-ship runs.

For common-case toggles (model profile, research/plan_check/verifier, branching strategy,
UI/AI phase gates), use /csp-settings.
```
</step>
...
<success_criteria>
- [ ] Current config read from resolved `$CSP_CONFIG_PATH`
...
- [ ] Writes routed through `csp-sdk query config-set` so unrelated keys are preserved
