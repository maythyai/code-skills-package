...
- csp-doc-writer — Writes and updates project documentation files
- csp-doc-verifier — Verifies factual claims in docs against the live codebase
</available_agent_types>
...
```bash
INIT=$(csp-sdk query docs-init)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(csp-sdk query agent-skills csp-doc-writer)
```
...
- `existing_docs` — array of `{path, has_csp_marker}` objects for existing Markdown files
...
- These will be passed to csp-doc-verifier in the verify_docs step for accuracy checking
- If inaccuracies are found, they will be dispatched to csp-doc-writer in `fix` mode for surgical corrections
...
4. For each gap the user selects:
   ...
   - The csp-doc-writer will receive a `doc_assignment` with `type: "custom"` and a description of what to document, using the project's source files as content discovery targets
...
3. If no docs in the queue have `has_csp_marker: false` in the `existing_docs` array: skip to detect_runtime_capabilities.

**For each queued doc where `has_csp_marker` is false (hand-written doc detected):**

Present the following choice using `AskUserQuestion` if available, or inline prompt otherwise:

```
{filename} appears to be hand-written (no CSP marker found).
...
How should this file be handled?
  ...
  [3] regenerate  -- Overwrite with a fresh CSP-generated doc.
```
...
Spawn a `csp-doc-writer` agent with `run_in_background=true`:
...
**Fallback when Task tool is unavailable:** Generate per-package READMEs sequentially inline after the `sequential_generation` step. For each package directory with a `package.json`, construct the equivalent `doc_assignment` block and generate the README following csp-doc-writer instructions.
...
Read `agents/csp-doc-writer.md` instructions once before beginning. Follow the create_mode or update_mode instructions from that agent for each doc, using the same doc_assignment fields as the parallel path.
...
1. **README** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase (Read, Grep, Glob, Bash) following csp-doc-writer create_mode / update_mode instructions
...
2. **ARCHITECTURE** — mode from resolve_modes; for update/supplement mode, include existing_content
   ...
   - Explore the codebase following csp-doc-writer instructions
...
- Follow csp-doc-writer instructions for per_package scope
...
1. Spawn the `csp-doc-verifier` agent (or invoke sequentially if Task tool is unavailable) with a `<verify_assignment>` block:
   ```xml
   <verify_assignment>
...
1. For each doc with `claims_failed > 0` in the latest verification_results:
   ...
   b. Spawn `csp-doc-writer` agent (or invoke sequentially) with a fix assignment:
      ```xml
      <doc_assignment>
...
Invoke the csp-doc-verifier agent in read-only mode for each file in `existing_docs` from the init JSON:

1. For each doc in `existing_docs`:
   a. Spawn `csp-doc-verifier` (or invoke sequentially if Task tool is unavailable) with:
      ```xml
      <verify_assignment>
      doc_path: {doc.path}
      project_root: {project_root from init JSON}
...
Display note:
```
To fix failures automatically: /csp-docs-update (runs generation + fix loop)
...
3. Re-run /csp-docs-update to regenerate clean docs
```
...
```bash
csp-sdk query commit "docs: generate project documentation" \
...
```
Run `/csp-docs-update --verify-only` to fact-check generated docs against the codebase.
```
...
- [ ] Generated docs contain zero CSP methodology content
...
- [ ] Hand-written docs (no CSP marker) prompted for preserve/supplement/regenerate before dispatch (unless --force)
...
- [ ] --verify-only invokes csp-doc-verifier for full fact-checking (not just VERIFY marker count)
</success_criteria>
