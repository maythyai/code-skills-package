...
Example: `/csp-phase --insert 72 Fix critical auth bug`
-> after = 72
...
Usage: /csp-phase --insert <after> <description>
Example: /csp-phase --insert 72 Fix critical auth bug
```
...
```bash
INIT=$(csp-sdk query init.phase-op "${after_phase}")
...
**Delegate the phase insertion to `csp-sdk query phase.insert`:**

```bash
RESULT=$(csp-sdk query phase.insert "${after_phase}" "${description}")
```
...
- Creating the phase directory (`.csp/planning/phases/{N.M}-{slug}/`)
...
1. Update STATE.md's next-phase pointer(s) to the newly inserted phase
   ...
   ```bash
   csp-sdk query state.patch '{"Current Phase":"{decimal_phase}","Next recommended run":"/csp-plan-phase {decimal_phase}"}'
   ```
...
2. Append a Roadmap Evolution entry via the dedicated handler. It creates the
   ...
   ```bash
   csp-sdk query state.add-roadmap-evolution \
     --phase {decimal_phase} \
     --action inserted \
     --after {after_phase} \
     --note "{description}" \
...
- Directory: .csp/planning/phases/{decimal-phase}-{slug}/
- Status: Not planned yet
- Marker: (INSERTED) - indicates urgent work

Roadmap updated: .csp/planning/ROADMAP.md
Project state updated: .csp/planning/STATE.md
...
- Don't use this for planned work at end of milestone (use /csp-add-phase)
- Don't insert before Phase 1 (decimal 0.1 makes no sense)
- Don't renumber existing phases
- Don't modify the target phase content
- Don't create plans yet (that's /csp-plan-phase)
...
- [ ] `csp-sdk query phase.insert` executed successfully
- [ ] Phase directory created
- [ ] Roadmap updated with new phase entry (includes "(INSERTED)" marker)
- [ ] `csp-sdk query state.add-roadmap-evolution ...` returned `{ added: true }` or `{ added: false, reason: "duplicate" }`
- [ ] `csp-sdk query state.patch` returned matched next-phase pointer field(s)
