...
- csp-code-reviewer — Reviews source files for bugs, security issues, and code quality
</available_agent_types>
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► QUICK TASK (DISCUSS + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► QUICK TASK (DISCUSS + RESEARCH)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► QUICK TASK (RESEARCH + VALIDATE)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
```bash
if ! command -v csp-sdk &>/dev/null; then
  echo "⚠ csp-sdk not found in PATH — /csp-quick requires it."
  echo ""
  echo "Install the query-capable CSP SDK CLI:"
  echo "  npm install -g code-skills-package-cc"
  echo ""
  echo "Or update CSP to get the latest packages:"
  echo "  /csp-update"
  exit 1
fi
```
...
```bash
INIT=$(csp-sdk query init.quick "$DESCRIPTION")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_PLANNER=$(csp-sdk query agent-skills csp-planner)
AGENT_SKILLS_EXECUTOR=$(csp-sdk query agent-skills csp-executor)
AGENT_SKILLS_CHECKER=$(csp-sdk query agent-skills csp-plan-checker)
AGENT_SKILLS_VERIFIER=$(csp-sdk query agent-skills csp-verifier)
```
...
if [ "$USE_WORKTREES" != "false" ]; then
  csp-sdk query worktree.reap-orphans 2>/dev/null || true
fi
```
...
**If `roadmap_exists` is false:** Error — Quick mode requires an active project with ROADMAP.md. Run `/csp-new-project` first.
...
if [ "${USE_WORKTREES:-true}" != "false" ]; then
  QUICK_WORKTREE_MANIFEST=$(mktemp "${TMPDIR:-/tmp}/csp-quick-worktree-XXXXXX.json")
  printf '{"worktrees":[]}\n' > "$QUICK_WORKTREE_MANIFEST"
  export QUICK_WORKTREE_MANIFEST
fi
```

Spawn csp-executor with plan reference:
...
1. **Worktree cleanup:** If the executor ran with `isolation="worktree"`, merge the worktree branch back and clean up:
   ...
   if command -v csp-sdk >/dev/null 2>&1; then
     csp-sdk query worktree.cleanup-wave --manifest "$QUICK_WORKTREE_MANIFEST" || exit 1
   else
...
Agent(
  ...
  Depth: quick",
  subagent_type="csp-code-reviewer",
  model="{executor_model}"
)
```
...
Stage and commit quick task artifacts. This step MUST always run — even if the executor already committed some files (e.g. when running without worktree isolation). The `csp-sdk query commit` command (or legacy `csp-tools.cjs` commit) handles already-committed files gracefully.
...
csp-sdk query commit "docs(quick-${quick_id}): ${DESCRIPTION}" --files ${file_list}
```
...
CSP > QUICK TASK COMPLETE (VALIDATED)
...
Ready for next task: /csp-quick ${CSP_WS}
```
