...
Seeds auto-surface during /csp-new-milestone when trigger conditions match the
...
status: dormant
planted: {ISO date}
planted_during: {current milestone/phase from STATE.md, or "unknown" if not in a CSP project}
...
_To be filled in. Run `/csp-capture --seed --enrich SEED-{PADDED}` to add context._

## When to Surface

**Trigger:** when relevant

This seed will surface during `/csp-new-milestone` when the milestone scope matches.

## Scope Estimate

**Unknown** — run `/csp-capture --seed --enrich SEED-{PADDED}` to estimate effort.
...
This seed will surface automatically when you run /csp-new-milestone.
```
</step>
...
Commit the update:
```bash
csp-sdk query commit "docs: enrich seed ${SEED_ID} — trigger + why + scope" --files "$SEED_FILE"
```
