...
Debug files use the `.csp/planning/debug/` path (hidden directory with leading dot).
</paths>
...
```bash
AGENT_SKILLS_DEBUGGER=$(csp-sdk query agent-skills csp-debugger)
...
- truth: "Comment appears immediately after submission"
  ...
  debug_session: .csp/planning/debug/comment-not-refreshing.md
```
...
```bash
csp-sdk query commit "docs({phase_num}): add root causes from diagnosis" --files ".csp/planning/phases/XX-name/{phase_num}-UAT.md"
```
</step>
