...
[R] Resolve — stop and fix items, then re-run /csp-complete-milestone
[A] Acknowledge all — document as deferred and proceed with close
[C] Cancel — exit without closing
```

If user chooses [A] (Acknowledge):
1. Re-run `csp-sdk query audit-open --json` to get structured data
...
2. **Run audit first** — `/csp-audit-milestone` to assess gap severity
...
**Note:** MILESTONES.md entry is now created automatically by `csp-sdk query milestone.complete` in the archive_milestone step. The entry includes version, date, phase/plan/task counts, and accomplishments extracted from SUMMARY.md files.
...
**Delegate archival to `csp-sdk query milestone.complete`:**

```bash
ARCHIVE=$(csp-sdk query milestone.complete "v[X.Y]" --name "[Milestone Name]")
```
...
**If doesn't exist:** Create from template at `~/.claude/code-skills-package/csp-workflow/templates/retrospective.md`.
...
```bash
INIT=$(csp-sdk query init.execute-phase "1")
...
<config-check>
Read `git.create_tag` via `csp-sdk query config-get git.create_tag 2>/dev/null || echo "true"`.
...
- [ ] RETROCSPECTIVE.md updated with milestone section
- [ ] Cross-milestone trends updated
- [ ] User knows next step (/csp-new-milestone)
