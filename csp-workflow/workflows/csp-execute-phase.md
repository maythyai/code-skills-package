...
- **Claude Code:** Uses `Agent(subagent_type="csp-executor", ...)` — blocks until complete, returns result
...
@~/.claude/code-skills-package/csp-workflow/references/context-budget.md
@~/.claude/code-skills-package/csp-workflow/references/gates.md
</required_reading>

<available_agent_types>
These are the valid CSP subagent types registered in .claude/agents/ (or equivalent for your runtime).
Always use the exact name from this list — do not fall back to 'general-purpose' or other built-in types:

- csp-executor — Executes plan tasks, commits, creates SUMMARY.md
- csp-verifier — Verifies phase completion, checks quality gates
...
- csp-debugger — Diagnoses and fixes issues
- csp-codebase-mapper — Maps project structure and dependencies
- csp-integration-checker — Checks cross-phase integration
- csp-nyquist-auditor — Validates verification coverage
- csp-ui-researcher — Researches UI/UX approaches
- csp-ui-checker — Reviews UI implementation quality
- csp-ui-auditor — Audits UI against design requirements
</available_agent_types>
...
```bash
INIT=$(csp-sdk query init.execute-phase "${PHASE_ARG}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS=$(csp-sdk query agent-skills csp-executor)
```
...
```bash
RUNTIME=$(csp-sdk query config-get runtime --default claude 2>/dev/null || echo "claude")
USE_WORKTREES=$(csp-sdk query config-get workflow.use_worktrees 2>/dev/null || echo "true")
EXECUTOR_STALL_INTERVAL_MINUTES=$(csp-sdk query config-get executor.stall_detect_interval_minutes 2>/dev/null || echo "5")
EXECUTOR_STALL_THRESHOLD_MINUTES=$(csp-sdk query config-get executor.stall_threshold_minutes 2>/dev/null || echo "10")
...
[ "$USE_WORKTREES" != "false" ] && csp-sdk query worktree.reap-orphans 2>/dev/null || true
```
...
- Executor agents omit extended deviation rule examples and checkpoint examples from inline prompt — load on-demand via @~/.claude/code-skills-package/csp-workflow/references/executor-examples.md
- Planner agents omit extended anti-pattern lists and specificity examples from inline prompt — load on-demand via @~/.claude/code-skills-package/csp-workflow/references/planner-antipatterns.md
...
Check if the current runtime is Copilot by testing for the `@csp-executor` agent pattern
...
MVP_MODE=$(csp-sdk query phase.mvp-mode "${PHASE_NUMBER}" $MVP_FLAG_ARG --pick active)
...
if [ "$MVP_MODE" = "true" ] && [ "$TDD_MODE" = "true" ]; then
  IS_BEHAVIOR_ADDING=$(csp-sdk query task.is-behavior-adding "$TASK_FILE" --pick is_behavior_adding)
  if [ "$IS_BEHAVIOR_ADDING" = "true" ]; then
    ...
    if [ -z "$RED_COMMIT" ]; then
      csp-sdk query state.update last_gate_trip "${PLAN_ID}/${TASK_ID}" || true
...
2. For each plan (sequentially, ignoring wave grouping):
   ...
   c. **If "Execute":** Read and follow `~/.claude/code-skills-package/csp-workflow/workflows/execute-plan.md` **inline**
...
- Maintains CSP's planning/tracking structure
- Best for: small phases, bug fixes, verification gaps, learning CSP
...
```bash
csp-sdk query state.begin-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
PLAN_INDEX=$(csp-sdk query phase-plan-index "${PHASE_NUMBER}")
```
...
```bash
CROSS_AI_ENABLED=$(csp-sdk query config-get workflow.cross_ai_execution 2>/dev/null || echo "false")
CROSS_AI_CMD=$(csp-sdk query config-get workflow.cross_ai_command 2>/dev/null || echo "")
CROSS_AI_TIMEOUT=$(csp-sdk query config-get workflow.cross_ai_timeout 2>/dev/null || echo "300")
```
...
`workflow.cross_ai_command` via `csp-sdk query config-set workflow.cross_ai_command "<command>"`.
...
tooling and `/csp-manager`'s background-completion handler can grep partial
   ...
   **Sequential dispatch for parallel execution (waves with 2+ agents):**
   ...
   Agent(
     ...
     prompt="
       ...
       ```
       Per-commit HEAD/cwd-drift/path-guard: `agents/csp-executor.md` steps 0/0a/0b + `references/worktree-path-safety.md` (in <execution_context>).
       </worktree_branch_check>
       ...
       <execution_context>
       @~/.claude/code-skills-package/csp-workflow/workflows/execute-plan.md
       @~/.claude/code-skills-package/csp-workflow/templates/summary.md
       @~/.claude/code-skills-package/csp-workflow/references/checkpoints.md
       @~/.claude/code-skills-package/csp-workflow/references/tdd.md
       @~/.claude/code-skills-package/csp-workflow/references/worktree-path-safety.md
       ${CONTEXT_WINDOW < 200000 ? '' : '@~/.claude/code-skills-package/csp-workflow/references/executor-examples.md'}
       </execution_context>
...
4. **Wait for all agents in wave to complete.**
   ...
   **Completion signal fallback (Copilot and runtimes where Agent() may not return):**
   ...
   COMMITS_SINCE_DICSPATCH=$(git log "${EXPECTED_BRANCH}" --since="${DICSPATCH_TS}" --oneline | head -1)
   ```
   ...
   **Configurable stall surveillance (#3212):** Every `${EXECUTOR_STALL_INTERVAL_MINUTES}`
   minutes while waiting, inspect `git log "${EXPECTED_BRANCH}" --since="${DICSPATCH_TS}"`
...
5. **Post-wave hook validation (parallel mode only):** Hooks run on every executor commit by default (#2924); this post-wave run only fires when `workflow.worktree_skip_hooks=true` opted out of per-commit hooks:
   ```bash
   SKIP_HOOKS=$(csp-sdk query config-get workflow.worktree_skip_hooks 2>/dev/null || echo "false")
   if [ "$SKIP_HOOKS" = "true" ]; then
     # Stash uncommitted changes under a named ref so we always pop (bare `git stash` strands them on hook/script failure). #3542: `refs/stash` is shared across worktrees, so this helper runs ONLY in the orchestrator's main checkout after all wave worktrees have been merged + removed; executors are forbidden from running any `git stash` subcommand (see `<destructive_git_prohibition>` in `agents/csp-executor.md`).
     STASHED=false
     if (! git diff --quiet || ! git diff --cached --quiet) && git stash push -u -m "csp-post-wave-hook-$$" >/dev/null 2>&1; then STASHED=true; fi
     git hook run pre-commit 2>&1 || echo "⚠ Pre-commit hooks failed — review before continuing"
     [ "$STASHED" = "true" ] && (git stash pop >/dev/null 2>&1 || echo "⚠ Could not pop csp-post-wave-hook stash — recover manually")
   fi
   ```
...
5.5. **Worktree cleanup (when `isolation="worktree"` was used):**
   ...
   **Manifest source of truth (#3384):** Cleanup consumes the `WAVE_WORKTREE_MANIFEST` created and populated during executor dispatch in step 3. Do not recreate or truncate it here.
   ...
   if command -v csp-sdk >/dev/null 2>&1; then
     csp-sdk query worktree.cleanup-wave --manifest "$WAVE_WORKTREE_MANIFEST" || exit 1
   else
     echo "WARN: csp-sdk unavailable; using manifest-scoped shell fallback (#3384)." >&2
   WT_PATHS_FILE=$(mktemp "${TMPDIR:-/tmp}/csp-worktree-paths-XXXXXX")
...
5.6. **Post-merge build & test gate:**
   ...
   Read and execute `code-skills-package/workflows/execute-phase/steps/post-merge-gate.md`.
   ...
   **Only update tracking when tests passed (TEST_EXIT=0).**
   ...
   if [ "${TEST_EXIT}" -eq 0 ]; then
     ...
     for plan_id in {completed_plan_ids}; do
       csp-sdk query roadmap.update-plan-progress "${PHASE_NUMBER}" "${plan_id}" "complete"
     done
...
7. **Handle failures:**
   **Step 7.0 — classify before branching (#3095):**
   ```bash
   CLASS_JSON=$(csp-sdk query agent.classify-failure -- "$AGENT_RETURN_BODY")
   ...
   **Step 7.1 — `class == "quota-exceeded"`:**
   ...
   ```
   Re-run `/csp-execute-phase` after quota reset for Option 1.
...
7b. **Pre-wave dependency check (waves 2+ only):**
    Before wave N+1, run `csp-sdk query verify.key-links {phase_dir}/{plan}-PLAN.md` for each upcoming plan.
...
```bash
AUTO_MODE=$(csp-sdk query check auto-mode --pick active 2>/dev/null || echo "false")
```
...
Resolve and re-run /csp execute-phase, or override with
/csp execute-phase {phase} --force-mvp-gate to ship anyway.
```
...
```bash
POST_PLAN_INDEX=$(csp-sdk query phase-plan-index "${PHASE_NUMBER}")
```
...
/csp-execute-phase {phase} ${CSP_WS}                # Continue remaining waves
/csp-execute-phase {phase} --wave {next} ${CSP_WS}  # Run the next wave explicitly
```
...
**Config gate:**
```bash
CODE_REVIEW_ENABLED=$(csp-sdk query config-get workflow.code_review 2>/dev/null || echo "true")
```
...
**Invoke review:**
```
Skill(skill="csp-code-review", args="${PHASE_NUMBER}")
```
...
/csp-code-review ${PHASE_NUMBER} --fix
```
...
```bash
PARENT_INFO=$(csp-sdk query find-phase "${PARENT_PHASE}" --raw)
...
REG_TEST_CMD=$(csp-sdk query config-get workflow.test_command --default "" 2>/dev/null || true)
...
```bash
SCHEMA_DRIFT=$(csp-sdk query verify.schema-drift "${PHASE_NUMBER}" 2>/dev/null)
```
...
Check for override:
```bash
SKIP_SCHEMA=$(echo "${CSP_SKIP_SCHEMA_CHECK:-false}")
```
...
Display:
```
⚠ Schema drift detected but CSP_SKIP_SCHEMA_CHECK=true — bypassing gate.
...
2. Skip schema check (CSP_SKIP_SCHEMA_CHECK=true) — bypass this gate
...
`code-skills-package/workflows/execute-phase/steps/codebase-drift-gate.md` —
...
```bash
VERIFIER_SKILLS=$(csp-sdk query agent-skills csp-verifier)
```
...
| `gaps_found` | Present gap summary, offer `/csp-plan-phase {phase} --gaps ${CSP_WS}` |
...
Commit the file:
```bash
csp-sdk query commit "test({phase_num}): persist human verification items as UAT" --files "{phase_dir}/{phase_num}-HUMAN-UAT.md"
```
...
Items saved to `{phase_num}-HUMAN-UAT.md` — they will appear in `/csp-progress` and `/csp-audit-uat`.

"approved" → continue | Report issues → gap closure
```

**If user says "approved":** Proceed to `update_roadmap`. The HUMAN-UAT.md file persists with `status: partial` and will surface in future progress checks until the user runs `/csp-verify-work` on it.
...
`/csp-plan-phase {X} --gaps ${CSP_WS}`

Also: `cat {phase_dir}/{phase_num}-VERIFICATION.md` — full report
Also: `/csp-verify-work {X} ${CSP_WS}` — manual testing first
```

Gap closure cycle: `/csp-plan-phase {X} --gaps ${CSP_WS}` reads VERIFICATION.md → creates gap plans with `gap_closure: true` → user runs `/csp-execute-phase {X} --gaps-only ${CSP_WS}` → verifier re-runs.
</step>
...
```bash
COMPLETION=$(csp-sdk query phase.complete "${PHASE_NUMBER}")
```
...
These items are tracked and will appear in `/csp-progress` and `/csp-audit-uat`.
```
...
entries from the completed phase to the global learnings store at `~/.csp/knowledge/`.

**Check config gate:**
```bash
GL_ENABLED=$(csp-sdk query config-get features.global_learnings --raw 2>/dev/null || echo "false")
```
...
```bash
csp-sdk query learnings.copy 2>/dev/null || echo "⚠ Learnings copy failed — continuing"
```
...
**Exception:** If `gaps_found`, the `verify_phase_goal` step already presents the gap-closure path (`/csp-plan-phase {X} --gaps`). No additional routing needed — skip auto-advance.
...
Read and follow `~/.claude/code-skills-package/csp-workflow/workflows/transition.md`, passing through the `--auto` flag so it propagates to the next phase invocation.

**If neither `--auto` nor `AUTO_MODE` is true:**

**STOP. Do not auto-advance. Do not execute transition. Do not plan next phase. Present options to the user and wait.**

**IMPORTANT: There is NO `/csp-transition` command. Never suggest it. The transition workflow is internal only.**
...
/csp-progress ${CSP_WS} — see updated roadmap
/csp-discuss-phase {next} ${CSP_WS} — start here: discuss next phase before planning  ← recommended
/csp-plan-phase {next} ${CSP_WS} — plan next phase (skip discuss)
/csp-execute-phase {next} ${CSP_WS} — execute next phase (skip discuss and plan)
```
...
/csp-progress ${CSP_WS} — see updated roadmap
/csp-plan-phase {next} ${CSP_WS} — start here: plan next phase (CONTEXT.md already present)  ← recommended
/csp-discuss-phase {next} ${CSP_WS} — re-discuss next phase
/csp-execute-phase {next} ${CSP_WS} — execute next phase (skip planning)
```
...
<failure_handling>
- **Quota / rate-limit (any runtime — #3095):** Agent return body contains a sentinel like `usage limit`, `rate limit`, `429`, `too many requests`, `RESOURCE_EXHAUSTED`, `usage_limit_reached`. Route via `csp-sdk query agent.classify-failure` → `class: "quota-exceeded"`. Do not offer retry-now; the right action is wait-for-reset and resume.
- **classifyHandoffIfNeeded false failure:** Agent reports "failed" but error is `classifyHandoffIfNeeded is not defined` → Claude Code bug, not CSP. Spot-check (SUMMARY exists, commits present) → if pass, treat as success
...
<resumption>
Re-run `/csp-execute-phase {phase}` → discover_plans finds completed SUMMARYs → skips them → resumes from first incomplete plan → continues wave execution.
