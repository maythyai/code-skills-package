...
Examples:
  `/csp-edit-phase 5`       → phase = 5, force = false
  `/csp-edit-phase 5 --force` → phase = 5, force = true
  `/csp-edit-phase 12.1`    → phase = 12.1, force = false

If no argument provided:

```
ERROR: Phase number required
Usage: /csp-edit-phase <phase-number> [--force]
Example: /csp-edit-phase 5
Example: /csp-edit-phase 5 --force
```
...
```bash
PHASE_DATA=$(csp-sdk query roadmap get-phase "${target}")
```
...
Available phases can be seen with /csp-progress.
```
...
```bash
ALL_PHASES=$(csp-sdk query roadmap analyze)
```
...
- `/csp-progress` — view updated roadmap
- `/csp-plan-phase {target}` — re-plan this phase (if needed)
- `/csp-discuss-phase {target}` — discuss implementation approach
