<purpose>
Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates csp-phase-researcher, csp-planner, and csp-plan-checker agents with a revision loop (max 3 iterations).
</purpose>
...
@~/.claude/code-skills-package/csp-workflow/references/revision-loop.md
@~/.claude/code-skills-package/csp-workflow/references/gate-prompts.md
@~/.claude/code-skills-package/csp-workflow/references/agent-contracts.md
@~/.claude/code-skills-package/csp-workflow/references/gates.md
</required_reading>

<available_agent_types>
Valid CSP subagent types (use exact names — do not fall back to 'general-purpose'):
- csp-phase-researcher — Researches technical approaches for a phase
- csp-pattern-mapper — Analyzes codebase for existing patterns, produces PATTERNS.md
- csp-planner — Creates detailed plans from phase scope
- csp-plan-checker — Reviews plan quality before execution
</available_agent_types>
...
```bash
INIT=$(csp-sdk query init.plan-phase "$PHASE")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(csp-sdk query agent-skills csp-phase-researcher)
AGENT_SKILLS_PLANNER=$(csp-sdk query agent-skills csp-planner)
AGENT_SKILLS_CHECKER=$(csp-sdk query agent-skills csp-plan-checker)
CONTEXT_WINDOW=$(csp-sdk query config-get context_window 2>/dev/null || echo "200000")
TDD_MODE=$(csp-sdk query config-get workflow.tdd_mode 2>/dev/null || echo "false")
MVP_MODE_CFG=$(csp-sdk query config-get workflow.mvp_mode 2>/dev/null || echo "false")
```

When `TDD_MODE` is `true`, the planner agent is instructed to apply `type: tdd` to eligible tasks using heuristics from `references/tdd.md`. The planner's `<required_reading>` is extended to include `@~/.claude/code-skills-package/csp-workflow/references/tdd.md` so gate enforcement rules are available during planning.
...
**If `planning_exists` is false:** Error — run `/csp-new-project` first.
...
/csp-plan-phase --reviews cannot replan a closed phase. If the review surfaced
...
with: /csp-plan-phase ${phase_number} --force
...
if [ "$MVP_MODE" = "true" ] && [ "$padded_phase" = "01" ]; then
  PRIOR_SUMMARIES=$(csp-sdk query phases.list --pick summaries_total 2>/dev/null || echo "0")
  if [ "$PRIOR_SUMMARIES" = "0" ]; then WALKING_SKELETON=true; fi
fi
```

When `WALKING_SKELETON=true`:
- Planner is instructed to produce `SKELETON.md` in the phase directory alongside `PLAN.md`. The template lives at `@~/.claude/code-skills-package/csp-workflow/references/skeleton-template.md`.
...
```bash
CHUNKED_CFG=$(csp-sdk query config-get workflow.plan_chunked 2>/dev/null || echo "false")
CHUNKED_MODE=false
...
Then re-run /csp-plan-phase {N} --reviews
```
Exit workflow.
...
```bash
PHASE_INFO=$(csp-sdk query roadmap.get-phase "${PHASE}")
```
...
```bash
MVP_MODE=$(csp-sdk query phase.mvp-mode "${PHASE}" $MVP_FLAG_ARG --pick active)
```
...
5. Commit:
```bash
csp-sdk query commit "docs(${padded_phase}): generate context from PRD" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
1. Display banner: `CSP ► ADR Ingest Express Path` with `{INGEST_PATH}` and `{INGEST_FORMAT}`.
2. Parse each resolved ADR through `code-skills-package/bin/lib/adr-parser.cjs` (`--input`, `--format`) and collect normalized records.
3. Status gate: reject `superseded`/`rejected`/`deprecated`; warn on `proposed`; missing status defaults to `accepted`.
4. Empty-decisions fallback: if all parsed ADRs have zero `decisions[]`, emit `ADR ingest produced no locked decisions; fall back to discuss-phase for this phase.` and exit with `/csp-discuss-phase {N}` guidance.
...
6. Commit with `csp-sdk query commit "docs(${padded_phase}): generate context from ADR ingest" --files "${phase_dir}/${padded_phase}-CONTEXT.md"` and set `context_content`; continue to step 5.
...
If "Run discuss-phase first":
  **IMPORTANT:** Do NOT invoke discuss-phase as a nested Skill/Task call — AskUserQuestion
  ...
  ```
  Run this command first, then re-run /csp-plan-phase {X} ${CSP_WS}:

  /csp-discuss-phase {X} ${CSP_WS}
  ```
...
AI_PHASE_CFG=$(csp-sdk query config-get workflow.ai_integration_phase 2>/dev/null || echo "true")
```

**Skip if `AI_PHASE_CFG` is `false`.**

**If `AI_CSPEC_FILE` is empty:** Check phase goal for AI keywords:
```bash
...
**If AI keywords detected AND no AI-CSPEC.md:**
```
◆ Note: This phase appears to involve AI system development.
  Consider running /csp-ai-integration-phase {N} before planning to:
  - Select the right framework for your use case
  - Research its docs and best practices
  - Design an evaluation strategy

  Continue planning without AI-CSPEC? (non-blocking — /csp-ai-integration-phase can be run after)
```

Use AskUserQuestion with options:
- "Continue — plan without AI-CSPEC"
- "Stop — I'll run /csp-ai-integration-phase {N} first"

If "Stop": Exit with `/csp-ai-integration-phase {N}` reminder.
If "Continue": Proceed. (Non-blocking — planner will note AI-CSPEC is absent.)

**If `AI_CSPEC_FILE` is non-empty:** Extract framework for planner context:
```bash
FRAMEWORK_LINE=$(grep "Selected Framework:" "${AI_CSPEC_FILE}" | head -1)
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► RESEARCHING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

◆ Spawning researcher...
```

### Spawn csp-phase-researcher

```bash
PHASE_DESC=$(csp-sdk query roadmap.get-phase "${PHASE}" --pick section)
```
...
<files_to_read>
- {context_path} (USER DECISIONS from /csp-discuss-phase)
...
Agent(
  ...
  subagent_type="csp-phase-researcher",
...
**If `RESEARCH_ONLY=true`:** the user invoked `/csp-plan-phase --research-phase <N>` for research-only mode. Do **not** continue to Section 5.5+ (validation strategy, planner, plan-checker, verification, gaps, bounce, post-planning-gaps). Print the research-complete summary and exit cleanly:
...
Re-run /csp-plan-phase ${PHASE} to plan the phase using this research,
or /csp-plan-phase ${PHASE} --research to refresh research and plan.
```
...
**If found:**
1. Read template: `~/.claude/code-skills-package/csp-workflow/templates/VALIDATION.md`
...
```bash
SECURITY_CFG=$(csp-sdk query config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
SECURITY_ASVS=$(csp-sdk query config-get workflow.security_asvs_level --raw 2>/dev/null || echo "1")
SECURITY_BLOCK=$(csp-sdk query config-get workflow.security_block_on --raw 2>/dev/null || echo "high")
```
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► SECURITY THREAT MODEL REQUIRED (ASVS L{SECURITY_ASVS})
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
UI_GATE_CFG=$(csp-sdk query config-get workflow.ui_safety_gate 2>/dev/null || echo "true")
```
...
```bash
PHASE_SECTION=$(csp-sdk query roadmap.get-phase "${PHASE}" 2>/dev/null)
...
CSP_REPO_ROOT=$(git rev-parse --show-toplevel 2>/dev/null || echo ".")
printf '%s' "$PHASE_SECTION" | node "${CSP_REPO_ROOT}/bin/lib/ui-safety-gate.cjs" > /dev/null 2>&1
HAS_UI=$?
```

**If `HAS_UI` is 0 (frontend indicators found):**

Check for existing UI-CSPEC:
```bash
UI_CSPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-CSPEC.md 2>/dev/null | head -1)
```

**If UI-CSPEC.md found:** Set `UI_CSPEC_PATH=$UI_CSPEC_FILE`. Display: `Using UI design contract: ${UI_CSPEC_PATH}`

**If UI-CSPEC.md missing AND `--skip-ui` flag is present in $ARGUMENTS:** Skip silently to step 6.

**If UI-CSPEC.md missing AND `UI_GATE_CFG` is `true`:**

Read ephemeral chain flag (same field as `check.auto-mode` → `auto_chain_active`):
```bash
AUTO_CHAIN=$(csp-sdk query check auto-mode --pick auto_chain_active 2>/dev/null || echo "false")
```

**If `AUTO_CHAIN` is `true` (running inside a `--chain` or `--auto` pipeline):**

Auto-generate UI-CSPEC without prompting:
```
Skill(skill="csp-ui-phase", args="${PHASE} --auto ${CSP_WS}")
```
After `csp-ui-phase` returns, re-read:
```bash
UI_CSPEC_FILE=$(ls "${PHASE_DIR}"/*-UI-CSPEC.md 2>/dev/null | head -1)
UI_CSPEC_PATH="${UI_CSPEC_FILE}"
```
Continue to step 6.
...
```
## ⚠ UI-CSPEC.md missing for Phase {N}
▶ Recommended next step:
`/csp-ui-phase {N} ${CSP_WS}` — generate UI design contract before planning
───────────────────────────────────────────────
Also available:
- `/csp-plan-phase {N} --skip-ui ${CSP_WS}` — plan without UI-CSPEC (not recommended for frontend phases)
```
...
```bash
PHASE_SECTION=$(csp-sdk query roadmap.get-phase "${PHASE}" --pick section 2>/dev/null)
```
...
```bash
_csp_field() { node -e "const o=JSON.parse(process.argv[1]); const v=o[process.argv[2]]; process.stdout.write(v==null?'':String(v))" "$1" "$2"; }
STATE_PATH=$(_csp_field "$INIT" state_path)
ROADMAP_PATH=$(_csp_field "$INIT" roadmap_path)
REQUIREMENTS_PATH=$(_csp_field "$INIT" requirements_path)
RESEARCH_PATH=$(_csp_field "$INIT" research_path)
VERIFICATION_PATH=$(_csp_field "$INIT" verification_path)
UAT_PATH=$(_csp_field "$INIT" uat_path)
CONTEXT_PATH=$(_csp_field "$INIT" context_path)
REVIEWS_PATH=$(_csp_field "$INIT" reviews_path)
PATTERNS_PATH=$(_csp_field "$INIT" patterns_path)

# Detect spike/sketch findings skills (project-local)
CSPIKE_FINDINGS_PATH=$(ls ./.claude/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
1. Re-run: `/csp-plan-phase {PHASE} --research ${CSP_WS}`
2. Disable Nyquist with the exact command:
   `csp-sdk query config-set workflow.nyquist_validation false`
3. Continue anyway (plans fail Dimension 8)

Proceed to Step 7.8 (or Step 8 if pattern mapper is disabled) only if user selects 2 or 3.

## 7.8. Spawn csp-pattern-mapper Agent (Optional)
...
Check config:
```bash
PATTERN_MAPPER_CFG=$(csp-sdk query config-get workflow.pattern_mapper 2>/dev/null || echo "true")
```
...
Agent(
  prompt="{above}",
  subagent_type="csp-pattern-mapper",
  model="{researcher_model}",
)
```
...
## 8. Spawn csp-planner Agent

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► PLANNING PHASE {X}
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
- {UI_CSPEC_PATH} (UI Design Contract — visual/interaction specs, if exists)
- {CSPIKE_FINDINGS_PATH} (Spike Findings — validated patterns, constraints, landmines from experiments, if exists)
...
${TDD_MODE === 'true' ? `
<tdd_mode_active>
**TDD Mode is ENABLED.** Apply TDD heuristics from @~/.claude/code-skills-package/csp-workflow/references/tdd.md to all eligible tasks:
...
**MVP_MODE:** ${MVP_MODE} (when true, follow vertical-slice rules from `@~/.claude/code-skills-package/csp-workflow/references/planner-mvp-mode.md`; when false, ignore MVP guidance entirely.)
...
${MVP_MODE === 'true' ? `
<mvp_mode_active>
**MVP Mode is ENABLED.** Follow vertical-slice planning rules from @~/.claude/code-skills-package/csp-workflow/references/planner-mvp-mode.md. Each plan must deliver a complete vertical slice — thin end-to-end functionality rather than horizontal layers.
</mvp_mode_active>
` : ''}
</planning_context>

<downstream_consumer>
Output consumed by /csp-execute-phase. Plans need:
...
`/csp-plan-phase {N} --chunked` resumes from the last successfully committed plan.

**Intended for new or in-progress chunked runs.** To recover plans already written by a prior
*non-chunked* run, use step 6's "Add more plans" or proceed directly to `/csp-execute-phase`
...
5. **Commit per-plan:**
   ```bash
   csp-sdk query commit "docs(${PADDED_PHASE}): plan ${plan_id} (chunked)" --files "${PHASE_DIR}/${plan_id}-PLAN.md"
   ```
...
- **`## PHASE CSPLIT RECOMMENDED`:** The planner determined the phase exceeds the context budget for full-fidelity implementation of all source items. Handle in step 9b.
...
**Triggered when:** Agent() returns but the return contains no recognized marker (`## PLANNING COMPLETE`, `## PHASE CSPLIT RECOMMENDED`, `## ⚠ Source Audit`, `## CHECKPOINT REACHED`, `## PLANNING INCONCLUSIVE`).
...
3. **Stop** — exit; user can re-run `/csp-plan-phase {N}` to resume
...
When the planner returns `## PHASE CSPLIT RECOMMENDED`, it means the phase's source items exceed the context budget for full-fidelity implementation. The planner proposes groupings.
...
**If "Split":** Use `/csp-phase --insert` to create the sub-phases, then replan each.
...
**Stall detection:** If `issue_count >= prev_issue_count`:
  ...
  **If `stall_reentry_count >= 2`:**
    ...
    Suggest: "Consider resolving these issues manually or running `/csp-debug` to investigate root causes."
...
**Read pass count:**
```bash
BOUNCE_PASSES=$(csp-sdk query config-get workflow.plan_bounce_passes 2>/dev/null || echo "2")
BOUNCE_SCRIPT=$(csp-sdk query config-get workflow.plan_bounce_script 2>/dev/null | jq -r '.' 2>/dev/null || true)
```

Display banner:
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP ► BOUNCING PLANS (External Refinement)
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
5. **Re-run plan checker on bounced plans:** Spawn csp-plan-checker (same as step 10) on all modified plans. If a bounced plan fails the checker, restore original from its pre-bounce.md backup:
```
...
```bash
csp-sdk query commit "refactor(${padded_phase}): bounce plans through external refinement" --files "${PHASE_DIR}/*-PLAN.md"
```
...
```bash
GATE_CFG=$(csp-sdk query config-get workflow.context_coverage_gate 2>/dev/null || echo "true")
if [ "$GATE_CFG" != "false" ]; then
  GATE_RESULT=$(csp-sdk query check.decision-coverage-plan "${PHASE_DIR}" "${CONTEXT_PATH}")
...
```bash
csp-sdk query state.planned-phase --phase "${PHASE_NUMBER}" --name "${PHASE_NAME}" --plans "${PLAN_COUNT}"
```
...
```bash
csp-sdk query roadmap.annotate-dependencies "${PHASE_NUMBER}"
```
...
```bash
POST_PLANNING_GAPS=$(csp-sdk query config-get workflow.post_planning_gaps --default true 2>/dev/null || echo true)
if [ "$POST_PLANNING_GAPS" = "true" ]; then
  node "$HOME/.claude/code-skills-package/bin/csp-tools.cjs" gap-analysis --phase-dir "${PHASE_DIR}"
fi
```
...
re-run `/csp-plan-phase --gaps` to add plans, or proceed to execute-phase as-is.
...
3. **Sync chain flag with intent** — if user invoked manually (no `--auto` and no `--chain`), clear the ephemeral chain flag from any previous interrupted `--auto` chain. This does NOT touch `workflow.auto_advance` (the user's persistent settings preference):
   ...
   if [[ ! "$ARGUMENTS" =~ --auto ]] && [[ ! "$ARGUMENTS" =~ --chain ]]; then
     csp-sdk query config-set workflow._auto_chain_active false || true
   fi
   ```
...
if ([[ "$ARGUMENTS" =~ --auto ]] || [[ "$ARGUMENTS" =~ --chain ]]) && [[ "$AUTO_CHAIN" != "true" ]]; then
  csp-sdk query config-set workflow._auto_chain_active true
fi
```
...
```
Skill(skill="csp-execute-phase", args="${PHASE} --auto --no-transition ${CSP_WS}")
```
...
- **PHASE COMPLETE** → Display final summary:
  ```
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
   CSP ► PHASE ${PHASE} COMPLETE ✓
  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

  Auto-advance pipeline finished.

  Next: /csp-discuss-phase ${NEXT_PHASE} --auto ${CSP_WS}
  ```
- **GAPS FOUND / VERIFICATION FAILED** → Display result, stop chain:
  ...
  /csp-execute-phase ${PHASE} ${CSP_WS}
  ```
...
5. **Retry:** Restart Claude Code and run `/csp-plan-phase` again

If freezes persist, try `--skip-research` to reduce the agent chain from 3 to 2 agents:
```
/csp-plan-phase N --skip-research
```
</windows_troubleshooting>
...
- [ ] csp-phase-researcher spawned with CONTEXT.md
- [ ] Existing plans checked
- [ ] csp-planner spawned with CONTEXT.md + RESEARCH.md
- [ ] Plans created (PLANNING COMPLETE or CHECKPOINT handled)
- [ ] csp-plan-checker spawned with CONTEXT.md
