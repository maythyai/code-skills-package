...
<required_reading>
@~/.claude/code-skills-package/csp-workflow/references/domain-probes.md
@~/.claude/code-skills-package/csp-workflow/references/gate-prompts.md
@~/.claude/code-skills-package/csp-workflow/references/universal-anti-patterns.md
</required_reading>
...
1. **csp-phase-researcher** — Reads CONTEXT.md to know WHAT to research
2. **csp-planner** — Reads CONTEXT.md to know WHAT decisions are locked
...
**Express path available:** If you already have a PRD or acceptance criteria document, use `/csp-plan-phase {phase} --prd path/to/prd.md` to skip this discussion and go straight to planning.

<step name="initialize" priority="first">
Phase number from argument (required).

```bash
INIT=$(csp-sdk query init.phase-op "${PHASE}")
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_ADVISOR=$(csp-sdk query agent-skills csp-advisor-researcher)
```
...
Use /csp-progress ${CSP_WS} to see available phases.
```
Exit workflow.
...
if [ -f "$HOME/.claude/code-skills-package/USER-PROFILE.md" ]; then
  ADVISOR_MODE=true
else
  ADVISOR_MODE=false
fi
...
Check if a CSPEC.md (from `/csp-spec-phase`) exists for this phase. CSPEC.md locks requirements before implementation decisions.

```bash
ls ${phase_dir}/*-CSPEC.md 2>/dev/null | grep -v AI-CSPEC | head -1 || true
```

**If CSPEC.md is found:**
1. Read the CSPEC.md file.
2. Count requirements (numbered items in `## Requirements`).
3. Display: `Found CSPEC.md — {N} requirements locked. Focusing on implementation decisions.`
...
**If no CSPEC.md is found:** Continue with `spec_loaded = false`.

**Note:** CSPEC.md files named `AI-CSPEC.md` (from `/csp-ai-integration-phase`) are excluded — different purpose.
</step>
...
Read at most **3** prior CONTEXT.md files (most recent 3 phases before current). If `.csp/planning/DECISIONS-INDEX.md` exists, read that instead — it is a bounded rolling summary that supersedes per-phase reads.

```bash
(find .csp/planning/phases -name "*-CONTEXT.md" 2>/dev/null || true) | sort -r
```
...
```bash
CSPIKE_FINDINGS=$(ls ./.claude/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
SKETCH_FINDINGS=$(ls ./.claude/skills/sketch-findings-*/SKILL.md 2>/dev/null | head -1 || true)
RAW_CSPIKES=$(ls .csp/planning/spikes/MANIFEST.md 2>/dev/null)
RAW_SKETCHES=$(ls .csp/planning/sketches/MANIFEST.md 2>/dev/null)
```
...
If raw spikes/sketches exist but no findings skill, note: `⚠ Unpackaged spikes/sketches detected — run /csp-spike --wrap-up or /csp-sketch --wrap-up to make findings available.`
...
```bash
TODO_MATCHES=$(csp-sdk query todo.match-phase "${PHASE_NUMBER}")
```
...
Read `@~/.claude/code-skills-package/csp-workflow/references/scout-codebase.md` — it contains the phase-type→map selection table, single-read rule, no-maps fallback, and `<codebase_context>` output schema. Then execute:
1. `ls .csp/planning/codebase/*.md` to find existing maps
...
2b. **CSPEC.md awareness** — If `spec_loaded = true`: `<locked_requirements>` are pre-answered (Goal, Boundaries, Constraints, Acceptance Criteria). Do NOT generate gray areas about WHAT to build or WHY. Only generate gray areas about HOW to implement. When presenting, include: "Requirements are locked by CSPEC.md — discussing implementation decisions only."
...
**CSPEC.md integration** — If `spec_loaded = true`:
- Add the `<spec_lock>` section immediately after `<domain>`.
- Add the CSPEC.md file to `<canonical_refs>` with note "Locked requirements — MUST read before planning".
- Do NOT duplicate requirements text from CSPEC.md into `<decisions>` — agents read CSPEC.md directly.
...
`/csp-plan-phase ${PHASE} ${CSP_WS}`

---

**Also available:** `--chain` for auto plan+execute after; `/csp-plan-phase ${PHASE} --skip-research ${CSP_WS}` to plan without research; `/csp-ui-phase ${PHASE} ${CSP_WS}` for UI design contracts; review/edit CONTEXT.md before continuing.
```
</step>
...
```bash
csp-sdk query commit "docs(${padded_phase}): capture phase context" --files "${phase_dir}/${padded_phase}-CONTEXT.md" "${phase_dir}/${padded_phase}-DISCUSSION-LOG.md"
```
