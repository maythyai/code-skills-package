...
- **--from**: Import external plan → conflict detection → write PLAN.md → validate via csp-plan-checker
...
- If `--prd` is present: display message that `--prd` is not yet implemented and exit:
  ```
  CSP > --prd mode is planned for a future release. Use --from to import plan files.
  ```
- If neither flag is found: display usage and exit:

```
Usage: /csp-import --from <path>

  --from <path>   Import an external plan file into CSP format
```
...
- **CSP PLAN.md format**: Has YAML frontmatter with `phase:`, `plan:`, `type:` fields
...
Convert the imported content to CSP PLAN.md format.
...
If the imported plan references PBR plan naming (e.g., `PLAN-01.md`, `plan-01.md`), rename all references to CSP `{NN}-{MM}-PLAN.md` convention during conversion.

Apply CSP naming convention for the output filename:
...
Delegate validation to csp-plan-checker:
