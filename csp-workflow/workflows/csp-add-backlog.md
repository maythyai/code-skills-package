...
Invoked by `/csp-capture --backlog` (`commands/csp/capture.md`).
...
```bash
NEXT=$(csp-sdk query phase.next-decimal 999 --raw)
```
...
Plans:
- [ ] TBD (promote with /csp-review-backlog when ready)
```
...
Use /csp-discuss-phase {NEXT} to explore it further.
Use /csp-review-backlog to promote items to active milestone.
```
...
- Phase directories are created immediately so /csp-discuss-phase and /csp-plan-phase work on them
- No `Depends on:` field — backlog items are unsequenced by definition
- Sparse numbering is fine (999.1, 999.3) — always uses next-decimal
- Promote backlog items to the active milestone with /csp-review-backlog
</notes>
