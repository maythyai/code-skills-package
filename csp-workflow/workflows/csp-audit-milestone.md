...
AGENT_SKILLS_CHECKER=$(csp-sdk query agent-skills csp-integration-checker)
```
...
```bash
integration_checker_model=$(csp-sdk query resolve-model csp-integration-checker --raw)
```
...
PHASE_INFO=$(csp-sdk query find-phase 01 --raw)
...
${AGENT_SKILLS_CHECKER}",
  subagent_type="csp-integration-checker",
...
for summary in .csp/planning/phases/*-*/*-SUMMARY.md; do
  [ -e "$summary" ] || continue
  csp-sdk query summary-extract "$summary" --fields requirements_completed --pick requirements_completed
done
```
...
```bash
NYQUIST_CONFIG=$(csp-sdk query config-get workflow.nyquist_validation --raw 2>/dev/null)
```
...
Discovery only — never auto-calls `/csp-validate-phase`.

## 6. Aggregate into v{version}-MILESTONE-AUDIT.md

Create `.csp/planning/v{version}-v{version}-MILESTONE-AUDIT.md` with:
...
**Report:** .csp/planning/v{version}-MILESTONE-AUDIT.md
...
|-------|---------------|-----------|--------|
| {phase} | exists/missing | true/false/partial | `/csp-validate-phase {N}` |

Phases needing validation: run `/csp-validate-phase {N}` for each flagged phase.
...
/csp-phase --insert <N> "Close gap: <REQ-ID> — <description>"
/csp-discuss-phase <N>
/csp-plan-phase <N>
/csp-execute-phase <N>

For Nyquist-coverage gaps flagged in the table above, prefer running
`/csp-validate-phase <N>` for each flagged phase (and `/csp-secure-phase
...
**Also available:**
- cat .csp/planning/v{version}-MILESTONE-AUDIT.md — see full report
- /csp-complete-milestone {version} — proceed anyway (accept tech debt)
...
/csp-phase --insert <N> "Address tech debt: <area>"
/csp-discuss-phase <N>
/csp-plan-phase <N>
/csp-execute-phase <N>
