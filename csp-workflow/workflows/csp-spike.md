...
Saves artifacts to `.csp/planning/spikes/`. Companion to `/csp-spike --wrap-up`.
...
If no `.csp/planning/spikes/` directory exists, tell the user there's nothing to analyze and offer to start fresh with an idea instead.
...
**c. CONVENTIONS.md** — read `.csp/planning/spikes/CONVENTIONS.md` if it exists. Established stack and patterns.

**d. All spike READMEs** — read `.csp/planning/spikes/*/README.md` for verdicts, results, investigation trails, and tags.
...
Present all integration and frontier candidates, then ask which to run. When the user picks spikes, write definitions into `.csp/planning/spikes/MANIFEST.md` (appending to existing table) and proceed directly to building them starting at `research`.
</step>

<step name="setup_directory">
Create `.csp/planning/spikes/` if it doesn't exist:

```bash
mkdir -p .csp/planning/spikes
```

Check for existing spikes to determine numbering:
```bash
ls -d .csp/planning/spikes/[0-9][0-9][0-9]-* 2>/dev/null | sort | tail -1
```
...
**Check conventions first.** If `.csp/planning/spikes/CONVENTIONS.md` exists, follow its stack and patterns — these represent validated choices the user expects to see continued.
...
If `.csp/planning/spikes/` has existing content, load context in this priority order:

**a. Conventions:** Read `.csp/planning/spikes/CONVENTIONS.md` if it exists.

**b. Findings skills:** Glob for `./.claude/skills/spike-findings-*/SKILL.md` and read any that exist, plus their `references/*.md` files.

**c. Manifest:** Read `.csp/planning/spikes/MANIFEST.md` for the index of all spikes.

**d. Related READMEs:** Based on the new idea, identify which prior spikes are related by matching tags, names, technologies, or domain overlap. Read only those `.csp/planning/spikes/*/README.md` files. Skip unrelated ones.
...
If no `.csp/planning/spikes/` exists, skip this step.
</step>
...
Create or update `.csp/planning/spikes/MANIFEST.md`:
...
Before starting each spike (not just the first), re-read `.csp/planning/spikes/MANIFEST.md` and `.csp/planning/spikes/CONVENTIONS.md` to prevent drift within long sessions. Check the Requirements section — make sure the spike doesn't contradict any established requirements.
</step>
...
**a.** Create `.csp/planning/spikes/NNN-descriptive-name/`
...
**h.** Update `.csp/planning/spikes/MANIFEST.md` with the spike's row.

**i.** Commit (if `COMMIT_DOCS` is true):
```bash
csp-sdk query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .csp/planning/spikes/NNN-descriptive-name/ .csp/planning/spikes/MANIFEST.md
```
...
After all spikes in this session are built, update `.csp/planning/spikes/CONVENTIONS.md` with patterns that emerged or solidified.
...
```bash
csp-sdk query commit "docs(spikes): update conventions" --files .csp/planning/spikes/CONVENTIONS.md
```
</step>
...
**Also available:**
- `/csp-spike` — spike more ideas (or run with no argument for frontier mode)
- `/csp-plan-phase` — start planning the real implementation
- `/csp-explore` — continue exploring the idea
...
<success_criteria>
- [ ] `.csp/planning/spikes/` created (auto-creates if needed, no project init required)
