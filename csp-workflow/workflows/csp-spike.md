...
Saves artifacts to `.planning/spikes/`. Companion to `/csp-spike --wrap-up`.
...
```bash
csp-sdk query commit "docs(spike-NNN): [VERDICT] — [key finding]" --files .planning/spikes/NNN-descriptive-name/ .planning/spikes/MANIFEST.md
```
...
```bash
csp-sdk query commit "docs(spikes): update conventions" --files .planning/spikes/CONVENTIONS.md
```
</step>
