<purpose>
Validate built features through conversational testing with persistent state. Creates UAT.md that tracks test progress, survives /clear, and feeds gaps into /csp-plan-phase --gaps.
...
<template>
@~/.claude/code-skills-package/csp-workflow/templates/UAT.md
</template>
...
```bash
CSP_WS=""
echo "$ARGUMENTS" | grep -qE -- '--ws[[:space:]]+[^[:space:]]+' && CSP_WS=$(echo "$ARGUMENTS" | grep -oE -- '--ws[[:space:]]+[^[:space:]]+')
PHASE_ARG=$(echo "$ARGUMENTS" | sed -E 's/--ws[[:space:]]+[^[:space:]]+//g' | xargs)

INIT=$(csp-sdk query init.verify-work "${PHASE_ARG}" ${CSP_WS})
...
MVP_MODE=$(csp-sdk query phase.mvp-mode "${phase_number}" ${CSP_WS} --pick active)
```
</step>
...
Provide a phase number to start testing (e.g., /csp-verify-work 4)
```
...
```
UI_PHASE_FLAG=$(csp-sdk query config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
respond to tool calls) AND (`UI_PHASE_FLAG` is `true` OR `UI_CSPEC_FILE` is non-empty):**

For each UI checkpoint listed in the phase's UI-CSPEC.md (or inferred from SUMMARY.md):
...
**MVP-mode UAT framing.** When `MVP_MODE=true`, follow the rules in `@~/.claude/code-skills-package/csp-workflow/references/verify-mvp-mode.md`. Briefly:
...
```bash
PHASE_GOAL=$(csp-sdk query roadmap.get-phase "${phase_number}" ${CSP_WS} --pick goal)
USER_STORY_VALID=$(csp-sdk query user-story.validate --story "$PHASE_GOAL" --pick valid)
if [ "$USER_STORY_VALID" != "true" ]; then
  echo "Phase ${phase_number} has '**Mode:** mvp' in ROADMAP.md but the **Goal:** is not in user-story format."
  echo "Run /csp mvp-phase ${phase_number} to set a user-story goal before verifying."
  exit 1
fi
```
...
```bash
CHECKPOINT=$(csp-sdk query uat.render-checkpoint --file "$uat_path" --raw)
...
```
⚠ Security enforcement enabled — /csp-secure-phase {phase} has not run.
Run before advancing to the next phase.

All tests passed. Ready to continue.

- `/csp-secure-phase {phase}` — security review (required before advancing)
- `/csp-plan-phase {next}` — Plan next phase
- `/csp-execute-phase {next}` — Execute next phase
- `/csp-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
...
Read and follow `~/.claude/code-skills-package/csp-workflow/workflows/transition.md`.
...
- `/csp-plan-phase {next}` — Plan next phase
- `/csp-execute-phase {next}` — Execute next phase
- `/csp-secure-phase {phase}` — security review
- `/csp-ui-review {phase}` — visual quality audit (if frontend files were modified)
```
</step>
...
`audit-open` is CJS-only until registered on `csp-sdk query`:

```bash
csp-sdk query audit-open --json
```
...
If user declines: stop. User resolves items and re-runs `/csp-verify-work`.
...
- Follow @~/.claude/code-skills-package/csp-workflow/workflows/diagnose-issues.md
...
Spawn csp-planner in --gaps mode:
...
<downstream_consumer>
Output consumed by /csp-execute-phase
...
Spawn csp-planner with revision context:
...
3. Abandon (exit, user runs /csp-plan-phase manually)
...
`/clear` then `/csp-execute-phase {phase} --gaps-only`
...
- [ ] If issues: csp-planner creates fix plans (gap_closure mode)
- [ ] If issues: csp-plan-checker verifies fix plans
- [ ] If issues: revision loop until plans pass (max 3 iterations)
- [ ] Ready for `/csp-execute-phase --gaps-only` when complete
</success_criteria>
