...
- csp-project-researcher — Researches project-level technical decisions
- csp-research-synthesizer — Synthesizes findings from parallel research agents
- csp-roadmapper — Creates phased execution roadmaps
</available_agent_types>
...
- File reference: `/csp-new-project --auto @prd.md`
...
Usage:
  /csp-new-project --auto @your-idea.md
  /csp-new-project --auto [paste or write your idea here]
...
```bash
INIT=$(csp-sdk query init.new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(csp-sdk query agent-skills csp-project-researcher)
AGENT_SKILLS_SYNTHESIZER=$(csp-sdk query agent-skills csp-research-synthesizer)
AGENT_SKILLS_ROADMAPPER=$(csp-sdk query agent-skills csp-roadmapper)
```
...
```text
⚠ CSP agents not installed. The following agents are missing from your agents directory:
...
Subagent spawns (csp-project-researcher, csp-research-synthesizer, csp-roadmapper) will fail
with "agent type not found" if `required_agents_installed` is false. Run the installer with --global to make agents available:

  npx code-skills-package-cc@latest --global
...
**If `project_exists` is true:** Error — project already initialized. Use `/csp-progress`.
...
- options:
  - "Map codebase first" — Run /csp-map-codebase to understand existing architecture (Recommended)
  - "Skip mapping" — Proceed with project initialization

**If "Map codebase first":**

```
Run `/csp-map-codebase` first, then return to `/csp-new-project`
```
...
Ask which optional PRD-style sections `/csp-ship` should append to generated PR bodies. These map to `ship.pr_body_sections`; selected sections are written with `"enabled": true`, unselected seeded sections are written with `"enabled": false` so the project can enable them later without editing `ship.md`.
...
AskUserQuestion([
  {
    header: "PR Body",
    question: "Which optional PRD-style sections should /csp-ship include in PR bodies?",
    multiSelect: true,
    options: [
...
CSPIKE_SKILL=$(ls ./.claude/skills/spike-findings-*/SKILL.md 2>/dev/null | head -1 || true)
...
**After each phase transition** (via `/csp-transition`):
...
**After each milestone** (via `/csp-complete-milestone`):
...
**Check for global defaults** at `~/.csp/defaults.json`. If the file exists, read and display its contents before asking:

```bash
DEFAULTS_RAW=$(cat ~/.csp/defaults.json 2>/dev/null)
```
...
```text
Your saved defaults (~/.csp/defaults.json):
  • Mode: [value]
  • Granularity: [value]
...
**If "Configure fresh" or `~/.csp/defaults.json` doesn't exist:** proceed with the questions below.
...
**PR body onboarding:** Ask which optional PRD-style sections `/csp-ship` should append to generated PR bodies. Use the same `ship.pr_body_sections` mapping as Step 2a: selected sections get `enabled: true`, seeded-but-unselected sections get `enabled: false`, and selecting none writes an empty list. Prefer lean/agile PRD sections that make user value, acceptance criteria, Definition of Done, and stakeholder traceability explicit.
...
**Note:** Run `/csp-settings` anytime to update model profile, workflow agents, branching strategy, and other preferences.
...
- question: "I detected separate git repos in this workspace. Which directories contain code that CSP should commit to?"
- multiSelect: true
...
Spawn 4 parallel csp-project-researcher agents with path references:
...
Spawn csp-roadmapper agent with path references:
...
", subagent_type="csp-roadmapper", model="{roadmapper_model}", description="Create roadmap")
```
...
- Re-spawn roadmapper with revision context:
  ...
  ", subagent_type="csp-roadmapper", model="{roadmapper_model}", description="Revise roadmap")
  ```
...
```bash
csp-sdk query generate-claude-md --output "$INSTRUCTION_FILE"
```

This ensures new projects get the default CSP workflow-enforcement guidance and current project context in `$INSTRUCTION_FILE`.
...
Exit skill and invoke SlashCommand("/csp-discuss-phase 1 --auto")
...
```bash
PHASE1_SECTION=$(csp-sdk query roadmap.get-phase 1 2>/dev/null)
...
/csp-discuss-phase 1 — gather context and clarify approach

---

**Also available:**
- /csp-ui-phase 1 — generate UI design contract (recommended for frontend phases)
- /csp-plan-phase 1 — skip discussion, plan directly
...
- [ ] csp-roadmapper spawned with context
- [ ] Roadmap files written immediately (not draft)
- [ ] User feedback incorporated (if any)
- [ ] ROADMAP.md created with phases, requirement mappings, success criteria
- [ ] STATE.md initialized
- [ ] REQUIREMENTS.md traceability updated
- [ ] `$INSTRUCTION_FILE` generated with CSP workflow guidance (AGENTS.md for Codex, CLAUDE.md otherwise)
- [ ] User knows next step is `/csp-discuss-phase 1`
