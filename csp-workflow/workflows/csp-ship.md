...
```bash
CONFIG=$(csp-sdk query state.load)
```
...
```bash
BASE_BRANCH=$(csp-sdk query config-get git.base_branch 2>/dev/null || echo "")
...
```bash
CUSTOM_PR_SECTIONS=$(csp-sdk query config-get ship.pr_body_sections --default '[]' 2>/dev/null || echo '[]')
```
...
Use these sections for lean/agile PRD material that should travel with the PR without making the core `/csp-ship` body configurable:
...
```bash
PR_BODY_FILE=$(mktemp "${TMPDIR:-/tmp}/csp-pr-body.XXXXXX.md")
...
```bash
REVIEW_CMD=$(csp-sdk query config-get workflow.code_review_command 2>/dev/null | jq -r '.' 2>/dev/null || echo "")
```
...
2. **Load phase context from STATE.md:**
   ```bash
   STATE_STATUS=$(csp-sdk query state.load 2>/dev/null | head -20)
   ```
...
3. **Build review prompt and pipe to command via stdin:**
   ...
   REVIEW_OUTPUT=$(echo "${REVIEW_PROMPT}" | timeout 120 ${REVIEW_CMD} 2>/tmp/csp-review-stderr.log)
   REVIEW_EXIT=$?
   ```
...
4. **Handle timeout (120s) and failure:**
   ...
   if [ $REVIEW_EXIT -ne 0 ]; then
     REVIEW_STDERR=$(cat /tmp/csp-review-stderr.log 2>/dev/null)
...
```bash
csp-sdk query state.update "Last Activity" "$(date +%Y-%m-%d)"
csp-sdk query state.update "Status" "Phase ${PHASE_NUMBER} shipped — PR #${PR_NUMBER}"
```
...
- /csp-complete-milestone (if last phase in milestone)
- /csp-progress (to see what's next)
...
- /csp-complete-milestone — if all phases in milestone are done
- /csp-progress — see overall project state
- /csp-execute-phase {next} — continue to next phase
</offer_next>
...
### Observability
- [ ] Logging added for key operations (success and failure paths)
- [ ] Metrics exposed for monitoring (latency, error rate, throughput)
...
- [ ] Old and new systems can run in parallel during transition
- [ ] Rollback triggers defined (error rate threshold, latency threshold)

### Staged Rollout Strategy
- [ ] Canary deployment: 1% → 10% → 50% → 100%
- [ ] Monitor metrics at each stage for 15-30 minutes
...
- [ ] Slack/Teams channel for launch coordination
