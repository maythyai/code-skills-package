...
Todos are captured during work sessions with /csp-add-todo.

---

Would you like to:

1. Continue with current phase (/csp-progress)
2. Add a todo now (/csp-add-todo)
```
...
- `/csp-capture --list` → show all
- `/csp-capture --list api` → filter to area:api only
</step>
...
- `/csp-capture --list [area]` to filter by area
- `q` to exit
```
...
- options:
  - "Work on it now" — move to done, start working
  - "Create a phase" — /csp-add-phase with this scope
  - "Brainstorm approach" — think through before deciding
  - "Put it back" — return to list
</step>
...
**Create a phase:**
Display: `/csp-add-phase [description from todo]`
