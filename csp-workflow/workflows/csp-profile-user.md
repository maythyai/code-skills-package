...
This workflow wires Phase 1 (session pipeline) and Phase 2 (profiling engine) into a cohesive user-facing experience. All heavy lifting is done by existing `csp-sdk query` handlers (with legacy `csp-tools.cjs` parity where needed) and the csp-user-profiler agent -- this workflow orchestrates the sequence, handles branching, and provides the UX.
</purpose>
...
Key references:
- @$HOME/.claude/code-skills-package/references/ui-brand.md (display patterns)
- @$HOME/.claude/agents/csp-user-profiler.md (profiler agent definition)
- @$HOME/.claude/code-skills-package/references/user-profiling.md (profiling reference doc)
</required_reading>
...
```bash
PROFILE_PATH="$HOME/.claude/code-skills-package/USER-PROFILE.md"
...
```bash
cp "$HOME/.claude/code-skills-package/USER-PROFILE.md" "$HOME/.claude/USER-PROFILE.backup.md"
```
...
✓ Stores profile at $HOME/.claude/code-skills-package/USER-PROFILE.md
...
- options:
  ...
  - "Not now" -- Display "No worries. Run /csp-profile-user when ready." and exit
...
Run session scan:
```bash
SCAN_RESULT=$(csp-sdk query scan-sessions --json 2>/dev/null)
```
...
```bash
SAMPLE_RESULT=$(csp-sdk query profile-sample --json 2>/dev/null)
```
...
**Spawn csp-user-profiler agent using Task tool:**

Use the Task tool to spawn the `csp-user-profiler` agent. Provide it with:
- The sampled JSONL file path from profile-sample output
- The user-profiling reference doc at `$HOME/.claude/code-skills-package/references/user-profiling.md`
...
Reference: @$HOME/.claude/code-skills-package/references/user-profiling.md
...
**Get questions:**
```bash
QUESTIONS=$(csp-sdk query profile-questionnaire --json 2>/dev/null)
```
...
```bash
ANSWERS_PATH=$(mktemp /tmp/csp-profile-answers-XXXXXX.json)
```
...
```bash
ANALYSIS_RESULT=$(csp-sdk query profile-questionnaire --answers "$ANSWERS_PATH" --json 2>/dev/null)
```
...
```bash
ANALYSIS_PATH=$(mktemp /tmp/csp-profile-analysis-XXXXXX.json)
```
...
```bash
csp-sdk query write-profile --input "$ANALYSIS_PATH" --json
```

Display: "✓ Profile written to $HOME/.claude/code-skills-package/USER-PROFILE.md"
...
- options (ALL pre-selected by default):
  - "/csp-dev-preferences command file" -- "Load your preferences in any session"
...
**If no artifacts selected:** Display "No artifacts generated. Your profile is saved at $HOME/.claude/code-skills-package/USER-PROFILE.md" and jump to step 10.
...
**For /csp-dev-preferences (if selected):**

```bash
csp-sdk query generate-dev-preferences --analysis "$ANALYSIS_PATH" --json
```

Display: "✓ Generated /csp-dev-preferences at $HOME/.claude/skills/csp-dev-preferences/SKILL.md"

**For CLAUDE.md profile section (if selected):**

```bash
csp-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --json
```
...
```bash
csp-sdk query generate-claude-profile --analysis "$ANALYSIS_PATH" --global --json
```

Display: "✓ Added profile section to $HOME/.claude/CLAUDE.md"

**Error handling:** If any `csp-sdk query` or csp-tools.cjs call fails, display the error message and use AskUserQuestion to offer "Retry" or "Skip this artifact". On retry, re-run the command. On skip, continue to next artifact.
...
```
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
 CSP > PROFILE COMPLETE ✓
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

Your profile:    $HOME/.claude/code-skills-package/USER-PROFILE.md
```

Then list paths for each generated artifact:
```
Artifacts:
  ✓ /csp-dev-preferences   $HOME/.claude/skills/csp-dev-preferences/SKILL.md
...
- [ ] Artifacts generated sequentially via csp-sdk query (or csp-tools.cjs) subcommands
