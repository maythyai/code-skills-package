...
<required_reading>
@~/.claude/code-skills-package/csp-workflow/references/continuation-format.md
</required_reading>
...
```bash
INIT=$(csp-sdk query init.resume)
...
**If `planning_exists` is false:** This is a new project - route to /csp-new-project
</step>
...
- This is the primary resumption source — structured data from `/csp-pause-work`
...
📋 [N] pending todos — /csp-capture --list to review
...
1. Resume interrupted agent [if interrupted agent found]
   OR
1. Execute phase (/csp-execute-phase {phase} ${CSP_WS})
   OR
1. Discuss Phase 3 context (/csp-discuss-phase 3 ${CSP_WS}) [if CONTEXT.md missing]
   OR
1. Plan Phase 3 (/csp-plan-phase 3 ${CSP_WS}) [if CONTEXT.md exists or discuss option declined]
...
- **Plan phase** → Show direct next command:
  ...
  **Also available:**
  - `/csp-discuss-phase [N] ${CSP_WS}` — gather context first
  - `/csp-plan-phase --research-phase [N] ${CSP_WS}` — investigate unknowns
