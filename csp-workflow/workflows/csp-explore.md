...
offers mid-conversation research when useful, then routes crystallized outputs to CSP artifacts.
</purpose>

<required_reading>
Read all files referenced by the invoking prompt's execution_context before starting.

@~/.claude/code-skills-package/csp-workflow/references/questioning.md
...
- csp-phase-researcher — Researches specific questions and returns concise findings
</available_agent_types>
...
|------|-------------|-----------------|
| Note | `.csp/planning/notes/{slug}.md` | Observations, context, decisions worth remembering |
| Todo | `.csp/planning/todos/pending/{slug}.md` | Concrete actionable tasks identified |
| Seed | `.csp/planning/seeds/{slug}.md` | Forward-looking ideas with trigger conditions |
| Research question | `.csp/planning/research/questions.md` (append) | Open questions that need deeper investigation |
...
| Spike | `/csp-spike` (invoke) | Feasibility uncertainty surfaced — "will this API work?", "can we do X?" |
| Sketch | `/csp-sketch` (invoke) | Design direction unclear — "what should this look like?", "how should this feel?" |
...
- **Notes:** Create `.csp/planning/notes/{slug}.md` with frontmatter (title, date, context)
- **Todos:** Create `.csp/planning/todos/pending/{slug}.md` with frontmatter (title, date, priority)
- **Seeds:** Create `.csp/planning/seeds/{slug}.md` with frontmatter (title, trigger_condition, planted_date)
- **Research questions:** Append to `.csp/planning/research/questions.md`
- **Requirements:** Append to `.csp/planning/REQUIREMENTS.md` with next available REQ ID
- **Phases:** Use existing `/csp-add-phase` command via SlashCommand

Commit if `commit_docs` is enabled:
```bash
csp-sdk query commit "docs: capture exploration — {topic_slug}" --files {file_list}
```
...
Continue exploring with `/csp-explore` or start working with `/csp-progress --next`.
```
