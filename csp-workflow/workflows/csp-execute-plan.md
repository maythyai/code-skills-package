...
@~/.claude/code-skills-package/csp-workflow/references/git-integration.md
</required_reading>
...
# config settings can be fetched via csp-sdk query config-get if needed
```
...
2. Per segment:
   - Subagent route: spawn csp-executor for assigned tasks only. Prompt: task range, plan path, read full plan for context, execute assigned tasks, track deviations, NO SUMMARY/commit. Track via agent protocol.
...
```bash
csp-sdk query phases.list --type summaries --raw
...
Apply deviation rules from the csp-executor agent definition (single source of truth):
...
See `~/.claude/code-skills-package/csp-workflow/references/tdd.md` for structure.
</tdd_plan_execution>
...
See ~/.claude/code-skills-package/csp-workflow/references/checkpoints.md for details.
</step>
...
```bash
NODE_REPAIR=$(csp-sdk query config-get workflow.node_repair 2>/dev/null || echo "true")
```

If `NODE_REPAIR` is `true`: invoke `@./.claude/code-skills-package/workflows/node-repair.md` with:
...
Update STATE.md using csp-sdk query (or legacy csp-tools) state mutations:
...
csp-sdk query state.add-decision \
  --phase "${PHASE}" --summary-file "${DECISION_TEXT_FILE}" --rationale-file "${RATIONALE_FILE}"

# Add blockers if any found
csp-sdk query state.add-blocker --text-file "${BLOCKER_TEXT_FILE}"
```
</step>

<step name="update_session_continuity">
Update session info using csp-sdk query (or legacy csp-tools):
...
if [ "$IS_WORKTREE" != "true" ]; then
  ...
  csp-sdk query roadmap.update-plan-progress "${PHASE}"
fi
```
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/csp-execute-phase {phase}` + `/csp-verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/csp-plan-phase {Z+1}` + `/csp-verify-work {Z}` + `/csp-discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/csp-complete-milestone` + `/csp-verify-work` + `/csp-add-phase` |
