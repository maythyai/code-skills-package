...
- csp-security-auditor — Verifies threat mitigation coverage
</available_agent_types>
...
AGENT_SKILLS_AUDITOR=$(csp-sdk query agent-skills csp-security-auditor)
```

Parse: `phase_dir`, `phase_number`, `phase_name`, `phase_slug`, `padded_phase`.

```bash
AUDITOR_MODEL=$(csp-sdk query resolve-model csp-security-auditor --raw)
SECURITY_CFG=$(csp-sdk query config-get workflow.security_enforcement --raw 2>/dev/null || echo "true")
```

If `SECURITY_CFG` is `false`: exit with "Security enforcement disabled. Enable via /csp-settings."

Display banner: `CSP > SECURE PHASE {N}: {name}`
...
- **State C** (`SUMMARY_FILES` empty): Exit — "Phase {N} not executed. Run /csp-execute-phase {N} first."
...
Agent(
  ...
  subagent_type="csp-security-auditor",
  model="{AUDITOR_MODEL}",
...
**State B (create):**
1. Read template from `~/.claude/code-skills-package/csp-workflow/templates/SECURITY.md`
...
```
CSP > PHASE {N} SECURITY BLOCKED
{K} threats open — phase advancement blocked until threats_open: 0
▶ Fix mitigations then re-run: /csp-secure-phase {N}
...
```bash
csp-sdk query commit "docs(phase-${PHASE}): add/update security threat verification"
```
...
```
CSP > PHASE {N} THREAT-SECURE
threats_open: 0 — all threats have dispositions.
▶ /csp-validate-phase {N}    validate test coverage
▶ /csp-verify-work {N}       run UAT
```
