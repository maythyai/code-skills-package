...
# Step 2: Run quick CSP test
log_step "Step 2/2: Quick CSP test ($TEST_LIMIT instances)..."
echo ""
"$SCRIPT_DIR/run_csp.sh" --limit $TEST_LIMIT --model "$MODEL" --timeout $TIMEOUT
CSP_STATUS=$?
echo ""
...
if [ $VANILLA_STATUS -eq 0 ] && [ $CSP_STATUS -eq 0 ]; then
    ...
    [ $CSP_STATUS -ne 0 ] && log_error "  CSP test: FAILED (exit code $CSP_STATUS)"
    echo ""
