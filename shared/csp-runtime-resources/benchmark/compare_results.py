...
Compare evaluation results between vanilla Claude Code and CSP-enhanced runs.
Generates detailed comparison reports in multiple formats.

Usage:
    python compare_results.py --vanilla results/vanilla/ --csp results/csp/
    python compare_results.py --vanilla results/vanilla/ --csp results/csp/ --output comparison/
"""
...
def compare_results(
    ...
    csp_results: dict[str, Any]
) -> dict[str, Any]:
    """
    Compare vanilla and CSP results.
    ...
    comparison = {
        ...
        "regressions": {},
        "per_instance": {},
        "categories": defaultdict(lambda: {"vanilla": 0, "csp": 0})
    }

    # Overall comparison
    vanilla_pass = vanilla_results.get("passed", 0)
    csp_pass = csp_results.get("passed", 0)
    vanilla_total = vanilla_results.get("total", 0)
    csp_total = csp_results.get("total", 0)
    ...
    comparison["overall"] = {
        ...
        "csp": {
            "total": csp_total,
            "passed": csp_pass,
            "failed": csp_results.get("failed", 0),
            "pass_rate": csp_results.get("pass_rate", 0),
            "avg_tokens": csp_results.get("metadata", {}).get("avg_tokens", 0),
            "avg_time_seconds": csp_results.get("metadata", {}).get("avg_time_seconds", 0),
            "avg_cost_usd": csp_results.get("metadata", {}).get("avg_cost_usd", 0),
            "total_tokens": csp_results.get("metadata", {}).get("total_tokens", 0),
            "total_time_seconds": csp_results.get("metadata", {}).get("total_time_seconds", 0),
            "total_cost_usd": csp_results.get("metadata", {}).get("total_cost_usd", 0),
        },
        "delta": {
            "pass_rate": csp_results.get("pass_rate", 0) - vanilla_results.get("pass_rate", 0),
            "passed": csp_pass - vanilla_pass,
        }
    }
    ...
    if vanilla_pass > 0:
        comparison["overall"]["delta"]["pass_improvement_pct"] = (
            (csp_pass - vanilla_pass) / vanilla_pass * 100
        )
    else:
        comparison["overall"]["delta"]["pass_improvement_pct"] = 100.0 if csp_pass > 0 else 0.0

    vanilla_tokens = vanilla_results.get("metadata", {}).get("avg_tokens", 0)
    csp_tokens = csp_results.get("metadata", {}).get("avg_tokens", 0)
    if vanilla_tokens > 0:
        comparison["overall"]["delta"]["token_change_pct"] = (
            (csp_tokens - vanilla_tokens) / vanilla_tokens * 100
        )

    vanilla_time = vanilla_results.get("metadata", {}).get("avg_time_seconds", 0)
    csp_time = csp_results.get("metadata", {}).get("avg_time_seconds", 0)
    if vanilla_time > 0:
        comparison["overall"]["delta"]["time_change_pct"] = (
            (csp_time - vanilla_time) / vanilla_time * 100
        )

    # Per-instance comparison
    all_instances = set(vanilla_results.get("instances", {}).keys()) | \
                   set(csp_results.get("instances", {}).keys())
    ...
    for instance_id in all_instances:
        ...
        csp_inst = csp_results.get("instances", {}).get(instance_id, {})

        vanilla_status = vanilla_inst.get("status", "missing")
        csp_status = csp_inst.get("status", "missing")

        vanilla_passed = vanilla_status == "passed"
        csp_passed = csp_status == "passed"
        ...
        inst_comparison = {
            ...
            "csp_tokens": csp_inst.get("tokens_total", 0),
            "vanilla_time": vanilla_inst.get("time_seconds", 0),
            "csp_time": csp_inst.get("time_seconds", 0),
        }

        # Categorize change
        if not vanilla_passed and csp_passed:
            inst_comparison["change"] = "improvement"
            improvements.append(instance_id)
        elif vanilla_passed and not csp_passed:
            inst_comparison["change"] = "regression"
            regressions.append(instance_id)
        elif vanilla_passed and csp_passed:
        ...
        if "__" in instance_id:
            ...
            if csp_passed:
                comparison["categories"][repo]["csp"] += 1
...
def generate_markdown_report(comparison: dict[str, Any]) -> str:
    ...
    lines = [
        "# SWE-bench Comparison Report: Vanilla vs CSP",
        "",
    ...
    if delta["pass_rate"] > 0:
        lines.append(f"CSP improved pass rate by **{delta['pass_rate']:.1f} percentage points** "
                    f"({vanilla['pass_rate']:.1f}% -> {csp['pass_rate']:.1f}%).")
    elif delta["pass_rate"] < 0:
        lines.append(f"CSP decreased pass rate by **{abs(delta['pass_rate']):.1f} percentage points** "
                    f"({vanilla['pass_rate']:.1f}% -> {csp['pass_rate']:.1f}%).")
    else:
        lines.append("Pass rates are identical between vanilla and CSP.")

    lines.extend([
        "",
        f"- **Improvements:** {comparison['improvements']['count']} instances that vanilla failed but CSP passed",
        f"- **Regressions:** {comparison['regressions']['count']} instances that vanilla passed but CSP failed",
        "",
        "## Overall Metrics",
        "",
        "| Metric | Vanilla | CSP | Delta |",
        "|--------|---------|-----|-------|",
        f"| Total Instances | {vanilla['total']} | {csp['total']} | - |",
        f"| Passed | {vanilla['passed']} | {csp['passed']} | {delta['passed']:+d} |",
        f"| Failed | {vanilla['failed']} | {csp['failed']} | {csp['failed'] - vanilla['failed']:+d} |",
        f"| **Pass Rate** | **{vanilla['pass_rate']:.2f}%** | **{csp['pass_rate']:.2f}%** | **{delta['pass_rate']:+.2f}pp** |",
        "",
        "## Resource Usage",
        "",
        "| Metric | Vanilla | CSP | Change |",
        "|--------|---------|-----|--------|",
    ])
    ...
    lines.append(f"| Avg Tokens/Instance | {vanilla['avg_tokens']:,.0f} | {csp['avg_tokens']:,.0f} | {token_change_str} |")

    # Time comparison
    time_change = delta.get("time_change_pct", 0)
    time_change_str = f"{time_change:+.1f}%" if time_change else "N/A"
    lines.append(f"| Avg Time/Instance | {vanilla['avg_time_seconds']:.1f}s | {csp['avg_time_seconds']:.1f}s | {time_change_str} |")

    # Cost comparison
    lines.append(f"| Total Cost | ${vanilla['total_cost_usd']:.2f} | ${csp['total_cost_usd']:.2f} | ${csp['total_cost_usd'] - vanilla['total_cost_usd']:+.2f} |")

    lines.extend([
        "",
        "## Improvements (Vanilla FAIL -> CSP PASS)",
        "",
    ])
    ...
    lines.extend([
        "",
        "## Regressions (Vanilla PASS -> CSP FAIL)",
        "",
    ])
    ...
    if comparison["categories"]:
        lines.extend([
            ...
            "",
            "| Repository | Vanilla Passed | CSP Passed | Delta |",
            "|------------|----------------|------------|-------|",
        ])

        for repo, counts in sorted(comparison["categories"].items()):
            delta_count = counts["csp"] - counts["vanilla"]
            lines.append(f"| {repo} | {counts['vanilla']} | {counts['csp']} | {delta_count:+d} |")
...
def generate_csv(comparison: dict[str, Any], output_file: Path):
    ...
    fieldnames = [
        "instance_id", "vanilla_status", "csp_status", "change",
        "vanilla_tokens", "csp_tokens", "vanilla_time", "csp_time"
    ]
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare SWE-bench results between vanilla and CSP runs",
...
Examples:
    ...
    parser.add_argument(
        ...
        type=Path,
        required=True,
        help="Path to CSP-enhanced results directory"
    )
    ...
    if not args.csp_results.exists():
        logger.error(f"CSP results directory not found: {args.csp_results}")
        return 1
    ...
    logger.info(f"Loading CSP results from {args.csp_results}")
    csp_results = load_results(args.csp_results)

    # Compare
    logger.info("Comparing results...")
    comparison = compare_results(vanilla_results, csp_results)
    ...
    print(f"CSP Pass Rate:     {comparison['overall']['csp']['pass_rate']:.2f}%")
