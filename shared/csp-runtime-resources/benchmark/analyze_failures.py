...
and understand differences between vanilla and CSP runs.

Usage:
    python analyze_failures.py --results results/vanilla/ --predictions predictions.json
    python analyze_failures.py --vanilla results/vanilla/ --csp results/csp/ --compare
"""
...
def compare_failures(
    ...
    """Compare failure patterns between vanilla and CSP."""
    comparison = {
        "timestamp": datetime.now().isoformat(),
        "vanilla_failures": vanilla_analysis["total_failures"],
        "csp_failures": csp_analysis["total_failures"],
        "category_comparison": {},
        "unique_to_vanilla": [],
        "unique_to_csp": [],
        "common_failures": [],
    ...
    all_categories = set(vanilla_analysis["category_counts"].keys()) | \
                    set(csp_analysis["category_counts"].keys())

    for category in all_categories:
        vanilla_count = vanilla_analysis["category_counts"].get(category, 0)
        csp_count = csp_analysis["category_counts"].get(category, 0)

        comparison["category_comparison"][category] = {
            "vanilla": vanilla_count,
            "csp": csp_count,
            "delta": csp_count - vanilla_count
        }

    # Instance comparison
    vanilla_failed = {f["instance_id"] for f in vanilla_analysis["failures"]}
    csp_failed = {f["instance_id"] for f in csp_analysis["failures"]}

    comparison["unique_to_vanilla"] = list(vanilla_failed - csp_failed)
    comparison["unique_to_csp"] = list(csp_failed - vanilla_failed)
    comparison["common_failures"] = list(vanilla_failed & csp_failed)

    # Generate insights
    insights = []

    if len(comparison["unique_to_vanilla"]) > len(comparison["unique_to_csp"]):
        insights.append({
            "type": "positive",
            "message": f"CSP fixed {len(comparison['unique_to_vanilla'])} failures that vanilla couldn't solve."
        })
    elif len(comparison["unique_to_csp"]) > len(comparison["unique_to_vanilla"]):
        insights.append({
            "type": "negative",
            "message": f"CSP introduced {len(comparison['unique_to_csp'])} new failures compared to vanilla."
        })
    ...
    for category, counts in comparison["category_comparison"].items():
        if counts["delta"] < -2:
            insights.append({
                "type": "positive",
                "message": f"CSP reduced '{category}' failures by {abs(counts['delta'])}."
            })
        elif counts["delta"] > 2:
            insights.append({
                "type": "negative",
                "message": f"CSP increased '{category}' failures by {counts['delta']}."
            })
...
def generate_failure_report(
    ...
    if comparison:
        lines.extend([
            "",
            "## Vanilla vs CSP Comparison",
            "",
            f"- **Vanilla Failures:** {comparison['vanilla_failures']}",
            f"- **CSP Failures:** {comparison['csp_failures']}",
            f"- **Fixed by CSP:** {len(comparison['unique_to_vanilla'])}",
            f"- **New in CSP:** {len(comparison['unique_to_csp'])}",
            f"- **Common Failures:** {len(comparison['common_failures'])}",
            "",
            "### Category Changes",
            "",
            "| Category | Vanilla | CSP | Delta |",
            "|----------|---------|-----|-------|",
        ])
        ...
        for category, counts in sorted(
            ...
            lines.append(f"| {category} | {counts['vanilla']} | {counts['csp']} | {delta_str} |")
...
Examples:
    ...
    python analyze_failures.py --results results/csp/ --predictions predictions.json

    # Compare vanilla vs CSP failures
    python analyze_failures.py --vanilla results/vanilla/ --csp results/csp/ --compare
        """
    )
    ...
    parser.add_argument(
        "--csp",
        type=Path,
        help="Path to CSP results for comparison"
    )

    parser.add_argument(
        "--compare",
        action="store_true",
        help="Compare vanilla vs CSP (requires --vanilla and --csp)"
    )
    ...
    # Validate arguments
    if args.compare:
        if not args.vanilla or not args.csp_results:
            parser.error("--compare requires both --vanilla and --csp")
    elif not args.results:
        parser.error("Either --results or (--vanilla, --csp, --compare) required")
    ...
    if args.compare:
        ...
        for pred_path in [args.csp / "predictions.json", args.csp_results.parent / "csp_predictions.json"]:
            if pred_path.exists():
                csp_predictions = load_predictions(pred_path)
                break

        logger.info("Analyzing failures...")
        vanilla_analysis = analyze_failures(vanilla_results, vanilla_predictions)
        csp_analysis = analyze_failures(csp_results, csp_predictions)

        logger.info("Comparing failures...")
        comparison = compare_failures(vanilla_analysis, csp_analysis)
        ...
        report = generate_failure_report(csp_analysis, comparison)
        ...
        print(f"CSP Failures:     {csp_analysis['total_failures']}")
        print(f"Fixed by CSP:     {len(comparison['unique_to_vanilla'])}")
        print(f"New in CSP:       {len(comparison['unique_to_csp'])}")
