...
Automated benchmark comparison between vanilla Claude Code and CSP-enhanced Claude Code.
...
- Tests both vanilla and CSP modes
...
- Standard Claude Code without CSP
...
### run_csp.sh
Run CSP-enhanced benchmark:
- Claude Code with code-skills-package orchestration
- Saves predictions to `predictions/csp/`
- Logs to `logs/csp_*.log`
...
**Examples:**
```bash
./run_csp.sh                    # Full benchmark
./run_csp.sh --limit 100        # First 100 instances
```
...
- `--skip-csp` - Skip CSP benchmark run
...
./run_full_comparison.sh --skip-vanilla     # Only run CSP (reuse vanilla results)
```
...
│   └── csp_*.log            # CSP run logs
├── results/
│   ├── vanilla_results.json  # Vanilla evaluation
│   ├── csp_results.json      # CSP evaluation
...
4. **Review Results**:
   ...
   - Inspect predictions in `predictions/vanilla/` and `predictions/csp/`
   - Review logs in `logs/`
...
./run_csp.sh --model claude-opus-4-6-20260205
```
...
2. **Parallel Runs**: Don't run vanilla and CSP in parallel (share API rate limits)
