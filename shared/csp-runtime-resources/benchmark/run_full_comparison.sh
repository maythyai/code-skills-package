...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Run complete benchmark comparison between vanilla and CSP modes."
            echo ""
            echo "Options:"
            ...
            echo "  --skip-csp        Skip CSP benchmark run"
...
# Step 2: Run CSP benchmark
if [ "$SKIP_CSP" = false ]; then
    log_step "Step 2/4: Running CSP-enhanced benchmark..."
    echo ""
    "$SCRIPT_DIR/run_csp.sh" $ARGS
    if [ $? -ne 0 ]; then
        log_error "CSP benchmark failed. Aborting."
        exit 1
    fi
    echo ""
else
    log_warn "Skipping CSP benchmark (--skip-csp)"
    echo ""
fi
...
if [ "$SKIP_EVAL" = false ]; then
    ...
    log_step "Step 4/4: Evaluating CSP predictions..."
    echo ""
    if [ -f "$SCRIPT_DIR/evaluate.py" ]; then
        python3 "$SCRIPT_DIR/evaluate.py" \
            --predictions "$SCRIPT_DIR/predictions/csp" \
            --output "$SCRIPT_DIR/results/csp_results.json"
        if [ $? -ne 0 ]; then
            log_warn "CSP evaluation had issues (continuing...)"
        fi
    else
        log_warn "evaluate.py not found, skipping evaluation"
    fi
...
if [ -f "$SCRIPT_DIR/compare_results.py" ]; then
    python3 "$SCRIPT_DIR/compare_results.py" \
        ...
        --csp "$SCRIPT_DIR/predictions/csp/predictions.jsonl" \
...
### CSP-Enhanced
Location: \`predictions/csp/\`
Results: \`results/csp_results.json\`
...
log_info "  CSP predictions:     $SCRIPT_DIR/predictions/csp/"
