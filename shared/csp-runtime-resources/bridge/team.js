...
function findWorkspaceRoot(startDir) {
  if (process.env.CSP_DISABLE_MULTIREPO === "1") return null;
...
function getCspRoot(worktreeRoot) {
  const customDir = process.env.CSP_STATE_DIR;
  if (customDir) {
    ...
    if (!dualDirWarnings.has(warningKey) && existsSync(legacyPath) && existsSync(centralizedPath)) {
      ...
      console.warn(
        `[csp] Both legacy state dir (${legacyPath}) and centralized state dir (${centralizedPath}) exist. Using centralized dir. Consider migrating data from the legacy dir and removing it.`
      );
    }
...
var init_worktree_paths = __esm({
  "src/lib/worktree-paths.ts"() {
    "use strict";
    init_config_dir();
    WORKSPACE_MARKER = ".csp-workspace";
    OmcPaths = {
      ROOT: ".csp",
      STATE: ".csp/state",
      SESSIONS: ".csp/state/sessions",
      PLANS: ".csp/plans",
      RESEARCH: ".csp/research",
      NOTEPAD: ".csp/notepad.md",
      PROJECT_MEMORY: ".csp/project-memory.json",
      DRAFTS: ".csp/drafts",
      NOTEPADS: ".csp/notepads",
      LOGS: ".csp/logs",
      SCIENTIST: ".csp/scientist",
      AUTOPILOT: ".csp/autopilot",
      SKILLS: ".csp/skills",
      SHARED_MEMORY: ".csp/state/shared-memory",
      DEEPINIT_MANIFEST: ".csp/deepinit-manifest.json"
    };
...
var init_state_paths = __esm({
  "src/team/state-paths.ts"() {
    "use strict";
    TeamPaths = {
      root: (teamName) => `.csp/state/team/${teamName}`,
      config: (teamName) => `.csp/state/team/${teamName}/config.json`,
      shutdown: (teamName) => `.csp/state/team/${teamName}/shutdown.json`,
      tasks: (teamName) => `.csp/state/team/${teamName}/tasks`,
      taskFile: (teamName, taskId) => `.csp/state/team/${teamName}/tasks/${normalizeTaskFileStem(taskId)}.json`,
      workers: (teamName) => `.csp/state/team/${teamName}/workers`,
      workerDir: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}`,
      heartbeat: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/heartbeat.json`,
      inbox: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/inbox.md`,
      outbox: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/outbox.jsonl`,
      ready: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/.ready`,
      overlay: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/AGENTS.md`,
      shutdownAck: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/shutdown-ack.json`,
      mailbox: (teamName, workerName) => `.csp/state/team/${teamName}/mailbox/${workerName}.json`,
      mailboxLockDir: (teamName, workerName) => `.csp/state/team/${teamName}/mailbox/.lock-${workerName}`,
      dispatchRequests: (teamName) => `.csp/state/team/${teamName}/dispatch/requests.json`,
      dispatchLockDir: (teamName) => `.csp/state/team/${teamName}/dispatch/.lock`,
      workerStatus: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/status.json`,
      workerIdleNotify: (teamName) => `.csp/state/team/${teamName}/worker-idle-notify.json`,
      workerPrevNotifyState: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/prev-notify-state.json`,
      events: (teamName) => `.csp/state/team/${teamName}/events.jsonl`,
      approval: (teamName, taskId) => `.csp/state/team/${teamName}/approvals/${taskId}.json`,
      manifest: (teamName) => `.csp/state/team/${teamName}/manifest.json`,
      monitorSnapshot: (teamName) => `.csp/state/team/${teamName}/monitor-snapshot.json`,
      summarySnapshot: (teamName) => `.csp/state/team/${teamName}/summary-snapshot.json`,
      phaseState: (teamName) => `.csp/state/team/${teamName}/phase-state.json`,
      scalingLock: (teamName) => `.csp/state/team/${teamName}/.scaling-lock`,
      workerIdentity: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/identity.json`,
      workerAgentsMd: (teamName) => `.csp/state/team/${teamName}/worker-agents.md`,
      shutdownRequest: (teamName, workerName) => `.csp/state/team/${teamName}/workers/${workerName}/shutdown-request.json`
    };
  }
});
...
function resolveDispatchLockTimeoutMs(env = process.env) {
  const raw = env[CSP_DISPATCH_LOCK_TIMEOUT_ENV];
  ...
  while (true) {
    try {
      ...
      if (Date.now() > deadline) {
        throw new Error(
          `Timed out acquiring dispatch lock for ${teamName} after ${timeoutMs}ms. Set ${CSP_DISPATCH_LOCK_TIMEOUT_ENV} to increase (current: ${timeoutMs}ms, max: ${MAX_DISPATCH_LOCK_TIMEOUT_MS}ms).`
        );
      }
...
var CSP_DISPATCH_LOCK_TIMEOUT_ENV, DEFAULT_DISPATCH_LOCK_TIMEOUT_MS, MIN_DISPATCH_LOCK_TIMEOUT_MS, MAX_DISPATCH_LOCK_TIMEOUT_MS, DISPATCH_LOCK_INITIAL_POLL_MS, DISPATCH_LOCK_MAX_POLL_MS, LOCK_STALE_MS;
var init_dispatch_queue = __esm({
  "src/team/dispatch-queue.ts"() {
    ...
    init_fs_utils();
    init_contracts();
    CSP_DISPATCH_LOCK_TIMEOUT_ENV = "CSP_TEAM_DISPATCH_LOCK_TIMEOUT_MS";
...
function logSwallowedError(context, error) {
  try {
    console.warn(`[csp] ${context}: ${formatSwallowedError(error)}`);
  } catch {
  }
}
...
async function waitForShellReady(paneId, opts = {}) {
  ...
  const envTimeout = Number.parseInt(process.env.CSP_TEAM_SHELL_READY_TIMEOUT_MS ?? "", 10);
...
function buildWorkerStartCommand(config) {
  ...
  const shouldSourceRc = process.env.CSP_TEAM_NO_RC !== "1";
...
async function createTeamSession(teamName, workerCount, cwd, options = {}) {
  ...
  if (useDedicatedWindow) {
    ...
    const windowName = `csp-${sanitizeName(teamName)}`.slice(0, 32);
...
async function waitForPaneReady(paneId, opts = {}) {
  const envTimeout = Number.parseInt(process.env.CSP_SHELL_READY_TIMEOUT_MS ?? "", 10);
  ...
  console.warn(
    `[tmux-session] waitForPaneReady: pane ${paneId} timed out after ${timeoutMs}ms (set CSP_SHELL_READY_TIMEOUT_MS to tune)`
  );
  return false;
}
...
function shouldAttemptAdaptiveRetry(args) {
  if (process.env.CSP_TEAM_AUTO_INTERRUPT_RETRY === "0") return false;
...
async function injectToLeaderPane(sessionName2, leaderPaneId, message) {
  const prefixed = `[CSP_TMUX_INJECT] ${message}`.slice(0, 200);
  try {
...
async function killWorkerPanes(opts) {
  ...
  const shutdownPath = join8(getCspRoot(cwd), "state", "team", teamName, "shutdown.json");
  try {
...
async function killTeamSession(sessionName2, workerPaneIds, leaderPaneId, options = {}) {
  ...
  if (process.env.CSP_TEAM_ALLOW_KILL_CURRENT_SESSION !== "1" && process.env.TMUX) {
    try {
...
// src/utils/csp-cli-rendering.ts
...
function resolveOmcCliPrefix(options = {}) {
  ...
  const cspAvailable = options.cspAvailable ?? commandExists(CSP_CLI_BINARY, env);
  if (cspAvailable) {
    return CSP_CLI_BINARY;
  }
  const pluginRoot = typeof env.CLAUDE_PLUGIN_ROOT === "string" ? env.CLAUDE_PLUGIN_ROOT.trim() : "";
  if (pluginRoot) {
    return CSP_PLUGIN_BRIDGE_PREFIX;
  }
  return CSP_CLI_BINARY;
}
...
function formatOmcCliInvocation(commandSuffix, options = {}) {
  const suffix = commandSuffix.trim().replace(/^csp\s+/, "");
  return `${resolveInvocationPrefix(suffix, options)} ${suffix}`.trim();
}
var CSP_CLI_BINARY, CSP_PLUGIN_BRIDGE_PREFIX;
var init_csp_cli_rendering = __esm({
  "src/utils/csp-cli-rendering.ts"() {
    "use strict";
    CSP_CLI_BINARY = "csp";
    CSP_PLUGIN_BRIDGE_PREFIX = 'node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs';
  }
});
...
function getLegacyOmcDir() {
  return join11(getUserHomeDir(), ".csp");
}
function getGlobalOmcStateRoot() {
  const explicitRoot = process.env.CSP_HOME?.trim();
  if (explicitRoot) {
    return join11(explicitRoot, "state");
  }
  if (prefersXdgOmcDirs()) {
    return join11(getStateDir(), "csp");
  }
...
function shouldAutoForceInherit() {
  if (process.env.CSP_ROUTING_FORCE_INHERIT === "true") {
    return true;
  }
  if (process.env.CLAUDE_CODE_USE_BEDROCK === "1") {
    return true;
...
function buildDefaultConfig() {
  ...
  return {
    agents: {
      csp: { model: defaultTierModels.HIGH },
...
function getConfigPaths() {
  ...
  return {
    user: join12(userConfigDir, "claude-csp", "config.jsonc"),
    project: join12(process.cwd(), ".claude", "csp.jsonc")
  };
}
...
function loadEnvConfig() {
  ...
  if (process.env.CSP_PARALLEL_EXECUTION !== void 0) {
    config.features = {
      ...config.features,
      parallelExecution: process.env.CSP_PARALLEL_EXECUTION === "true"
    };
  }
  if (process.env.CSP_LSP_TOOLS !== void 0) {
    config.features = {
      ...config.features,
      lspTools: process.env.CSP_LSP_TOOLS === "true"
    };
  }
  if (process.env.CSP_MAX_BACKGROUND_TASKS) {
    const maxTasks = parseInt(process.env.CSP_MAX_BACKGROUND_TASKS, 10);
    if (!isNaN(maxTasks)) {
      config.permissions = {
        ...config.permissions,
  ...
  if (process.env.CSP_ROUTING_ENABLED !== void 0) {
    config.routing = {
      ...config.routing,
      enabled: process.env.CSP_ROUTING_ENABLED === "true"
    };
  }
  if (process.env.CSP_ROUTING_FORCE_INHERIT !== void 0) {
    config.routing = {
      ...config.routing,
      forceInherit: process.env.CSP_ROUTING_FORCE_INHERIT === "true"
    };
  }
  if (process.env.CSP_ROUTING_DEFAULT_TIER) {
    const tier = process.env.CSP_ROUTING_DEFAULT_TIER.toUpperCase();
  ...
  for (const key of aliasKeys) {
    const envVal = process.env[`CSP_MODEL_ALIAS_${key}`];
    if (envVal) {
  ...
  if (process.env.CSP_ESCALATION_ENABLED !== void 0) {
    config.routing = {
      ...config.routing,
      escalationEnabled: process.env.CSP_ESCALATION_ENABLED === "true"
    };
  }
  const externalModelsDefaults = {};
  if (process.env.CSP_EXTERNAL_MODELS_DEFAULT_PROVIDER) {
    const provider = process.env.CSP_EXTERNAL_MODELS_DEFAULT_PROVIDER;
    if (provider === "codex" || provider === "gemini") {
      externalModelsDefaults.provider = provider;
    }
  }
  if (process.env.CSP_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL) {
    externalModelsDefaults.codexModel = process.env.CSP_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL;
  } else if (process.env.CSP_CODEX_DEFAULT_MODEL) {
    externalModelsDefaults.codexModel = process.env.CSP_CODEX_DEFAULT_MODEL;
  }
  if (process.env.CSP_EXTERNAL_MODELS_DEFAULT_GEMINI_MODEL) {
    externalModelsDefaults.geminiModel = process.env.CSP_EXTERNAL_MODELS_DEFAULT_GEMINI_MODEL;
  } else if (process.env.CSP_GEMINI_DEFAULT_MODEL) {
    externalModelsDefaults.geminiModel = process.env.CSP_GEMINI_DEFAULT_MODEL;
  }
  if (process.env.CSP_EXTERNAL_MODELS_DEFAULT_GROK_MODEL) {
    externalModelsDefaults.grokModel = process.env.CSP_EXTERNAL_MODELS_DEFAULT_GROK_MODEL;
  } else if (process.env.CSP_GROK_DEFAULT_MODEL) {
    externalModelsDefaults.grokModel = process.env.CSP_GROK_DEFAULT_MODEL;
  }
  const externalModelsFallback = {
    onModelFailure: "provider_chain"
  };
  if (process.env.CSP_EXTERNAL_MODELS_FALLBACK_POLICY) {
    const policy = process.env.CSP_EXTERNAL_MODELS_FALLBACK_POLICY;
  ...
  if (process.env.CSP_DELEGATION_ROUTING_ENABLED !== void 0) {
    config.delegationRouting = {
      ...config.delegationRouting,
      enabled: process.env.CSP_DELEGATION_ROUTING_ENABLED === "true"
    };
  }
  if (process.env.CSP_DELEGATION_ROUTING_DEFAULT_PROVIDER) {
    const provider = process.env.CSP_DELEGATION_ROUTING_DEFAULT_PROVIDER;
...
function warnOnDeprecatedDelegationRouting(config) {
  ...
  console.warn(
    "[CSP] delegationRouting to Codex/Gemini is deprecated and falls back to Claude Task. Use /team for Codex/Gemini CLI workers instead."
  );
}
function validateTeamConfig(config) {
  ...
  if (ops && typeof ops === "object") {
    if (ops.defaultAgentType !== void 0) {
      if (typeof ops.defaultAgentType !== "string" || !TEAM_ROLE_PROVIDERS.has(ops.defaultAgentType)) {
        throw new Error(
          `[CSP] team.ops.defaultAgentType: invalid value "${String(ops.defaultAgentType)}". Allowed: ${[...TEAM_ROLE_PROVIDERS].join(", ")}`
        );
      }
    }
    if (ops.worktreeMode !== void 0) {
      ...
      if (typeof ops.worktreeMode !== "string" || !allowed.has(ops.worktreeMode)) {
        throw new Error(
          `[CSP] team.ops.worktreeMode: invalid value "${String(ops.worktreeMode)}". Allowed: ${[...allowed].join(", ")}`
        );
      }
    }
  }
  ...
  for (const [rawRoleKey, rawSpec] of Object.entries(roleRouting)) {
    ...
    if (!CANONICAL_TEAM_ROLE_SET.has(normalized)) {
      throw new Error(
        `[CSP] team.roleRouting: unknown role "${rawRoleKey}". Allowed roles: ${[...CANONICAL_TEAM_ROLE_SET].join(", ")}`
      );
    }
    if (!rawSpec || typeof rawSpec !== "object" || Array.isArray(rawSpec)) {
      throw new Error(
        `[CSP] team.roleRouting.${rawRoleKey}: must be an object, got ${Array.isArray(rawSpec) ? "array" : typeof rawSpec}`
      );
    }
    const spec = rawSpec;
    if (normalized === "orchestrator") {
      for (const key of Object.keys(spec)) {
        if (key !== "model") {
          throw new Error(
            `[CSP] team.roleRouting.orchestrator: key "${key}" is not allowed (orchestrator is pinned to claude; only "model" is configurable)`
          );
        }
      }
      if (spec.model !== void 0 && !isValidModelValue(spec.model)) {
        throw new Error(
          `[CSP] team.roleRouting.orchestrator.model: must be a tier name (HIGH|MEDIUM|LOW) or model ID string, got ${typeof spec.model}`
        );
      }
      continue;
    }
    if (spec.provider !== void 0) {
      if (typeof spec.provider !== "string" || !TEAM_ROLE_PROVIDERS.has(spec.provider)) {
        throw new Error(
          `[CSP] team.roleRouting.${rawRoleKey}.provider: invalid value "${String(spec.provider)}". Allowed: ${[...TEAM_ROLE_PROVIDERS].join(", ")}`
        );
      }
    }
    if (spec.model !== void 0 && !isValidModelValue(spec.model)) {
      throw new Error(
        `[CSP] team.roleRouting.${rawRoleKey}.model: must be a tier name (HIGH|MEDIUM|LOW) or a non-empty model ID string`
      );
    }
    if (spec.agent !== void 0) {
      if (typeof spec.agent !== "string" || !KNOWN_AGENT_NAME_SET.has(spec.agent)) {
        throw new Error(
          `[CSP] team.roleRouting.${rawRoleKey}.agent: unknown agent "${String(spec.agent)}". Allowed: ${[...KNOWN_AGENT_NAME_SET].join(", ")}`
        );
      }
    }
  }
...
function parseTeamRoleOverridesFromEnv() {
  const raw = process.env.CSP_TEAM_ROLE_OVERRIDES;
  if (!raw) return void 0;
  try {
    const parsed = JSON.parse(raw);
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      console.warn(
        "[CSP] CSP_TEAM_ROLE_OVERRIDES: expected a JSON object; ignoring."
      );
      return void 0;
    }
    return parsed;
    ...
    console.warn(
      `[CSP] CSP_TEAM_ROLE_OVERRIDES: invalid JSON, ignoring (${err.message})`
    );
    return void 0;
  }
}
function loadConfig() {
  ...
  if (config.routing?.forceInherit !== true && process.env.CSP_ROUTING_FORCE_INHERIT === void 0 && shouldAutoForceInherit()) {
    config.routing = {
      ...config.routing,
      forceInherit: true
    };
...
var init_executor = __esm({
  "src/agents/executor.ts"() {
    ...
    executorAgent = {
      name: "executor",
      description: "Focused task executor. Execute tasks directly. NEVER delegate or spawn other agents. Same discipline as CSP, no delegation.",
...
function loadSecurityFromConfigFiles() {
  const paths = [
    join13(process.cwd(), ".claude", "csp.jsonc"),
    join13(getConfigDir(), "claude-csp", "config.jsonc")
  ];
...
function getSecurityConfig() {
  ...
  const isStrict = process.env.CSP_SECURITY === "strict";
...
function getTrustedPrefixes() {
  ...
  const custom = (process.env.CSP_TRUSTED_CLI_DIRS ?? "").split(":").map((part) => part.trim()).filter(Boolean).filter((part) => isAbsolute6(part));
  trusted.push(...custom);
  return trusted;
}
...
function resolveCliBinaryPath(binary) {
  ...
  if (!isTrustedPrefix(resolvedPath)) {
    console.warn(`[csp:cli-security] CLI binary '${binary}' resolved to non-standard path: ${resolvedPath}`);
  }
...
function getWorkerEnv(teamName, workerName, agentType, env = process.env) {
  ...
  const workerEnv = {
    CSP_TEAM_WORKER: `${teamName}/${workerName}`,
    CSP_TEAM_NAME: teamName,
    CSP_WORKER_AGENT_TYPE: agentType
  };
...
function resolveClaudeWorkerModel(env = process.env) {
  if (env.CSP_ROUTING_FORCE_INHERIT === "true") {
    return void 0;
  }
  ...
  const cspModel = env.CSP_MODEL_MEDIUM || "";
  if (cspModel) {
    return cspModel;
  }
  return void 0;
...
var init_model_contract = __esm({
  "src/team/model-contract.ts"() {
    ...
    WORKER_MODEL_ENV_ALLOWLIST = [
      ...
      "CSP_MODEL_MEDIUM",
      "CSP_MODEL_LOW",
      "CSP_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL",
      "CSP_CODEX_DEFAULT_MODEL",
      "CSP_EXTERNAL_MODELS_DEFAULT_GEMINI_MODEL",
      "CSP_GEMINI_DEFAULT_MODEL",
      "CSP_EXTERNAL_MODELS_DEFAULT_GROK_MODEL",
      "CSP_GROK_DEFAULT_MODEL"
    ];
  }
});
...
- **Environment**: CSP_TEAM_WORKER=${teamName}/${workerName}
...
- Resolve the team state root in this order: \`CSP_TEAM_STATE_ROOT\` env -> worker identity \`team_state_root\` -> config/manifest \`team_state_root\` -> ${params.cwd}/.csp/state/team/${teamName}.
- \`CSP_TEAM_STATE_ROOT\` is the team-specific root (\`.../.csp/state/team/${teamName}\`). When it is set, append worker/mailbox paths directly below it; do not append another \`team/${teamName}\` segment.
- Worktree-backed workers MUST use the canonical leader-owned state root for inbox, mailbox, task lifecycle, status, heartbeat, and shutdown files; do not use a local worktree \`.csp/state\` when \`CSP_TEAM_STATE_ROOT\` is set.
...
async function composeInitialInbox(teamName, workerName, content, cwd, cliOutputContract) {
  const inboxPath = join14(cwd, `.csp/state/team/${teamName}/workers/${workerName}/inbox.md`);
...
async function appendToInbox(teamName, workerName, message, cwd) {
  ...
  const inboxPath = join14(cwd, `.csp/state/team/${safeTeam}/workers/${safeWorker}/inbox.md`);
...
async function ensureWorkerStateDir(teamName, workerName, cwd) {
  const workerDir = join14(cwd, `.csp/state/team/${teamName}/workers/${workerName}`);
  await mkdir3(workerDir, { recursive: true });
  const mailboxDir = join14(cwd, `.csp/state/team/${teamName}/mailbox`);
  await mkdir3(mailboxDir, { recursive: true });
  const tasksDir = join14(cwd, `.csp/state/team/${teamName}/tasks`);
  await mkdir3(tasksDir, { recursive: true });
}
async function writeWorkerOverlay(params) {
  const { teamName, workerName, cwd } = params;
  const overlay = generateWorkerOverlay(params);
  const overlayPath = join14(cwd, `.csp/state/team/${teamName}/workers/${workerName}/AGENTS.md`);
...
var init_worker_bootstrap = __esm({
  "src/team/worker-bootstrap.ts"() {
    "use strict";
    init_prompt_helpers();
    init_csp_cli_rendering();
    init_tmux_session();
    init_fs_utils();
    init_model_contract();
    DEFAULT_INSTRUCTION_STATE_ROOT = ".csp/state";
  }
});
...
function getWorktreePath(repoRoot, teamName, workerName) {
  return join17(getCspRoot(repoRoot), "team", sanitizeName(teamName), "worktrees", sanitizeName(workerName));
}
function getBranchName(teamName, workerName) {
  return `csp-team/${sanitizeName(teamName)}/${sanitizeName(workerName)}`;
}
...
function getMetadataPath(repoRoot, teamName) {
  return join17(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName), "worktrees.json");
}
function getLegacyMetadataPath(repoRoot, teamName) {
  return join17(getCspRoot(repoRoot), "state", "team-bridge", sanitizeName(teamName), "worktrees.json");
}
function getWorkerStateDir(repoRoot, teamName, workerName) {
  return join17(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName), "workers", sanitizeName(workerName));
}
...
function readRootAgentsBackup(repoRoot, teamName, workerName) {
  ...
  try {
    ...
    process.stderr.write(`[csp] warning: worktree root AGENTS backup parse error: ${msg}
`);
...
function installWorktreeRootAgents(teamName, workerName, repoRoot, worktreePath, overlayContent) {
  const cspRoot = getCspRoot(repoRoot);
  validateResolvedPath(worktreePath, cspRoot);
  const agentsPath = join17(worktreePath, "AGENTS.md");
  validateResolvedPath(agentsPath, worktreePath);
  const backupPath = getRootAgentsBackupPath(repoRoot, teamName, workerName);
  validateResolvedPath(backupPath, cspRoot);
...
function restoreWorktreeRootAgents(teamName, workerName, repoRoot, worktreePath) {
  ...
  validateResolvedPath(resolvedWorktreePath, cspRoot);
...
function readMetadataResult(repoRoot, teamName) {
  ...
  for (const metaPath of paths) {
    ...
    try {
      ...
      process.stderr.write(`[csp] warning: worktrees.json parse error at ${metaPath}: ${message}
`);
    }
  }
...
function listRootAgentsBackupIssues(repoRoot, teamName, entries) {
  const workersDir = join17(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName), "workers");
...
function writeMetadata(repoRoot, teamName, entries) {
  ...
  validateResolvedPath(metaPath, join17(getCspRoot(repoRoot), "state", "team"));
  ensureDirWithMode(join17(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName)));
...
function ensureWorkerWorktree(teamName, workerName, repoRoot, options = {}) {
  ...
  validateResolvedPath(wtPath, join17(getCspRoot(repoRoot), "team"));
  try {
  ...
  const wtDir = join17(getCspRoot(repoRoot), "team", sanitizeName(teamName), "worktrees");
...
function checkWorkerWorktreeRemovalSafety(teamName, workerName, repoRoot, worktreePath) {
  ...
  validateWorktreeRemovalTarget({
    ...
    expectedRoots: [join17(getCspRoot(repoRoot), "team", sanitizeName(teamName), "worktrees")],
...
function cleanupTeamWorktrees(teamName, repoRoot) {
  ...
  for (const entry of entries) {
    try {
      ...
      process.stderr.write(`[csp] warning: preserved worktree ${entry.path}: ${reason}
`);
    }
  }
  return { removed, preserved };
...
function leaderInboxPath(teamName, cwd) {
  ...
  return join21(cwd, `.csp/state/team/${safe}/leader/inbox.md`);
}
...
function extendLeaderBootstrapPrompt(teamName) {
  const safe = sanitizeName(teamName);
  const path4 = `.csp/state/team/${safe}/leader/inbox.md`;
...
function mergerWorktreePathFor(repoRoot, teamName) {
  return join23(getCspRoot(repoRoot), "team", sanitizeName(teamName), "merger");
}
...
function assertRuntimeV2Gate() {
  if (!isRuntimeV2Enabled()) {
    throw new Error("auto-merge requires runtime v2 (CSP_RUNTIME_V2 is explicitly disabled).");
  }
}
...
async function startMergeOrchestrator(config) {
  ...
  validateResolvedPath(mergerPath, join23(getCspRoot(config.repoRoot), "team"));
...
async function spawnV2Worker(opts) {
  ...
  const instructionStateRoot = opts.worktreePath ? "$CSP_TEAM_STATE_ROOT" : void 0;
  ...
  const envVars = {
    ...
    CSP_TEAM_STATE_ROOT: teamStateRoot(opts.cwd, opts.teamName),
    CSP_TEAM_LEADER_CWD: opts.cwd,
    ...opts.worktreePath ? { CSP_TEAM_WORKTREE_PATH: opts.worktreePath } : {},
    ...opts.workerCwd ? { CSP_TEAM_WORKER_CWD: opts.workerCwd } : {}
  };
  ...
  const modelForAgent = opts.model ?? (() => {
    if (opts.agentType === "codex") {
      return process.env.CSP_EXTERNAL_MODELS_DEFAULT_CODEX_MODEL || process.env.CSP_CODEX_DEFAULT_MODEL || void 0;
    }
    if (opts.agentType === "gemini") {
      return process.env.CSP_EXTERNAL_MODELS_DEFAULT_GEMINI_MODEL || process.env.CSP_GEMINI_DEFAULT_MODEL || void 0;
    }
    if (opts.agentType === "grok") {
      return process.env.CSP_EXTERNAL_MODELS_DEFAULT_GROK_MODEL || process.env.CSP_GROK_DEFAULT_MODEL || void 0;
    }
...
async function startTeamV2(config) {
  ...
  let worktreeMode = normalizeTeamWorktreeMode(
    process.env.CSP_TEAM_WORKTREE_MODE ?? pluginCfg.team?.ops?.worktreeMode
  );
  let autoMergeLeaderBranch;
  if (config.autoMerge) {
    if (!isRuntimeV2Enabled()) {
      throw new Error("auto-merge requires CSP_RUNTIME_V2=1 (this feature is v2-only).");
    }
  ...
  await mkdir10(join24(getCspRoot(leaderCwd), "state", "team", sanitized, "mailbox"), { recursive: true });
...
`
  ...
  try {
    for (let i = 0; i < workerNames.length; i++) {
      ...
      const overlayPath = await writeWorkerOverlay({
        ...
        ...workerWorktrees.has(wName) ? { instructionStateRoot: "$CSP_TEAM_STATE_ROOT" } : {}
      });
  ...
  const permissionsSnapshot = {
    approval_mode: process.env.CSP_APPROVAL_MODE || "default",
    sandbox_mode: process.env.CSP_SANDBOX_MODE || "default",
    network_access: process.env.CSP_NETWORK_ACCESS === "1"
  };
  const teamManifest = {
    schema_version: 2,
    name: sanitized,
...
`);
  ...
  for (const w of config.workers) {
    try {
      ...
      const shutdownAckPath = w.worktree_path ? `$CSP_TEAM_STATE_ROOT/workers/${w.name}/shutdown-ack.json` : TeamPaths.shutdownAck(sanitized, w.name);
...
async function resumeTeamV2(teamName, cwd) {
  ...
  if (!config) return null;
  try {
    const sessionName2 = config.tmux_session || `csp-team-${sanitized}`;
...
async function findActiveTeamsV2(cwd) {
  const root = join24(getCspRoot(cwd), "state", "team");
...
function stateRoot(cwd, teamName) {
  validateTeamName(teamName);
  return join19(cwd, `.csp/state/team/${teamName}`);
}
...
async function resumeTeam(teamName, cwd) {
  ...
  const sName = configData.tmuxSession || `csp-team-${teamName}`;
  try {
...
function teamStateExists(teamName, candidateCwd) {
  ...
  const teamRoot = join25(getCspRoot(candidateCwd), "state", "team", teamName);
...
function parseTeamWorkerContextFromEnv(env = process.env) {
  return parseTeamWorkerEnv(env.CSP_TEAM_WORKER) ?? parseTeamWorkerEnv(env.OMX_TEAM_WORKER);
}
function readTeamStateRootFromEnv(env = process.env) {
  const candidate = typeof env.CSP_TEAM_STATE_ROOT === "string" && env.CSP_TEAM_STATE_ROOT.trim() !== "" ? env.CSP_TEAM_STATE_ROOT.trim() : typeof env.OMX_TEAM_STATE_ROOT === "string" && env.OMX_TEAM_STATE_ROOT.trim() !== "" ? env.OMX_TEAM_STATE_ROOT.trim() : "";
...
async function executeTeamCleanupViaRuntime(teamName, cwd) {
  ...
  if (isLegacyRuntimeConfig(config)) {
    ...
    const sessionName2 = typeof legacyConfig.tmuxSession === "string" && legacyConfig.tmuxSession.trim() !== "" ? legacyConfig.tmuxSession.trim() : `csp-team-${teamName}`;
...
function stateRootToWorkingDirectory(stateRoot2) {
  ...
  for (const marker of ["/.csp/state/team/", "/.omx/state/team/"]) {
  ...
  for (const marker of ["/.csp/state", "/.omx/state"]) {
    const idx = normalized.lastIndexOf(marker);
    if (idx >= 0) {
      const workspaceRoot = absolute.slice(0, idx);
      if (workspaceRoot && workspaceRoot !== "/") return workspaceRoot;
...
var WORKTREE_TRIGGER_STATE_ROOT = "$CSP_TEAM_STATE_ROOT";
...
function getPlansDirCandidates(cwd) {
  return [join26(getCspRoot(cwd), "plans"), join26(cwd, ".omx", "plans")];
}
...
// src/cli/team.ts
init_worktree_paths();
var JOB_ID_PATTERN = /^csp-[a-z0-9]{1,16}$/;
...
var TEAM_API_USAGE = `
Usage:
  csp team api <operation> --input '<json>' [--json] [--cwd DIR]

Supported operations:
  ${Array.from(SUPPORTED_API_OPERATIONS).join(", ")}
`.trim();
function getTeamWorkerIdentityFromEnv(env = process.env) {
  const csp = typeof env.CSP_TEAM_WORKER === "string" ? env.CSP_TEAM_WORKER.trim() : "";
  if (csp) return csp;
...
function resolveJobsDir(env = process.env) {
  return env.CSP_JOBS_DIR || getGlobalOmcStatePath("team-jobs");
}
function resolveRuntimeCliPath(env = process.env) {
  if (env.CSP_RUNTIME_CLI_PATH) {
    return env.CSP_RUNTIME_CLI_PATH;
  }
...
function teamStateRoot2(cwd, teamName) {
  return join27(getCspRoot(cwd), "state", "team", teamName);
}
...
function generateJobId(now = Date.now()) {
  return `csp-${now.toString(36)}${randomUUID6().slice(0, 8)}`;
}
...
function autoTeamName(task) {
  ...
  return `csp-${slug}-${Date.now().toString(36).slice(-4)}`;
}
  ...
  if (!canonicalOperation || !SUPPORTED_API_OPERATIONS.has(canonicalOperation)) {
    return {
      ...
      error: {
        ...
        message: `Unsupported csp team api operation: ${operation}`
      }
    };
  }
...
var TEAM_USAGE = `
Usage:
  csp team start --agent <claude|codex|gemini|cursor|grok>[,<agent>...] --task "<task>" [--count N] [--name TEAM] [--cwd DIR] [--new-window] [--auto-merge] [--json]
  csp team status <job_id|team_name> [--json] [--cwd DIR]
  csp team wait <job_id> [--timeout-ms MS] [--json]
  csp team cleanup <job_id> [--grace-ms MS] [--json]
  csp team resume <team_name> [--json] [--cwd DIR]
  csp team shutdown <team_name> [--force] [--json] [--cwd DIR]
  csp team api <operation> [--input '<json>'] [--json] [--cwd DIR]
  csp team [ralph] <N:agent-type[:role]> "task" [--json] [--cwd DIR] [--new-window]

Worktrees:
  Native per-worker git worktree mode is opt-in/config-gated with team.ops.worktreeMode or CSP_TEAM_WORKTREE_MODE=detached|named.
...
Auto-merge (v2-only):
  --auto-merge          Enable per-commit auto-merge to leader and auto-rebase fanout.
                        Each worker runs in a dedicated git worktree on csp-team/{team}/{worker}.
                        Bursts of rapid worker commits coalesce to a single merge of HEAD.
                        Requires CSP_RUNTIME_V2=1. Leader branch must not be 'main' or 'master'.
                        Equivalent to CSP_TEAMS_AUTO_MERGE=1.

Examples:
  csp team start --agent codex --count 2 --task "review auth flow" --new-window
  csp team status csp-abc123
  csp team status auth-review
  csp team resume auth-review
  csp team shutdown auth-review --force
  csp team api list-tasks --input '{"teamName":"auth-review"}' --json
  csp team 3:codex "refactor launch command"
...
function parseStartArgs(args) {
  ...
  let autoMerge = process.env.CSP_TEAMS_AUTO_MERGE === "1";
  for (let i = 0; i < args.length; i += 1) {
    ...
    throw new Error(`Unknown argument for "csp team start": ${token}`);
  }
...
function parseCommonJobArgs(args, command) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    throw new Error(`Unknown argument for "csp team ${command}": ${token}`);
  }
  if (!target) {
    throw new Error(`Missing required target for "csp team ${command}".`);
  }
  return {
    target,
    json,
...
function parseTeamTargetArgs(args, command) {
  ...
  if (!teamName) {
    throw new Error(`Missing required <team_name> for "csp team ${command}".`);
  }
  return {
    teamName,
    json,
...
function parseApiArgs(args) {
  ...
  for (let i = 0; i < args.length; i += 1) {
    ...
    throw new Error(`Unknown argument for "csp team api": ${token}`);
  }
  if (!operation) {
    throw new Error(`Missing required <operation> for "csp team api"
...
function parseLegacyStartAlias(args) {
  ...
  if (shortFollowup) {
    ...
    const command = `csp team ${ralph ? "ralph " : ""}${spec} ${JSON.stringify(task)}`;
    const approvedHintOutcome = readApprovedExecutionLaunchHintOutcome(cwd, "team", {
      task,
      command
    });
