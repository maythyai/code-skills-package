...
var TMUX_SESSION_PREFIX = "csp-team";
...
function validateConfigPath(configPath2, homeDir, claudeConfigDir) {
  ...
  const normalizedOmcDir = (0, import_path15.resolve)(homeDir, ".csp");
  const hasOmcComponent = resolved.includes("/.csp/") || resolved.endsWith("/.csp");
...
function main() {
  ...
  if (!validateConfigPath(configPath2, home, claudeConfigDir)) {
    console.error(`Config path must be under ~/ with ${claudeConfigDir} or ~/.csp/ subpath: ${configPath2}`);
    process.exit(1);
  }
  let config;
  try {
