...
async function killWorkerPanes(opts) {
  ...
  const shutdownPath = (0, import_path9.join)(getCspRoot(cwd), "state", "team", teamName, "shutdown.json");
  try {
...
function getConfigPaths() {
  ...
  return {
    user: (0, import_path5.join)(userConfigDir, "claude-csp", "config.jsonc"),
    project: (0, import_path5.join)(process.cwd(), ".claude", "csp.jsonc")
  };
}
...
function loadSecurityFromConfigFiles() {
  const paths = [
    (0, import_path6.join)(process.cwd(), ".claude", "csp.jsonc"),
    (0, import_path6.join)(getConfigDir(), "claude-csp", "config.jsonc")
  ];
...
function getTrustedPrefixes() {
  ...
  const custom = (process.env.CSP_TRUSTED_CLI_DIRS ?? "").split(":").map((part) => part.trim()).filter(Boolean).filter((part) => (0, import_path7.isAbsolute)(part));
  trusted.push(...custom);
  return trusted;
}
...
function getWorkerEnv(teamName, workerName2, agentType, env = process.env) {
  ...
  const workerEnv = {
    CSP_TEAM_WORKER: `${teamName}/${workerName2}`,
...
var CSP_CLI_BINARY = "csp";
var CSP_PLUGIN_BRIDGE_PREFIX = 'node "$CLAUDE_PLUGIN_ROOT"/bridge/cli.cjs';
...
// src/team/worker-bootstrap.ts
var DEFAULT_INSTRUCTION_STATE_ROOT = ".csp/state";
...
- **Environment**: CSP_TEAM_WORKER=${teamName}/${workerName2}
...
async function composeInitialInbox(teamName, workerName2, content, cwd, cliOutputContract) {
  const inboxPath = (0, import_path12.join)(cwd, `.csp/state/team/${teamName}/workers/${workerName2}/inbox.md`);
...
async function appendToInbox(teamName, workerName2, message, cwd) {
  ...
  const inboxPath = (0, import_path12.join)(cwd, `.csp/state/team/${safeTeam}/workers/${safeWorker}/inbox.md`);
...
async function ensureWorkerStateDir(teamName, workerName2, cwd) {
  const workerDir = (0, import_path12.join)(cwd, `.csp/state/team/${teamName}/workers/${workerName2}`);
  await (0, import_promises2.mkdir)(workerDir, { recursive: true });
  const mailboxDir = (0, import_path12.join)(cwd, `.csp/state/team/${teamName}/mailbox`);
  await (0, import_promises2.mkdir)(mailboxDir, { recursive: true });
  const tasksDir = (0, import_path12.join)(cwd, `.csp/state/team/${teamName}/tasks`);
...
async function writeWorkerOverlay(params) {
  ...
  const overlayPath = (0, import_path12.join)(cwd, `.csp/state/team/${teamName}/workers/${workerName2}/AGENTS.md`);
...
function getWorktreePath(repoRoot, teamName, workerName2) {
  return (0, import_node_path2.join)(getCspRoot(repoRoot), "team", sanitizeName(teamName), "worktrees", sanitizeName(workerName2));
}
function getBranchName(teamName, workerName2) {
  return `csp-team/${sanitizeName(teamName)}/${sanitizeName(workerName2)}`;
}
...
function getMetadataPath(repoRoot, teamName) {
  return (0, import_node_path2.join)(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName), "worktrees.json");
}
function getLegacyMetadataPath(repoRoot, teamName) {
  return (0, import_node_path2.join)(getCspRoot(repoRoot), "state", "team-bridge", sanitizeName(teamName), "worktrees.json");
}
function getWorkerStateDir(repoRoot, teamName, workerName2) {
  return (0, import_node_path2.join)(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName), "workers", sanitizeName(workerName2));
}
...
function listRootAgentsBackupIssues(repoRoot, teamName, entries) {
  const workersDir = (0, import_node_path2.join)(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName), "workers");
...
function writeMetadata(repoRoot, teamName, entries) {
  ...
  validateResolvedPath(metaPath, (0, import_node_path2.join)(getCspRoot(repoRoot), "state", "team"));
  ensureDirWithMode((0, import_node_path2.join)(getCspRoot(repoRoot), "state", "team", sanitizeName(teamName)));
...
function ensureWorkerWorktree(teamName, workerName2, repoRoot, options = {}) {
  ...
  validateResolvedPath(wtPath, (0, import_node_path2.join)(getCspRoot(repoRoot), "team"));
  try {
  ...
  const wtDir = (0, import_node_path2.join)(getCspRoot(repoRoot), "team", sanitizeName(teamName), "worktrees");
...
function checkWorkerWorktreeRemovalSafety(teamName, workerName2, repoRoot, worktreePath) {
  ...
  validateWorktreeRemovalTarget({
    ...
    expectedRoots: [(0, import_node_path2.join)(getCspRoot(repoRoot), "team", sanitizeName(teamName), "worktrees")],
...
var TeamPaths = {
  ...
  workerDir: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}`,
  heartbeat: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/heartbeat.json`,
  inbox: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/inbox.md`,
  outbox: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/outbox.jsonl`,
  ready: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/.ready`,
  overlay: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/AGENTS.md`,
  shutdownAck: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/shutdown-ack.json`,
  mailbox: (teamName, workerName2) => `.csp/state/team/${teamName}/mailbox/${workerName2}.json`,
  mailboxLockDir: (teamName, workerName2) => `.csp/state/team/${teamName}/mailbox/.lock-${workerName2}`,
  ...
  workerStatus: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/status.json`,
  workerIdleNotify: (teamName) => `.csp/state/team/${teamName}/worker-idle-notify.json`,
  workerPrevNotifyState: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/prev-notify-state.json`,
  ...
  workerIdentity: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/identity.json`,
  workerAgentsMd: (teamName) => `.csp/state/team/${teamName}/worker-agents.md`,
  shutdownRequest: (teamName, workerName2) => `.csp/state/team/${teamName}/workers/${workerName2}/shutdown-request.json`
};
...
function canonicalTasksDir(teamName, cwd) {
  ...
  validateResolvedPath(dir, (0, import_path14.join)(getCspRoot(root), "state", "team"));
  return dir;
}
...
function stateRoot(cwd, teamName) {
  validateTeamName(teamName);
  return (0, import_path15.join)(cwd, `.csp/state/team/${teamName}`);
}
...
async function writePanesTrackingFileIfPresent(runtime) {
  const jobId = process.env.CSP_JOB_ID;
  const cspJobsDir = process.env.CSP_JOBS_DIR;
  if (!jobId || !cspJobsDir) return;
  const panesPath = (0, import_path15.join)(cspJobsDir, `${jobId}-panes.json`);
...
function buildInitialTaskInstruction(teamName, workerName2, task, taskId) {
  const donePath = `.csp/state/team/${teamName}/workers/${workerName2}/done.json`;
  return [
...
var DEFAULT_FACTCHECK_POLICY = {
  ...
  forbidden_path_prefixes: ["${CLAUDE_CONFIG_DIR}/plugins/cache/csp/"],
  forbidden_path_substrings: ["/.csp/", ".csp-config.json"],
...
function expandTokens(value, workspace) {
  ...
  const ws = workspace ?? process.env.CSP_WORKSPACE ?? process.cwd();
...
// src/team/dispatch-queue.ts
var CSP_DISPATCH_LOCK_TIMEOUT_ENV = "CSP_TEAM_DISPATCH_LOCK_TIMEOUT_MS";
...
function leaderInboxPath(teamName, cwd) {
  ...
  return (0, import_path20.join)(cwd, `.csp/state/team/${safe}/leader/inbox.md`);
}
...
function mergerWorktreePathFor(repoRoot, teamName) {
  return (0, import_node_path4.join)(getCspRoot(repoRoot), "team", sanitizeName(teamName), "merger");
}
...
async function startMergeOrchestrator(config) {
  ...
  validateResolvedPath(mergerPath, (0, import_node_path4.join)(getCspRoot(config.repoRoot), "team"));
...
async function startTeamV2(config) {
  ...
  await (0, import_promises10.mkdir)((0, import_path22.join)(getCspRoot(leaderCwd), "state", "team", sanitized, "mailbox"), { recursive: true });
...
function assertAutoMergeRuntimeSupported(useV2, autoMerge) {
  if (autoMerge && !useV2) {
    throw new Error("--auto-merge requires runtime v2; unset CSP_RUNTIME_V2=0 or disable --auto-merge");
  }
}
...
async function writeResultArtifact(output, finishedAt, jobId = process.env.CSP_JOB_ID, cspJobsDir = process.env.CSP_JOBS_DIR) {
  if (!jobId || !cspJobsDir) return;
  const resultPath = (0, import_path23.join)(cspJobsDir, `${jobId}-result.json`);
...
function buildTerminalCliResult(stateRoot2, teamName, phase, workerCount, startTimeMs) {
  ...
  return {
    ...
    notice: `[runtime-cli] phase=${phase} reached terminal state; preserving team state for inspection. Run "csp team shutdown ${teamName}" when explicit cleanup is desired.
`
  };
}
async function writePanesFile(jobId, paneIds, leaderPaneId, sessionName2, ownsWindow) {
  const cspJobsDir = process.env.CSP_JOBS_DIR;
  if (!jobId || !cspJobsDir) return;
  const panesPath = (0, import_path23.join)(cspJobsDir, `${jobId}-panes.json`);
  await (0, import_promises11.writeFile)(
    panesPath + ".tmp",
...
`);
  ...
  const stateRoot2 = (0, import_path23.join)(cwd, `.csp/state/team/${teamName}`);
  const config = {
    teamName,
    workerCount,
    agentTypes,
