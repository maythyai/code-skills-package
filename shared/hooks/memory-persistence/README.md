...
These lifecycle hook definitions document CSP's memory persistence contract for Claude Code plugin and manual installs.
...
- `session-activity-tracker.js` records tool usage and file activity for CSP status and observability.
...
| `PostToolUse` | `post:session-activity-tracker` | Record tool and file activity for CSP metrics | no |
...
- Bound context loaded at session start with `CSP_SESSION_START_MAX_CHARS`.
- Allow opt-out with `CSP_SESSION_START_CONTEXT=off`.
- Keep lifecycle hooks profile-gated through `CSP_HOOK_PROFILE` and `CSP_DISABLED_HOOKS`.
