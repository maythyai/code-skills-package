...
调研收敛后 spawn `roadmap-planner`，基于调研结论 + 既有 `docs/strategy/ROADMAP.md`，规划**长周期、多版本结构化产品路线**：从当前最新版本号往后推演 vX.Y.0 序列（如 v0.22、v0.23 … 一直到 v1.50+），每个版本明确主题、新增 Feature 清单、对应解决的用户场景/竞品差距、优先级与依赖。不要局限在一两个版本，深入挖掘到功能饱和为止。路线写回 `docs/strategy/ROADMAP.md` 。
