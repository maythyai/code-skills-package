...
- **评审/批准 gate（人工决策点，非自主阶段）**：三处人工决策 gate——① **PRD 评审**（01 完成前，reviewer≠author，`status: Approved` 才进 02）；② **Git 发布**（06，默认 off 需人工确认，tag+GitHub Release 一起做）；③ **07 复盘**（里程碑后可选触发）。gate 不替代阶段穷尽——PRD 评审通过前 01 不算 done。
