...
description: >
  ...
  Use when working on design tasks related to stories.
version: "1.0.0"
layer: 3
category: design
phase: plan
...
2. 读取项目目录 `.csp/spark/context/brief.json` / `frame.json` / `scope.json` / `journey.json` / `hmw.json`
...
2. **写入项目文件**：`.csp/spark/context/stories.json`（目录不存在时先创建）

3. **额外保存 Markdown 报告**：`.csp/spark/stories/[direction-slug].md`，便于人类阅读和分享。
...
- **链式上下文双通道**：写入 `.csp/spark/context/stories.json` + 会话内 marker block，下游 PRD / Sitemap / Flow Web/Mobile 可直接读取
...
| **Notion / 飞书** | Step 4 输出后 | Story 集合写入团队 wiki 作为需求文档，便于跨角色对齐 | 未装时输出本地 .md，提示手动归档 |
...
**Step 1 — 写盘到 `.csp/spark/context/stories.json`**（必做，主持久化通道；目录不存在先创建）。写入以下完整 JSON：
...
```
<!-- spark-context:stories ref=".csp/spark/context/stories.json" -->
...
```
.csp/spark/stories/[direction-slug].md
```
