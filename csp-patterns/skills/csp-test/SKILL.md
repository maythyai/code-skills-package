...
2. 读取项目目录 `.csp/spark/context/brief.json` 等文件
...
1. **写盘**：`.csp/spark/context/test.json`（目录不存在先创建）
2. **会话内输出紧凑 marker**：

   ```
   <!-- spark-context:test ref=".csp/spark/context/test.json" -->
...
- **链式上下文双通道**：写入 `.csp/spark/context/test.json` + 会话内 marker block，下游 Retro / Metric 可直接读取
...
| 连接器 | 阶段 | 增强能力 | 降级路径 |
| --- | --- | --- | --- |
| **Notion / 飞书** | 执行流程输出后 | 测试报告（任务量化结果 + Findings + 修复优先级）一键写入团队 wiki | 未装时输出本地 `test-{project}.md` |
...
保存到 `.csp/spark/test/[project-slug].md`：
...
**Step 1 — 写盘到 `.csp/spark/context/test.json`**（必做）：
...
- **协作与转录**：协作文档（Notion / Confluence / 腾讯文档）记录 / 录屏（OBS / 系统自带） / 会议自动转录（Otter / 飞书妙记）
