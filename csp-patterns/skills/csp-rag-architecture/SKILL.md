...
Use this skill to design, build, and harden retrieval-augmented generation systems that deliver accurate, grounded, and cost-effective answers from private or domain-specific knowledge bases.

## When to Activate

- Designing a new RAG system from scratch or migrating from naive retrieval
- Evaluating chunking strategies for document collections of varying structure
- Selecting embedding models, vector databases, or reranking approaches
- Debugging poor retrieval quality, hallucination, or latency/cost issues
- Adding hybrid search, reranking, or multi-stage retrieval pipelines
- Building evaluation harnesses for retrieval quality and answer faithfulness
- Optimizing caching, indexing, or serving infrastructure for production scale

## Related Skills

- `csp-llm-app-development` for prompt engineering, function calling, and multi-model routing
- `csp-prompt-engineering` for prompt template management and evaluation frameworks
- `postgres-patterns` for pgvector integration with existing PostgreSQL infrastructure
- `docker-patterns` and `deployment-patterns` for containerized serving
- `clickhouse-io` for analytics on retrieval logs and usage patterns
...
Chunking determines what the system can retrieve and what context the LLM receives. Bad chunking is the most common root cause of poor RAG quality.

### Fixed-Size Chunking

Split text into overlapping windows of N tokens. Simple and predictable.
...
def fixed_size_chunk(
    ...
    Works at the character level with an approximate token ratio.
    For production, use a real tokenizer (tiktoken, sentencepiece).
    """
...
**When to use:** Uniform document collections (articles, documentation), quick prototyping, predictable cost modeling.

**When to avoid:** Structured documents (code, tables, legal contracts) where semantic boundaries matter more than size.

### Semantic Chunking

Split at natural semantic boundaries using sentence or paragraph detection, topic modeling, or embedding similarity thresholds.
...
def semantic_chunk(
    ...
    Groups consecutive sentences whose embeddings exceed the similarity
    threshold, creating chunks that respect topic boundaries.
    """
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < similarity_threshold or len(current_sentences) >= max_chunk_sentences:
...
**When to use:** Heterogeneous documents, FAQ collections, documents with clear topic shifts, multi-topic reports.

### Recursive Chunking

Apply hierarchical splitting: document → sections → paragraphs → sentences → tokens, stopping at a target size.

```python
from langchain_text_splitters import RecursiveCharacterTextSplitter

# Standard recursive splitter (paragraph → sentence → character)
...
**When to use:** Mixed document types, codebases, documents with hierarchical structure, most general-purpose RAG systems. This is the recommended default for new projects.

### Structured Chunking

Parse documents into structured elements (tables, headers, lists, code blocks) and chunk by element type.
...
class StructuredChunk:
    elements: list[dict]
    section_path: list[str]  # e.g. ["Chapter 3", "3.2 Methods", "Data Collection"]
    element_types: list[ElementType]

    @property
    def text(self) -> str:
        """Render chunk as retrieval-friendly text with section context."""
        context = " > ".join(self.section_path)
        parts = [f"[Section: {context}]"]
        for elem in self.elements:
            parts.append(elem["text"])
        return "\n\n".join(parts)


def structured_chunk_document(parsed_doc: dict) -> list[StructuredChunk]:
    """Chunk a parsed document respecting structure.

    Requires a document parser that identifies element types and hierarchy.
    Tools: Unstructured.io, LlamaParse, Azure Document Intelligence, or
    ...
    max_chunk_tokens = 400  # approximate

    for element in parsed_doc["elements"]:
        if element["type"] == ElementType.HEADING:
            level = element.get("level", 1)
            current_section_path = current_section_path[: level - 1] + [element["text"]]
            continue

        # Tables and code blocks are kept atomic
        if element["type"] in (ElementType.TABLE, ElementType.CODE_BLOCK):
            if current_elements:
        ...
        token_estimate = len(element["text"]) // 4
        total_tokens = sum(len(e["text"]) // 4 for e in current_elements) + token_estimate

        if total_tokens > max_chunk_tokens and current_elements:
            chunks.append(StructuredChunk(
                elements=current_elements,
                section_path=list(current_section_path),
                element_types=current_types,
...
| Model | Dimensions | Max Tokens | MTEB Avg | Cost | Notes |
|-------|-----------|------------|----------|------|-------|
