---
name: csp-agent-sort
description: Build an evidence-backed CSP install plan for a specific repo by sorting skills, commands, rules, hooks, and extras into DAILY vs LIBRARY buckets using parallel repo-aware review passes. Use when CSP should be trimmed to what a project actually needs instead of loading the full bundle.
layer: 4
category: patterns
---
...
Use this skill when a repo needs a project-specific CSP surface instead of the default full install.

The goal is not to guess what "feels useful." The goal is to classify CSP components with evidence from the actual codebase.

## When to Use

- A project only needs a subset of CSP and full installs are too noisy
...
- Prefer CSP-native surfaces; do not introduce a second install system
...
```text
skills/csp-frontend-patterns | skill | DAILY | 84 .tsx files, next.config.ts present | core frontend stack
skills/csp-django-patterns   | skill | LIBRARY | no .py files, no pyproject.toml       | not active in this repo
