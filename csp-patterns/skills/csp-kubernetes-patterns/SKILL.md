...
Production-grade patterns for running workloads on Kubernetes.

## When to Activate

- Designing Deployment, Service, and Ingress configurations for a new application
- Tuning resource requests and limits based on actual usage metrics
- Setting up HPA/VPA autoscaling for variable-load workloads
- Hardening pod security with Pod Security Standards and NetworkPolicies
- Designing Helm charts with proper templating and values schema
- Managing multi-environment deployments with Kustomize overlays
- Planning multi-cluster or cluster-autoscaling strategies

## Deployment, Service, and Ingress Design
...
spec:
  replicas: 3
  revisionHistoryLimit: 5             # Keep 5 previous ReplicaSets for rollback
  strategy:
    type: RollingUpdate
    rollingUpdate:
