...
description: >
  ...
  Use when working on design tasks related to qa.
version: "1.0.0"
layer: 3
category: design
phase: verify
...
2. 读取项目目录 `.csp/spark/context/brief.json` / `flow-web.json` / `flow-mobile.json` / `check.json` / `sitemap.json` / `stories.json`
...
2. **写入项目文件**：`.csp/spark/context/qa.json`（目录不存在时先创建）
...
- **链式上下文双通道**：写入 `.csp/spark/context/qa.json` + 会话内 marker block，下游 Retro / Metric 可直接读取
...
```
.csp/spark/qa/[project-slug]-验收报告.md
```

目录不存在时先创建。此文件供团队归档 / 离线查阅，与 `.csp/spark/context/qa.json` 是互补关系（JSON 供链路消费，Markdown 供人阅读）。

⛔ **禁止保存到项目根目录**（如 `QA-设计验收报告.md`），必须统一归入 `.csp/spark/qa/` 目录下。

#### 4.2 双通道 Context 输出

按 [chain-protocol.md](../../chain-protocol.md) §2.1 v1.1 智能适配规则：

**Step 1 — 写盘到 `.csp/spark/context/qa.json`**（必做，主持久化通道；目录不存在先创建）。写入以下完整 JSON：
...
```
<!-- spark-context:qa ref=".csp/spark/context/qa.json" -->
