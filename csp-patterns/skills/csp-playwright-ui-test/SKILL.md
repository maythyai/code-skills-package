...
anti_rationalizations:
  ...
  "下一个性价比最高的遗留项是 X，需要时说一声": "禁止遗留菜单。审计范围圈定后范围内必须穷尽；范围外显式标'本次范围外'列出，不混入遗留菜单。未做项要么本轮做完 done、要么 BLOCKED(附真实外部阻塞+谁解除)、要么 deferred(附真实解除条件如'沙箱开通后补')。'性价比低/优先级低/按指示搁置'不是 deferred 理由——优先级只用于轮内排序，不跨轮延后；自决策授权下下一个该做的项自己拍板做，不抛菜单给用户。"
---
