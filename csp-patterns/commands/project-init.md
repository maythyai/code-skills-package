---
description: Detect a project's stack and produce a dry-run CSP onboarding plan using the repository's install manifests and stack mappings.
---

# /project-init

Create a safe, reviewable CSP onboarding plan for the current project. This command should start in dry-run mode and only write files after explicit user approval.
...
3. Use CSP's installer and manifest tooling. Do not hand-copy files or clone arbitrary remotes as an install shortcut.
...
- CSP config: `csp-install.json`
- optional stack map: `config/project-stack-mappings.json` in the CSP repo

When the CSP checkout is available, use `config/project-stack-mappings.json` as the stack-to-rules/skills reference. If the file is unavailable, fall back to the installed CSP manifests and explicit user choices.
...
3. Resolve the smallest useful CSP plan:
   - project has an `csp-install.json`: `node scripts/install-plan.js --config csp-install.json --json`
...
- `/csp-guide` for interactive feature discovery before installing
