...
```bash
npx --package csp-universal csp loop-status --json
```
...
- `csp loop-status --json` emits machine-readable status for recent local
  Claude transcripts.
- `csp loop-status --home <dir>` scans a different home directory when
  inspecting another local profile or mounted workspace.
- `csp loop-status --transcript <session.jsonl>` inspects one transcript
  directly.
- `csp loop-status --bash-timeout-seconds 1800` adjusts the stale Bash
  threshold.
- `csp loop-status --exit-code` exits `2` when stale loop or tool signals are
...
- `csp loop-status --watch` refreshes status until interrupted.
- `csp loop-status --watch --watch-count 3 --exit-code` refreshes a bounded
