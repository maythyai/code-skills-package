...
```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".csp/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${CSP_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 3 "$CONFIG_TYPE"
```

## Step 2.2: Repair Stale Plugin Cache References

After a marketplace update, Claude Code may still have old CSP cache paths in the running session or plugin registry. Repair those references before any cache cleanup so setup does not repeatedly emit stale plugin directory errors.

```bash
node "${CSP_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/repair-plugin-cache.mjs"
```
...
const b=p.join(d,'plugins','cache','csp','code-skills-package');
...
// Try .csp-version.json second
if(v==='')try{const j=JSON.parse(f.readFileSync('.csp-version.json','utf-8'));v=j.version||''}catch{}
// Try CLAUDE.md header third
if(v==='')for(const c of['.claude/CLAUDE.md',p.join(d,'CLAUDE.md')]){try{const m=f.readFileSync(c,'utf-8').match(/^# code-skills-package.*?(v?\d+\.\d+\.\d+)/m);if(m){v=m[1].replace(/^v/,'');break}}catch{}}
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin code-skills-package"
  else
...
Store the preference in `~/.claude/.csp-config.json`:
...
## Step 2.5: Install CSP CLI Tool

The CSP CLI (`csp` command) provides standalone helper commands such as `csp hud`, `csp teleport`, and `csp team ...`.

First, check if the CLI is already installed:

```bash
if command -v csp &>/dev/null; then
  CSP_CLI_VERSION=$(csp --version 2>/dev/null | head -1 || echo "installed")
  echo "CSP CLI already installed: $CSP_CLI_VERSION"
  CSP_CLI_INSTALLED="true"
else
  CSP_CLI_INSTALLED="false"
fi
```

If `CSP_CLI_INSTALLED` is `"true"`, skip the rest of this step.

If `CSP_CLI_INSTALLED` is `"false"`, use AskUserQuestion:

**Question:** "Would you like to install the CSP CLI globally for standalone helper commands? (`csp`, `csp hud`, `csp teleport`)"
...
if ! command -v npm &>/dev/null; then
  echo "WARNING: npm not found. Cannot install CSP CLI automatically."
  ...
  if npm install -g oh-my-claude-sisyphus 2>&1; then
    echo "CSP CLI installed successfully."
    if command -v csp &>/dev/null; then
      CSP_CLI_VERSION=$(csp --version 2>/dev/null | head -1 || echo "installed")
      echo "Verified: csp $CSP_CLI_VERSION"
    else
      echo "Installed but 'csp' not on PATH. You may need to restart your shell."
    fi
  else
    echo "WARNING: Failed to install CSP CLI (permission issue or network error)."
...
```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".csp/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${CSP_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 4 "$CONFIG_TYPE"
```
