...
- /code-skills-package:team 3:executor "fix all TypeScript errors"
- /code-skills-package:team 5:debugger "fix build errors in src/"
Teams use Claude Code native tools (TeamCreate/SendMessage/TaskCreate).

MCP SERVERS:
Run /code-skills-package:mcp-setup to add tools like web search, GitHub, etc.

HUD STATUSLINE:
The status bar now shows CSP state. Restart Claude Code to see it.

CSP CLI HELPERS (if installed):
- csp hud         - Render the current HUD statusline
- csp teleport    - Create an isolated git worktree
- csp team status - Inspect a running team job
- Session summaries are written to `.csp/sessions/*.json`
...
```
CSP Setup Complete! (Upgraded from 2.x)

GOOD NEWS: Your existing commands still work!
- /ralph, /ultrawork, /csp-plan, etc. all still function

WHAT'S NEW in 3.0:
You no longer NEED those commands. Everything is automatic now:
- Just say "don't stop until done" instead of /ralph
- Just say "fast" or "parallel" instead of /ultrawork
- Just say "plan this" instead of /csp-plan
...
| csp-plan | /csp-plan | "plan the endpoints" |
...
CSP includes rule templates you can copy to your project's `.claude/rules/` directory for automatic context injection:
...
cp "${CSP_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/templates/rules/"*.md .claude/rules/
```
...
```bash
gh api user/starred/Yeachan-Heo/code-skills-package &>/dev/null
```
...
**Question:** "If you're enjoying code-skills-package, would you like to support the project by starring it on GitHub?"
...
```bash
gh api -X PUT /user/starred/Yeachan-Heo/code-skills-package 2>/dev/null && echo "Thanks for starring!" || true
```
...
```bash
echo ""
echo "If you enjoy code-skills-package, consider starring the repo:"
echo "  https://github.com/Yeachan-Heo/code-skills-package"
echo ""
```

## Mark Completion

Get the current CSP version and mark setup complete:

```bash
# Get current CSP version from CLAUDE.md
CSP_VERSION=""
if [ -f ".claude/CLAUDE.md" ]; then
  CSP_VERSION=$(grep -m1 'CSP:VERSION:' .claude/CLAUDE.md 2>/dev/null | sed -E 's/.*CSP:VERSION:([^ ]+).*/\1/' || true)
elif [ -f "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/CLAUDE.md" ]; then
  CSP_VERSION=$(grep -m1 'CSP:VERSION:' "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/CLAUDE.md" 2>/dev/null | sed -E 's/.*CSP:VERSION:([^ ]+).*/\1/' || true)
fi
if [ -z "$CSP_VERSION" ]; then
  CSP_VERSION=$(csp --version 2>/dev/null | head -1 || true)
fi
if [ -z "$CSP_VERSION" ]; then
  CSP_VERSION="unknown"
fi

bash "${CSP_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-progress.sh" complete "$CSP_VERSION"
```
