...
```bash
grep -q "code-skills-package" "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/settings.json" && echo "Plugin verified" || echo "Plugin NOT found - run: claude /install-plugin code-skills-package"
```
...
#### 3.3.3: Configure Team Defaults in csp-config
...
Store the team configuration in `~/.claude/.csp-config.json`:
...
Or by running `/code-skills-package:csp-setup --force` and choosing to enable teams.

## Save Progress

```bash
CONFIG_TYPE=$(jq -r '.configType // "unknown"' ".csp/state/setup-state.json" 2>/dev/null || echo "unknown")
bash "${CSP_SETUP_PLUGIN_ROOT:-${CLAUDE_PLUGIN_ROOT}}/scripts/setup-progress.sh" save 6 "$CONFIG_TYPE"
```
