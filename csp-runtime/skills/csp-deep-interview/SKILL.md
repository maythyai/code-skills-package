...
<Purpose>
Deep Interview implements Ouroboros-inspired Socratic questioning with mathematical ambiguity scoring. It replaces vague ideas with crystal-clear specifications by asking targeted questions that expose hidden assumptions, measuring clarity across weighted dimensions, and refusing to proceed until ambiguity drops below the resolved threshold for this run. The output feeds into a gated pipeline: **deep-interview → csp-plan consensus refinement → pending approval → explicitly approved execution**, ensuring maximum clarity before any mutation starts.
</Purpose>
...
- User wants to explore options or brainstorm -- use `csp-plan` skill instead
...
- Once ready, do **not** bridge into `csp-plan`, `autopilot`, `ralph`, `team`, or the hard-deprecated `csp-autoresearch` CLI. Instead write the mission/evaluator setup artifacts and invoke:
  - `Skill("code-skills-package:autoresearch")`
...
If this raw bundled skill is loaded by Claude Code's native plugin skill loader through `/code-skills-package:deep-interview` or `Skill("code-skills-package:deep-interview")`, do not treat that path as permission to skip rendered CSP setup. The user-facing preferred invocation is `/deep-interview`; do not recommend or advertise `/code-skills-package:deep-interview` as the deep-interview entrypoint. Regardless of invocation path, Phase 0 below remains blocking and must resolve `csp.deepInterview.ambiguityThreshold` from settings before any announcement, state write, question, or ambiguity score.
...
2. **Resolve threshold and source**:
   - Read `csp.deepInterview.ambiguityThreshold` from both files when present.
...
3.6. **Normalize oversized initial context before state init**:
   ...
   - Wait until the summary exists before ambiguity scoring, weakest-dimension selection, brownfield exploration prompts, or any bridge to `csp-plan`, `autopilot`, `ralph`, or `team`.
3.7. **Artifact path discipline**:
   - Final specs MUST be written to `.csp/specs/deep-interview-{slug}.md` exactly.
   - Ephemeral interview artifacts (scoring scratchpads, prompt-safe summaries, transient queues, resume metadata) belong in `.csp/state/` or in `state_write` state, never in the repo root or arbitrary working files.
