...
export async function resolveSelfImprovePaths({ projectRoot = process.cwd(), topic = '', slug = '', sessionId = '' } = {}) {
  ...
  // Falls back to CSP_SESSION_ID env var when explicit arg is absent.
  // Legacy layout (no topic/slug supplied, flat .csp/self-improve/ exists) is
  // preserved as-is — session scoping only applies to the topic-scoped layout.
  const rawSessionId = sessionId && sessionId.trim()
    ? sessionId.trim()
    : (process.env.CSP_SESSION_ID && process.env.CSP_SESSION_ID.trim() ? process.env.CSP_SESSION_ID.trim() : '');
...
function printHelp() {
  process.stdout.write(
    [
      ...
      '- New runs default to .csp/self-improve/topics/<topic-slug>/',
      '- Pass --session-id to isolate parallel runs: .csp/self-improve/topics/<slug>/sessions/<sid>/',
      '- Legacy flat .csp/self-improve/ is preserved only when no topic/slug is supplied and a flat layout already exists',
      '',
    ].join('\n'),
  );
}
