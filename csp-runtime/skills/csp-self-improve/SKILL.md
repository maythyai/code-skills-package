...
- New runs default to `.csp/self-improve/topics/default/`.
- When the user provides a topic or slug, use `.csp/self-improve/topics/{topic_slug}/`.
- Legacy single-track state at `.csp/self-improve/` remains valid only as a compatibility fallback when no explicit topic/slug is supplied and that flat layout already exists.
...
CSP mode lifecycle: `.csp/state/sessions/{sessionId}/self-improve-state.json`
...
| Step | Role | CSP Agent | Model |
|------|------|-----------|-------|
| Research | Codebase analysis + hypothesis generation | general-purpose Agent | opus |
| Planning | Hypothesis → structured plan | code-skills-package:planner | opus |
| Architecture Review | 6-point plan review | code-skills-package:architect | opus |
| Critic Review | Harness rule enforcement | code-skills-package:critic | opus |
| Execution | Implement plan + run benchmark | code-skills-package:executor | opus |
| Git Operations | Atomic merge/tag/PR | code-skills-package:git-master | sonnet |
...
All git operations happen inside the target repo, NOT in the CSP project root.
...
- **Worktree setup** (SKILL.md creates before each executor):
  ...
  ```
- **Winner merges** via `code-skills-package:git-master`:
  ```
...
Spawn N `code-skills-package:planner`(model=opus) agents in parallel (N = `number_of_agents` from settings).
...
**6a. Architecture Review**: Spawn `code-skills-package:architect` with the plan + 6-point checklist:
...
**6b. Critic Review**: Spawn `code-skills-package:critic` with the plan + harness rules:
...
For each approved plan, spawn `code-skills-package:executor`(model=opus) in parallel.
...
4. **Ranked-candidate loop** — for each candidate in rank order (best first):
   ...
   b. **Merge** via `code-skills-package:git-master`: `git merge experiment/round_{n}_executor_{id} --no-ff -m "Iteration {n}: {hypothesis} (score: {before} → {after})"`
...
4. Print summary report:
   ...
   ```
5. Run `/code-skills-package:cancel` for clean state cleanup
