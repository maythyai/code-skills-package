...
USER SKILLS (~/.claude/skills/csp-learned/):
| Name              | Triggers           | Quality | Usage | Scope |
|-------------------|--------------------|---------|-------|-------|
| error-handler     | fix, error         | 95%     | 42    | user  |
| api-builder       | api, endpoint      | 88%     | 23    | user  |

PROJECT SKILLS (.csp/skills/):
| Name              | Triggers           | Quality | Usage | Scope   |
|-------------------|--------------------|---------|-------|---------|
...
**Built-in skill note:** Built-in skills are bundled with code-skills-package and are discoverable/readable, but not removed or edited through `/skill remove` or `/skill edit`.
...
5. **Ask for scope:**
   - `user` → `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/csp-learned/<name>/SKILL.md`
   - `project` → `.csp/skills/<name>/SKILL.md`
...
```
/code-skills-package:<name> example-arg
```
...
✓ Created skill at ~/.claude/skills/csp-learned/custom-logger/SKILL.md
...
1. **Search for skill** in both scopes:
   - `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/csp-learned/<name>/SKILL.md`
   - `.csp/skills/<name>/SKILL.md`
2. **If found:**
   - Display skill info (name, description, scope)
   - **Ask for confirmation:** "Delete '<name>' skill from <scope>? (yes/no)"
3. **If confirmed:**
   - Delete entire skill directory (e.g., `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/csp-learned/<name>/`)
...
Assistant: Found skill 'old-logger' in user scope:
  ...
  Scope: user (~/.claude/skills/csp-learned/)
...
```
Skill: custom-logger
Scope: user (~/.claude/skills/csp-learned/custom-logger/)
Description: Enhanced logging with structured output
Triggers: log, logger, logging
Argument Hint: <level> [message]
Quality: 95% (if available)
Usage Count: 42 times (if available)
File Path: /home/user/.claude/skills/csp-learned/custom-logger/SKILL.md
...
Description: Enhanced logging with structured output
Triggers: log, logger, logging
File: ~/.claude/skills/csp-learned/custom-logger/SKILL.md
...
**Behavior:**
1. **Scan both scopes:**
   - User skills: `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/csp-learned/`
   - Project skills: `.csp/skills/`
...
User: yes
Assistant: ✓ Copied 'error-handler' to .csp/skills/
...
USER_SKILLS_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/csp-learned"
...
echo "=== USER-LEVEL SKILLS (~/.claude/skills/csp-learned/) ==="
if [ -d "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/csp-learned" ]; then
  USER_COUNT=$(find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/csp-learned" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $USER_COUNT"

  if [ $USER_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find "${CLAUDE_CONFIG_DIR:-$HOME/.claude}/skills/csp-learned" -name "*.md" -type f -exec sh -c '
      FILE="$1"
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.csp/skills/) ==="
if [ -d ".csp/skills" ]; then
  PROJECT_COUNT=$(find ".csp/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".csp/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
Then ask for scope:
- **User-level** (~/.claude/skills/csp-learned/) - Available across all projects
- **Project-level** (.csp/skills/) - Only for this project
...
- `/code-skills-package:skill list` - Show detailed skill inventory
- `/code-skills-package:skill add` - Start skill creation (invoke skillify)
- `/code-skills-package:skill scan` - Scan both skill directories
...
**Version Control**: Project-level skills (`.csp/skills/`) are intended to be committed with your code so the whole team benefits. In linked worktrees, uncommitted skills remain local to that worktree and disappear if it is removed.
...
- `/code-skills-package:skillify` - Extract a skill from current conversation (`/code-skills-package:learner` is a deprecated alias)
- `/code-skills-package:note` - Save quick notes (less formal than skills)
- `/code-skills-package:deepinit` - Generate AGENTS.md codebase hierarchy
...
```
> /code-skills-package:skill list

Checking skill directories...
✓ User skills directory exists: ~/.claude/skills/csp-learned/
✓ Project skills directory exists: .csp/skills/
...
- Run `/code-skills-package:skill list` periodically to review your skill library
