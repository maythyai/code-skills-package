...
```
/code-skills-package:scicsp <goal>                    # Standard research with user checkpoints
/code-skills-package:scicsp AUTO: <goal>              # Fully autonomous until complete
/code-skills-package:scicsp status                    # Check current research session status
/code-skills-package:scicsp resume                    # Resume interrupted research session
/code-skills-package:scicsp list                      # List all research sessions
/code-skills-package:scicsp report <session-id>       # Generate report for session
```

### Quick Examples

```
/code-skills-package:scicsp What are the performance characteristics of different sorting algorithms?
/code-skills-package:scicsp AUTO: Analyze authentication patterns in this codebase
/code-skills-package:scicsp How does the error handling work across the API layer?
```
...
Task(subagent_type="code-skills-package:scientist", model="haiku", prompt="[RESEARCH_STAGE:1] Investigate...")

// Stage 2 - Standard analysis
Task(subagent_type="code-skills-package:scientist", model="sonnet", prompt="[RESEARCH_STAGE:2] Analyze...")

// Stage 3 - Complex reasoning
Task(subagent_type="code-skills-package:scientist", model="opus", prompt="[RESEARCH_STAGE:3] Deep analysis of...")
```
...
Task(subagent_type="code-skills-package:scientist", model="sonnet", prompt="
...
4. **Cancellation:** `/code-skills-package:cancel` or "stop", "cancel"

### AUTO Mode Example

```
/code-skills-package:scicsp AUTO: Comprehensive security analysis of the authentication system
...
Task(subagent_type="code-skills-package:scientist", model="haiku", prompt="[STAGE:1] Analyze src/api/...")
Task(subagent_type="code-skills-package:scientist", model="haiku", prompt="[STAGE:2] Analyze src/utils/...")
Task(subagent_type="code-skills-package:scientist", model="haiku", prompt="[STAGE:3] Analyze src/components/...")
```
...
Task(subagent_type="code-skills-package:scientist", model="sonnet", prompt="[HYPOTHESIS:A] Test if caching improves...")
Task(subagent_type="code-skills-package:scientist", model="sonnet", prompt="[HYPOTHESIS:B] Test if batching reduces...")
Task(subagent_type="code-skills-package:scientist", model="sonnet", prompt="[HYPOTHESIS:C] Test if lazy loading helps...")
```
...
Task(subagent_type="code-skills-package:scientist", model="opus", prompt="
[CROSS_VALIDATION]
...
| Command | Action |
|---------|--------|
| `/code-skills-package:scicsp status` | Show current session progress |
| `/code-skills-package:scicsp resume` | Resume most recent interrupted session |
| `/code-skills-package:scicsp resume <session-id>` | Resume specific session |
| `/code-skills-package:scicsp list` | List all sessions with status |
| `/code-skills-package:scicsp report <session-id>` | Generate/regenerate report |
| `/code-skills-package:scicsp cancel` | Cancel current session (preserves state) |
...
Progress is preserved in `.csp/research/{session-id}/` for resume.
