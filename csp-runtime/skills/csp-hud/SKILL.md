...
| `/code-skills-package:hud focused` | Switch to focused display (default) |
| `/code-skills-package:hud full` | Switch to full display |
| `/code-skills-package:hud status` | Show detailed HUD status |
...
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude');console.log(f.existsSync(p.join(d,'hud','csp-hud.mjs'))?'EXISTS':'MISSING')"
```

**Step 2:** Verify the plugin is installed:
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude'),b=p.join(d,'plugins','cache','csp','code-skills-package');try{const v=f.readdirSync(b).filter(x=>/^\d/.test(x)).sort((a,c)=>a.localeCompare(c,void 0,{numeric:true}));if(v.length===0){console.log('Plugin not installed - run: /plugin install code-skills-package');process.exit()}const l=v[v.length-1],h=p.join(b,l,'dist','hud','index.js');console.log('Version:',l);console.log(f.existsSync(h)?'READY':'NOT_FOUND - try reinstalling: /plugin install code-skills-package')}catch{console.log('Plugin not installed - run: /plugin install code-skills-package')}"
```

**Step 3:** If csp-hud.mjs is MISSING or argument is `setup`, install the HUD wrapper and its dependency from the canonical template:

```bash
HUD_DIR="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud"
mkdir -p "$HUD_DIR/lib"
cp "${CLAUDE_PLUGIN_ROOT}/scripts/lib/hud-wrapper-template.txt" "$HUD_DIR/csp-hud.mjs"
...
```bash
node -e "if(process.platform==='win32'){console.log('Skipped (Windows)')}else{require('fs').chmodSync(require('path').join(process.env.CLAUDE_CONFIG_DIR||require('path').join(require('os').homedir(),'.claude'),'hud','csp-hud.mjs'),0o755);console.log('Done')}"
```
...
```bash
node -e "const p=require('path').join(require('os').homedir(),'.claude','hud','csp-hud.mjs').split(require('path').sep).join('/');console.log(JSON.stringify(p))"
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node ${CLAUDE_CONFIG_DIR:-$HOME/.claude}/hud/csp-hud.mjs"
  }
}
```
...
```json
{
  "statusLine": {
    "type": "command",
    "command": "node C:/Users/username/.claude/hud/csp-hud.mjs"
  }
}
```
...
```bash
node -e "const p=require('path'),f=require('fs'),d=process.env.CLAUDE_CONFIG_DIR||p.join(require('os').homedir(),'.claude'),t=p.join(d,'hud','csp-hud.js');try{if(f.existsSync(t)){f.unlinkSync(t);console.log('Removed legacy csp-hud.js')}else{console.log('No legacy script found')}}catch{}"
```
...
```
[CSP] ralph | ultrawork | todos:2/5
```

### Focused (Default)
Shows all relevant elements:
```
[CSP] branch:main | ralph:3/10 | US-002 | ultrawork skill:planner | ctx:67% | agents:2 | bg:3/5 | todos:2/5
```

### Full
Shows everything including multi-line agent details:
```
[CSP] repo:code-skills-package branch:main | ralph:3/10 | US-002 (2/5) | ultrawork | ctx:[████░░]67% | agents:3 | bg:3/5 | todos:2/5
...
HUD config is stored in `~/.claude/settings.json` under the `cspHud` key (or your custom config directory if `CLAUDE_CONFIG_DIR` is set).

Legacy config location (deprecated): `~/.claude/.csp/hud-config.json`
...
1. Run `/code-skills-package:hud setup` to auto-install and configure
