...
**Options:**
1. **Recommended starter setup** - Fast path for the most common CSP MCP additions
...
- Run /code-skills-package:csp-doctor to diagnose issues

MANAGING MCP SERVERS:
- Add more servers: /code-skills-package:mcp-setup or `claude mcp add ...`
...
If the custom server is meant to provide organization-specific reference material to CSP workflows, prefer a single tool named `get_company_context` that returns markdown via `{ context: string }`.
...
Then point CSP at the full tool name in `.claude/csp.jsonc` or `~/.config/claude-csp/config.jsonc`:
