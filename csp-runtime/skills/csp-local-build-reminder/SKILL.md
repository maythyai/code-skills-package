...
**Always-on reminder for CSP fork development.** When CSP is running in local
mode (HUD shows `[CSP#X.Y.ZL]` with an `L` suffix), Claude Code loads compiled
...
4. The user runs an CSP command and expects new behavior tied to a TS edit.
...
- The user is in a Claude Code session that isn't running CSP locally
  (no `L` in the HUD).
