...
| `attach <session>` | Attach to session | `/psm attach csp:pr-123` |
| `kill <session>` | Kill session | `/psm kill csp:pr-123` |
...
Supported formats:
- **Alias**: `csp#123` (requires `~/.psm/projects.json`)
- **Full**: `owner/repo#123`
...
```json
{
  "aliases": {
    "csp": {
      "repo": "Yeachan-Heo/code-skills-package",
      "local": "~/Workspace/code-skills-package",
      "default_base": "main"
    }
  },
  "defaults": {
...
|------|--------------|--------------|
| PR Review | `psm:csp:pr-123` | `~/.psm/worktrees/csp/pr-123` |
| Issue Fix | `psm:csp:issue-42` | `~/.psm/worktrees/csp/issue-42` |
| Feature | `psm:csp:feat-auth` | `~/.psm/worktrees/csp/feat-auth` |
...
9. **Output session info**:
   ...
   Session ready!
     ...
     ID: csp:pr-123
     Worktree: ~/.psm/worktrees/csp/pr-123
     Tmux: psm:csp:pr-123

   To attach: tmux attach -t psm:csp:pr-123
   ```
...
4. **Format output**:
   ...
   -------------------|---------|----------|---------------------------
   csp:pr-123        | review  | active   | ~/.psm/worktrees/csp/pr-123
   csp:issue-42      | fix     | detached | ~/.psm/worktrees/csp/issue-42
   ```
...
5. **Report**:
   ```
   Cleanup complete:
     Removed: csp:pr-123 (merged)
     Removed: csp:issue-42 (closed)
     Kept: csp:feat-auth (active)
   ```
...
The `csp teleport` command provides a lightweight alternative to full PSM sessions. It creates git worktrees without tmux session management — ideal for quick, isolated development.
...
csp teleport #123
csp teleport owner/repo#123
csp teleport https://github.com/owner/repo/issues/42

# Create worktree for a feature
csp teleport my-feature

# List existing worktrees
csp teleport list

# Remove a worktree
csp teleport remove issue/my-repo-123
csp teleport remove --force feat/my-repo-my-feature
```
...
| `--path <path>` | Custom worktree root directory | `~/Workspace/csp-worktrees/` |
