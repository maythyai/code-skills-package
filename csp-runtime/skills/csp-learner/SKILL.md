...
- **User-level**: `${CLAUDE_CONFIG_DIR:-~/.claude}/skills/csp-learned/<skill-name>.md` - Rare. Only for truly portable insights.
- **Project-level**: `.csp/skills/<skill-name>.md` - Default. Intended to be committed with the repo when you want the team to keep the skill. In linked worktrees, uncommitted skills are still worktree-local and disappear if that worktree is deleted.
...
- /code-skills-package:note - Save quick notes that survive compaction (less formal than skills)
- /code-skills-package:ralph - Start a development loop with learning capture
