...
`/csp-teams` is a legacy compatibility skill for the CLI-first runtime: use `csp team ...` commands (not deprecated MCP runtime tools).

## Usage

```bash
/code-skills-package:csp-teams N:claude "task description"
/code-skills-package:csp-teams N:codex "task description"
/code-skills-package:csp-teams N:gemini "task description"
```
...
```bash
/csp-teams 2:claude "implement auth module with tests"
/csp-teams 2:codex "review the auth module for security issues"
/csp-teams 3:gemini "redesign UI components for accessibility"
```
...
- If `$TMUX` is set, `csp team` can reuse the current tmux window/panes directly.
- If `$TMUX` is empty but `CMUX_SURFACE_ID` is set, report that the user is running inside **cmux**. Do **not** say tmux is missing or that they are "not inside tmux"; `csp team` will create **native cmux splits** for workers.
- If neither `$TMUX` nor `CMUX_SURFACE_ID` is set, report that the user is in a **plain terminal**. `csp team` can still launch a **detached tmux session**, but if they specifically want in-place pane/window topology they should start from a classic tmux session first.
...
- Reject unsupported agent types up front. `/csp-teams` only supports **`claude`**, **`codex`**, and **`gemini`**.
- If the user asks for an unsupported type such as `expert`, explain that `/csp-teams` launches external CLI workers only.
- For native Claude Code team agents/roles, direct them to **`/code-skills-package:team`** instead.
...
`csp team` launches all workers with one shared working directory. For single-repo
...
- If the task references a plan artifact under one repo (for example
  `tool/.csp/plans/task-1200-gwd-gifs.md`) and target paths in sibling repos
...
- Do not anchor the launch cwd to only the repo containing `.csp/plans/...` when
...
- If no safe shared workspace root can be identified, do not launch `/csp-teams`.
...
```bash
csp team <N>:<claude|codex|gemini> "<task>"
```
...
```bash
csp team <N>:<claude|codex|gemini> "<task with absolute plan path and explicit repo paths>" --cwd <workspace-root>
```
...
```bash
csp team status <team-name>
csp team api list-tasks --input '{"team_name":"<team-name>"}' --json
```

Use `csp team api ...` for task claiming, task transitions, mailbox delivery, and worker state updates.

### Phase 5: Shutdown (only when needed)

```bash
csp team shutdown <team-name>
csp team shutdown <team-name> --force
```
...
- `csp_run_team_status`
- `csp_run_team_wait`
- `csp_run_team_cleanup`

If encountered, switch to `csp team ...` CLI commands.
