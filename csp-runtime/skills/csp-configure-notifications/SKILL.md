...
Set up Telegram notifications so CSP can message you when sessions end, need input, or complete background tasks.

### How This Skill Works

This is an interactive, natural-language configuration skill. Walk the user through setup by asking questions with AskUserQuestion. Write the result to `${CLAUDE_CONFIG_DIR:-~/.claude}/.csp-config.json`.

### Step 1: Detect Existing Configuration

```bash
CONFIG_FILE="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/.csp-config.json"
...
2. Send /newbot
3. Choose a name (e.g., "My CSP Notifier")
4. Choose a username (e.g., "my_csp_bot")
...
RESPONSE=$(curl -s -w "\n%{http_code}" \
  ...
  -d "text=CSP test notification - Telegram is configured!")
...
Config saved to: ~/.claude/.csp-config.json

You can also set these via environment variables:
  CSP_TELEGRAM_BOT_TOKEN=123456789:ABCdefGHI...
  CSP_TELEGRAM_CHAT_ID=123456789

To reconfigure: /code-skills-package:configure-notifications telegram
To configure Discord: /code-skills-package:configure-notifications discord
To configure Slack: /code-skills-package:configure-notifications slack
```
...
```bash
export CSP_TELEGRAM_BOT_TOKEN="123456789:ABCdefGHIjklMNOpqrsTUVwxyz"
export CSP_TELEGRAM_CHAT_ID="123456789"
```

Env vars are auto-detected by the notification system without needing `.csp-config.json`.

---

## Discord Setup

Set up Discord notifications so CSP can ping you when sessions end, need input, or complete background tasks.
...
Build the notifications object with the collected values and merge into `.csp-config.json` using jq:
...
curl -s -o /dev/null -w "%{http_code}" \
  ...
  -d "{\"content\": \"${MENTION:+$MENTION\\n}CSP test notification - Discord is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  CSP_DISCORD_WEBHOOK_URL=https://discord.com/api/webhooks/...
  CSP_DISCORD_MENTION=<@1465264645320474637>

To reconfigure: /code-skills-package:configure-notifications discord
To configure Telegram: /code-skills-package:configure-notifications telegram
To configure Slack: /code-skills-package:configure-notifications slack
```
...
**Webhook method:**
```bash
export CSP_DISCORD_WEBHOOK_URL="https://discord.com/api/webhooks/..."
export CSP_DISCORD_MENTION="<@1465264645320474637>"  # optional
```

**Bot API method:**
```bash
export CSP_DISCORD_NOTIFIER_BOT_TOKEN="your-bot-token"
export CSP_DISCORD_NOTIFIER_CHANNEL="your-channel-id"
export CSP_DISCORD_MENTION="<@1465264645320474637>"  # optional
```
...
Set up Slack notifications so CSP can message you when sessions end, need input, or complete background tasks.
...
3. Name your app (e.g., "CSP Notifier") and select your workspace
...
curl -s -o /dev/null -w "%{http_code}" \
  -H "Content-Type: application/json" \
  -d "{\"text\": \"${MENTION_PREFIX}CSP test notification - Slack is configured!\"}" \
  "$WEBHOOK_URL"
```
...
You can also set these via environment variables:
  CSP_SLACK_WEBHOOK_URL=https://hooks.slack.com/services/...
  CSP_SLACK_MENTION=<@U1234567890>

To reconfigure: /code-skills-package:configure-notifications slack
To configure Discord: /code-skills-package:configure-notifications discord
To configure Telegram: /code-skills-package:configure-notifications telegram
```
...
```bash
export CSP_SLACK_WEBHOOK_URL="https://hooks.slack.com/services/T00/B00/xxx"
export CSP_SLACK_MENTION="<@U1234567890>"  # optional
```
...
- `csp --telegram` — Activates Telegram notifications (sets `CSP_TELEGRAM=1`)
...
Customize notification messages per event and per platform using `csp_config.hook.json`.
...
Check if `${CLAUDE_CONFIG_DIR:-~/.claude}/csp_config.hook.json` exists. If it does, show the current configuration. If not, explain what it does.
...
Config file: ~/.claude/csp_config.hook.json
```
...
RAW FIELDS:
  ...
  {{activeMode}}     - Active CSP mode name
...
Read or create `${CLAUDE_CONFIG_DIR:-~/.claude}/csp_config.hook.json` and merge the new settings:
...
- `/code-skills-package:configure-openclaw` — Configure OpenClaw gateway integration
...
If `~/.claude/csp_config.openclaw.json` exists, detect and offer migration:

**Step 1: Detect Legacy Config**
```bash
LEGACY_CONFIG="${CLAUDE_CONFIG_DIR:-$HOME/.claude}/csp_config.openclaw.json"
...
If migrate:
- Read `csp_config.openclaw.json`
- Transform to custom integration format
- Save to `.csp-config.json`
- Backup legacy to `csp_config.openclaw.json.bak`
...
curl -X POST \
  ...
  -d '{"event":"test","instruction":"CSP test notification","timestamp":"'$(date -u +%Y-%m-%dT%H:%M:%SZ)'"}' \
  "$WEBHOOK_URL"
```
