...
2. Claude runs via CLI (skill nesting not supported):
   - `csp ask codex "<codex prompt>"`
   - `csp ask gemini "<gemini prompt>"`

3. Artifacts are written under `.csp/artifacts/ask/`
...
```text
.csp/artifacts/ask/codex-*.md
.csp/artifacts/ask/gemini-*.md
```
...
```bash
/code-skills-package:ccg <task description>
```

Example:

```bash
/code-skills-package:ccg Review this PR - architecture/security via Codex and UX/readability via Gemini
```
