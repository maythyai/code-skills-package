...
- You need durable experiment logs under `.csp/autoresearch/`
...
- You want the deprecated `csp-autoresearch` CLI flow — it is no longer authoritative
</Do_Not_Use_When>
...
<Required_Artifacts>
Canonical persistent storage lives under `.csp/autoresearch/<mission-slug>/` and/or `.csp/logs/autoresearch/<run-id>/`.
...
<Execution_Policy>
- Do not hand execution back to `csp-autoresearch`
- Do not create multi-mission orchestration
- Prefer reusing `csp-runtime/skills/csp-autoresearch/*` runtime/schema helpers where they already match the stricter contract
