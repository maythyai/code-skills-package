...
Intelligent cancellation that detects and cancels the active CSP mode.

**The cancel skill is the standard way to complete and exit any CSP mode.**
...
```
/code-skills-package:cancel
```

Or say: "cancelcsp", "stopcsp"
...
```
ToolSearch(query="select:mcp__plugin_code-skills-package_t__state_clear,mcp__plugin_code-skills-package_t__state_read,mcp__plugin_code-skills-package_t__state_write,mcp__plugin_code-skills-package_t__state_list_active,mcp__plugin_code-skills-package_t__state_get_status")
```
...
**WARNING:** Do NOT use this fallback for `autopilot` or `csp-teams`. Autopilot requires
`state_write(active=false)` to preserve resume data. csp-teams requires tmux session
...
REPO_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || { d="$PWD"; while [ "$d" != "/" ] && [ ! -d "$d/.csp" ]; do d="$(dirname "$d")"; done; echo "$d"; })"

# Cross-platform SHA-256 (macOS: shasum, Linux: sha256sum)
sha256portable() { printf '%s' "$1" | (sha256sum 2>/dev/null || shasum -a 256) | cut -c1-16; }

# Resolve state directory (supports CSP_STATE_DIR centralized storage)
if [ -n "${CSP_STATE_DIR:-}" ]; then
  ...
  CSP_STATE="$CSP_STATE_DIR/${DIR_NAME}-${HASH}/state"
  [ ! -d "$CSP_STATE" ] && { echo "ERROR: State dir not found at $CSP_STATE" >&2; exit 1; }
elif [ "$REPO_ROOT" != "/" ] && [ -d "$REPO_ROOT/.csp" ]; then
  CSP_STATE="$REPO_ROOT/.csp/state"
else
  echo "ERROR: Could not locate .csp state directory" >&2
  exit 1
fi
MODE="ralplan"  # <-- replace with the target mode

# Clear session-scoped state for the specific mode
if [ -n "$SESSION_ID" ] && [ -d "$CSP_STATE/sessions/$SESSION_ID" ]; then
  rm -f "$CSP_STATE/sessions/$SESSION_ID/${MODE}-state.json"
  rm -f "$CSP_STATE/sessions/$SESSION_ID/${MODE}-stop-breaker.json"
  rm -f "$CSP_STATE/sessions/$SESSION_ID/skill-active-state.json"
  ...
  printf '{"active":true,"requested_at":"%s","expires_at":"%s","mode":"%s","source":"bash_fallback"}' \
    "$NOW_ISO" "$EXPIRES_ISO" "$MODE" > "$CSP_STATE/sessions/$SESSION_ID/cancel-signal-state.json"
fi

# Clear legacy state only if no session ID (avoid clearing another session's state)
if [ -z "$SESSION_ID" ]; then
  rm -f "$CSP_STATE/${MODE}-state.json"
fi
```

## Auto-Detection

`/code-skills-package:cancel` follows the session-aware state contract:
- By default the command inspects the current session via `state_list_active` and `state_get_status`, navigating `.csp/state/sessions/{sessionId}/…` to discover which mode is active.
- When a session id is provided or already known, that session-scoped path is authoritative. Legacy files in `.csp/state/*.json` are consulted only as a compatibility fallback if the session id is missing or empty.
- Swarm is a shared SQLite/marker mode (`.csp/state/swarm.db` / `.csp/state/swarm-active.marker`) and is not session-scoped.
...
9. CSP Teams (tmux CLI workers)
10. Plan Consensus (standalone)
11. Self-Improve (standalone — clear state, clean orphaned worktrees, preserve iteration_state for resume, set status: "user_stopped" in the resolved `<self-improve-root>/state/agent-settings.json`; new runs use `.csp/self-improve/topics/<topic-slug>/`, with flat `.csp/self-improve/` retained only for legacy single-track resumes)
...
```
/code-skills-package:cancel --force
```

```
/code-skills-package:cancel --all
```

Steps under the hood:
1. `state_list_active` enumerates `.csp/state/sessions/{sessionId}/…` to find every known session.
2. `state_clear` runs once per session to drop that session’s files.
3. A global `state_clear` without `session_id` removes legacy files under `.csp/state/*.json`, `.csp/state/swarm*.db`, and compatibility artifacts (see list).
4. Team artifacts (`~/.claude/teams/*/`, `~/.claude/tasks/*/`, `.csp/state/team-state.json`) are best-effort cleared as part of the legacy fallback.
   - Cancel for native team does NOT affect csp-teams state, and vice versa.

Every `state_clear` command honors the `session_id` argument, so even force mode still uses the session-aware paths first before deleting legacy files.

Legacy compatibility list (removed only under `--force`/`--all`):
- `.csp/state/autopilot-state.json`
- `.csp/state/ralph-state.json`
- `.csp/state/ralph-plan-state.json`
- `.csp/state/ralph-verification.json`
- `.csp/state/ultrawork-state.json`
- `.csp/state/ultraqa-state.json`
- `.csp/state/swarm.db`
- `.csp/state/swarm.db-wal`
- `.csp/state/swarm.db-shm`
- `.csp/state/swarm-active.marker`
- `.csp/state/swarm-tasks.db`
- `.csp/state/ultrapilot-state.json`
- `.csp/state/ultrapilot-ownership.json`
- `.csp/state/pipeline-state.json`
- `.csp/state/csp-teams-state.json`
- `.csp/state/plan-consensus.json`
- `.csp/state/ralplan-state.json`
- `.csp/state/boulder.json`
- `.csp/state/hud-state.json`
- `.csp/state/subagent-tracking.json`
- `.csp/state/subagent-tracker.lock`
- `.csp/state/rate-limit-daemon.pid`
- `.csp/state/rate-limit-daemon.log`
- `.csp/state/checkpoints/` (directory)
- `.csp/state/sessions/` (empty directory cleanup after clearing sessions)
...
1. Call `state_list_active` to enumerate `.csp/state/sessions/{sessionId}/…` and discover every active session.
...
3. If a `session_id` was supplied to `/code-skills-package:cancel`, skip legacy fallback entirely and operate solely within that session path; otherwise, consult legacy files in `.csp/state/*.json` only if the state tools report no active session. Swarm remains a shared SQLite/marker mode outside session scoping.
...
Report: "No active CSP modes detected. Use --force to clear all state files anyway."
...
4. In force mode, iterate every active session, call `state_clear` per session, then run a global `state_clear` without `session_id` to drop legacy files (`.csp/state/*.json`, compatibility artifacts) and report success. Swarm remains a shared SQLite/marker mode outside session scoping.
5. Team artifacts (`~/.claude/teams/*/`, `~/.claude/tasks/*/`, `.csp/state/team-state.json`) remain best-effort cleanup items invoked during the legacy/global pass.
