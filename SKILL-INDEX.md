...
| **自主执行** | 3 | 2 | 5 | 1 | 🔴 高 | OMC autopilot/ralph + GSD autonomous 合并 |
...
| `designer` | OMC/agents | Agent | UI/UX 设计 | ✅ 保留（独特） |
...
| `kotlin-patterns` | ECC/.kiro | Kotlin |
| `rust-patterns` | ECC/.kiro | Rust |
| `python-patterns` | ECC/.kiro | Python |
| `golang-patterns` | ECC/.kiro | Go |
...
| `deepinit` | OMC | 深度初始化 | ✅ 保留 |
| `omc-doctor` | OMC | 安装诊断 | ✅ 保留 |
| `omc-setup` | OMC | 安装设置 | ✅ 保留 |
| `mcp-setup` | OMC | MCP 配置 | ✅ 保留 |
| `configure-notifications` | OMC | 通知配置 | ✅ 保留 |
| `hud` | OMC | HUD 配置 | ✅ 保留 |
| `skill` (OMC) | OMC | 技能管理 | ✅ 保留 |
| `skillify` | OMC | 技能化 | ✅ 保留 |
| `learner` | OMC | 学习提取 | ✅ 保留 |
| `ai-slop-cleaner` | OMC | AI 代码清理 | ✅ 保留（独特） |
| `self-improve` | OMC | 自我改进 | ✅ 保留（独特） |
...
| `project-session-manager` | OMC | worktree 环境管理 | ✅ 保留 |
| `ultragoal` | OMC | 持久多目标工作流 | ✅ 保留（独特） |
| `ccg` | OMC | 三模型编排 | ✅ 保留（独特） |
| `scientist` | OMC/agents | 数据分析 | ✅ 保留（独特） |
| `git-master` | OMC/agents | Git 专家 | ✅ 保留 |
...
| `golang-*` (5) | ✅ 与 ECC `golang-patterns` 协同 |
| `kotlin-*` (5) | ✅ 与 ECC `kotlin-patterns` 协同 |
| `php-*` (5) | ✅ 与 ECC `php-reviewer` 协同 |
| `python-*` (5) | ✅ 与 ECC `python-patterns` 协同 |
...
| 名称 | 路径 | 描述 | 合并建议 |
|------|------|------|----------|
| Benchmark 系统 | `benchmark/` | OMC vs vanilla Claude 对比评测(Docker+Python) | ✅ 保留(独特) |
| Multi-agent Bridge | `bridge/` | `team.js` + `team-bridge.cjs` + `team-mcp.cjs` 多 agent 协作 | ✅ 与 dmux-workflows 协同 |
| 内建 MCP Server | `mcp-server.cjs` | OMC 内建的 MCP 服务端 | ✅ 保留 |
...
- **通用功能**（code-review, debug, verify）：与 OMC 同类合并，ECC 版本为主
- **内容/营销**：ECC 独有，全部保留
