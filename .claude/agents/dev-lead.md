...
**与 dev-lead 既有 worktree-per-task 衔接**：6.2 的 worktree-per-task 隔离不变；6.4 是在其上加 PR+merge 流水线（从"worktree 内 commit"升级为"worktree→PR→squash-merge"），让并行产出可 review、可门控、可回滚。S 级（单 Feature 纯后端 CRUD）Lead 独立完成时不必开 PR，直接主干 commit 即可——PR 流水线用于 M+ 级并行流。
