...
- **禁止遗留菜单与性价比延后（最隐蔽的留尾）**：收尾时禁止输出“下一个性价比最高的遗留项是 X，需要时说一声”这类菜单——把下一步选择抛给用户就是非法留尾。自决策授权已把范围/优先级/deferred 取舍交给 Agent，下一个该做的项**自己拍板做掉**，不抛菜单。同理，“性价比低/优先级低/范围外/按你的指示搁置”都不是 `deferred` 的合法理由——优先级只用于本轮内排序，不用于跨轮延后；用户说“先搁置”也必须追出显式解除条件（何时做/什么条件做），否则本轮做完或标 `BLOCKED`。`deferred` 的唯一合法理由是**真实外部阻塞**（依赖未就绪/跨团队契约未签/需真实流量/沙箱未开通），不是 Agent 自判的“性价比不高”。命中即停：输出里出现“需要时说一声”“下一个该做 X”“按你的指示搁置”而无对应 `deferred` 解除条件 = 留尾，回去做完或标 BLOCKED。
