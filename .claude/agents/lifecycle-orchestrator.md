...
6. **不前置探测运行时能力**：**禁止**每次 spawn 前先"探测子智能体能否访问文件系统/工具是否可用"——这是偶发环境问题，不是标准步骤。直接 spawn 子 agent，让它读 `.csp/` 干活；**只在子 agent 产出异常（读不到文件/工具报错）时才排查环境**，不前置探测。06 Phase 0.5 工具链健康检查是验**项目构建工具链**（pnpm/tsc/build 可用），不是验 agent 运行时能力——不混淆。
